...
- **スキルとエージェント役割は `.claude/` と 1:1 に対応し、`make md-skill-lint` がそれを強制します。** 検査するのは存在だけです（本文はコピーではなく意味ポートのため）。片側の環境にだけ属するスキルは、[`scripts/skill-lint.mjs`](../scripts/skill-lint.mjs) の `PLATFORM_ONLY_SKILLS` へ理由付きで登録します。
