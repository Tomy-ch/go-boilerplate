...
移設先が ADR の場合、その記録の形をこちらで選ばないこと。既存の ADR-NNNN を書き換える / 新しい ADR を作る / 代わりに `docs/design` か README を使う / 今は移設しない —— このいずれかをユーザーに選んでもらう。承認されたら、英語の正本・隣の `.ja.md`・英日両方の ADR ログ表を揃えて更新する。パッケージ README への追記がその主張を実質的に変える場合は、続く対応として `back-prop` に言及する。
