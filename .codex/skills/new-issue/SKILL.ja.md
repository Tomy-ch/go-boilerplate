...
- **範囲は網羅されているか。** 「X だけがこうしている」という主張なら、呼び出し箇所を推測せず、`node .claude/scripts/graph-affected.ts <symbol> --depth 2` でグラフを逆走査する。結果は各箇所を関係ラベルと `file:line` 付きで示し、不在の主張に必要な証拠になる。先に `AGENTS.md` に従ってグラフの鮮度を確認し、遅れているかシンボルが新しい場合は、全呼出元と全兄弟の網羅的探索へフォールバックする。範囲の主張は不在の主張であり、不在こそ検索が担うもの。
...
`research` はこのスキルの上流に位置する。未決の選択肢を比較し、意図的に `決めるべきこと` で止まるため、その推奨は選択の根拠であって決定そのものではない。決定になるのは人間が承認した後であり、このスキルはその承認済みの結果を追跡可能な形にする。入力に未承認の推奨が残っているなら、その空白を隠さず、合意済みの計画ではなく 論点 として明示する。
