...
このスキルでは、**スキル起動直後に必ず `ask the user explicitly` で以下を確認する**。
...
- `.codex/settings.json` の `permissions.deny` に列挙された任意のパス
...
- [ ] 対象 canonical README のパスを `ask the user explicitly` でユーザーに確認済み
- [ ] スコープルートと深さ制限を確認済み
