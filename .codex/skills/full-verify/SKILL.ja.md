...
日本語で出力する。差分レビューではなく**全体検証**用途であることに注意（差分は `impl-review` / `/code-review`）。
...
- 差分・PR 単位のレビュー → `impl-review` / `/code-review`。
...
直したいときはこの 2 つ。差分単位なら `impl-review`（別モデル敵対）/ `/code-review`、
...
- **設計文書の有無**: `README*`, `docs/`, `ADR`, `AGENTS.md`/`AGENTS.md`, `INTENT.md`, 設計 md を検出。
