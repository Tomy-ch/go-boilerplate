...
3. For each in-scope layer, read the corresponding `.codex/agents/arch-auditor-<layer>.toml`, the layer README, and any applicable nearest package README. The agent definition gives the audit role and output contract; `AGENTS.md` and the READMEs remain the source of truth for repository rules.
...
5. Delegate independent layers to the matching `.codex/agents/arch-auditor-<layer>.toml` roles when delegation is available. Otherwise execute their instructions inline. Never run `make lint` more than once.
