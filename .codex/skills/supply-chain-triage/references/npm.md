...
State in the report which of these applies, because a score is evidence, not permission:
...
- **Under a pnpm `minimumReleaseAge`** (`pnpm-workspace.yaml`, stated in minutes), resolution fails
  ...
  resolver, which then writes the exclusion itself. The exemption must be committed to a tracked,
