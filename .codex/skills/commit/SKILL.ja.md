...
フックは自分で規模を決める。`.makefiles/load.mk` が開いている worktree の数から、重い Go ゲートを全速で走らせるか、絞るか、CI へ委譲するかを決定する。現在の帯は `make load-status` が報告し、`repo-ops` の §19 が方針を説明している。**この決定を迂回して `make lint` や `make test` を直接叩き、「本当に」検証しようとしないこと。** 窓が複数開いているとき、フルのローカル lint は、CI が同じものを走らせ直すだけの内容を再発見するために、飽和したホストを数分間占有する。帯が何をしたかを報告し、残りは push に運ばせること。
