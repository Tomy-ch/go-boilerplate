...
このスキルでは、**スキル起動直後に必ず `ask the user explicitly` で 2 つの値を順に確認する**。
...
3. `ask the user explicitly` を呼び出す:
...
`Write` を呼ぶ前に、作成内容をユーザーに提示し（インライン、または要約 + ファイルパス）、最後にもう一度 `ask the user explicitly` で確定する:
...
- [ ] `<FROM_TAG>` を `ask the user explicitly` で確認し、`origin` に存在することを検証した
- [ ] `<NEW_VERSION>` を `ask the user explicitly` で確認し、SemVer に対して検証した
...
- 出力ファイル本文の言語は `AGENTS.md` の言語ルールに従い**日本語**とする（対話中のやり取りの言語に関わらず）。
