...
`ask the user explicitly` を起動直後に必ず呼ぶ（`scaffold-endpoint` から呼ばれて context に feature 名がある場合は除く）:
...
完全 read-only。integrator と全 validator サブエージェントは spec / source ファイルを一切触らない。integrator が行うのは `ask the user explicitly`（feature 確認、単独時）と read-only validator の起動のみ。
...
- [ ] 対象 feature を `ask the user explicitly` で確認（または `scaffold-endpoint` から供与）
