...
Strictly read-only. The integrator and all validator subagents touch no spec or source files. The integrator only runs `ask the user explicitly` (feature confirmation, standalone) and spawns read-only validators.
...
- [ ] Target feature confirmed via `ask the user explicitly` (or supplied by `scaffold-endpoint`)
