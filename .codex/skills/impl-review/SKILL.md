...
- ✅ Apply comment quality findings in Step 7 after one confirmation (delete / rewrite-to-behavior), then `make fix` + `make lint`; skip with `--no-apply`.
...
- [ ] Unless `--no-apply`: comment findings applied in Step 7 (functional directives untouched, exported doc comments rewritten not deleted), then `make fix` + `make lint`; no auto-commit.
