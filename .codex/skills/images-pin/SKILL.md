...
Summarize in Japanese: images newly pinned / digest-refreshed (with the digest age), images kept on their prior aged pin (rule 2, with reason), any rule 3 hard failure (fresh + no prior lock — reported with its adoptable date, and whether the user chose to wait or bootstrap with `days=0`), and the verification result. List rule 2 held images so the user knows to re-run once they age. Do NOT commit, stage, or push — the user runs `/commit` (these changes are `CI:`- or `Build:`-prefixed) manually.
