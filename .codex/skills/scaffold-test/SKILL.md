...
2. **Never bundle multiple subjects into one `TestXxx` — strict 1:1, no exception.** Do NOT produce a unified `TestEntity_Accessors` / `*_Getters` covering multiple getters; each getter / accessor gets its own `TestXxx`. There is no `ask the user explicitly` bundling path and no rationale-comment exemption. The *only* exemption is a subject that genuinely should not carry its own real unit test (its branches are covered via a public caller / integration / DI-graph test): it STILL declares its convention-named `TestXxx` and calls `t.Skip("<covering test>")` — there is no allowlist; the exemption reason lives in the `t.Skip` string. This mirrors `docs/testing-conventions.md` §1, mechanically enforced by `internal/architest` (`TestUnitTestMappingCompleteness`).
...
- Any subject emitted as a skip-test (`TestXxx` + `t.Skip("<covering test>")`) because its branches are covered elsewhere, naming the covering test.
...
The strict 1:1 rule (no bundling; skip-test exemption via named `TestXxx` + `t.Skip`) holds in chained mode too — there is no bundling confirmation to obtain, since bundling is never allowed. (Table-driven tests likewise have no exception path: they are forbidden — always sequential `t.Run`.)
...
- ❌ Bundling multiple subjects into one `TestXxx` (strict 1:1, no exception; getters / accessors included). A subject covered elsewhere still gets its own named `TestXxx` + `t.Skip("<covering test>")` — never a bundle.
...
- [ ] Each produced `TestXxx` matches exactly one subject (strict 1:1, no bundling); a subject covered elsewhere is emitted as a named `TestXxx` + `t.Skip("<covering test>")`.
