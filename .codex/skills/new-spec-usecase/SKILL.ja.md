...
- `.codex/scaffold-spec/usecase-spec.md` — usecase 層の canonical 節リスト
...
`ask the user explicitly` 起動直後（`new-spec` 統合から context 提供時は除く）:
...
`.codex/scaffold-spec/usecase-spec.md` から canonical 節リスト:
...
`.codex/scaffold-spec/usecase-spec.md` の出力例を template として使用。
...
- [ ] usecase パッケージパス + interface 名を `ask the user explicitly` で確認
- [ ] `.codex/scaffold-spec/usecase-spec.md` を読んで現行節リスト取得
