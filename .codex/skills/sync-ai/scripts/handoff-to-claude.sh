#!/usr/bin/env sh
# Hand a sync-ai transfer contract to Claude's own `/manage-skill`, headlessly.
#
# The receiving side has to be Claude itself, because Codex never writes `.claude/`.
# Claude treats `.claude/**` as a sensitive-file class, which `acceptEdits` does
# not clear. `--permission-mode bypassPermissions` is therefore required below.
# This is a deliberate repository-owner decision: it is per invocation and leaves
# no standing permission widening. The child-operation preamble is consequently
# the restraint; do not "tidy" this flag away.
#
# Usage: handoff-to-claude.sh <contract-file>

set -eu

contract="${1:?usage: handoff-to-claude.sh <contract-file>}"
...
echo "handoff-to-claude.sh (pid $$)" > "$lock/started_by"
# ---------------------------------------------------------------------------

claude -p --permission-mode bypassPermissions <<EOF
You are a child operation of a sync-ai run started in Codex.

Carry out the transfer contract below using Claude's own \`manage-skill\` workflow.
...
- Write only under .claude/ and tmp/, and only for the single skill named in the contract.
- Do not commit, push, or delete the source skill.
- Report which items you ported, adapted, or omitted, and any intent Claude cannot express.
