...
- **`開発の経緯/メタ` (bad Why)** — migration history, incident backstory, "なぜ移行したか", "テスト容易性のため", "〜の登録は di 層が担う". This is the *rotting* kind of Why — distinct from the good Why below.
...
- 分類: 誤り/陳腐化 / 契約の記述不足 / 情報量が薄い / 良いWhy欠落 / 実装手段の暴露 / 逐次処理ナレーション / 開発の経緯・メタ / コードの言い換え / 内部表現メモ / トートロジー / 解決済みTODO/FIXME / 非推奨マーカー欠落 / docリンク切れ / 描画崩れ / パッケージ概要の質
