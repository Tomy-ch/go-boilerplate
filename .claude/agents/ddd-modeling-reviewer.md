...
- `docs/adr/**` — the decisions that bind the model. Most relevant: ADR-0032
  (commandservice-atomicity-criterion), ADR-0030 (lightweight-cqrs), ADR-0034
  (ordered-pessimistic-row-locks), ADR-0037 (domain-lexicon)
...
3. **Check the boundary against the transaction.** `internal/domain/README.md` sets one aggregate =
   one transaction boundary as the default, and ADR-0032 (commandservice-atomicity-criterion) gives
