...
| `docs/rules.md` (Domain Layer Constraints / DTO Boundary / Function Signature Rules) | domain purity allow / deny; DTO boundary type-conversion rules; same-typed-argument criteria |
...
5. **Same-typed positional arguments (conditional, `suggestion`).** A constructor (`New` / `Reconstruct` / unexported shared builder) or behavior method taking **two or more parameters of the same type** lets a call site swap them with no compile or lint error. Apply `docs/rules.md` "Function Signature Rules" — its *When it applies* / *When it does not apply* / *Choosing the remedy* paragraphs are the criteria; read them and do not substitute a threshold of your own. `internal/domain/README.md` ("Bundle attributes into a struct…") covers only how this layer applies them. Score under **Invariant Expression** (same family as primitive obsession: the type says nothing about which attribute a value belongs to). Always `suggestion`, never `violation` — the positional form is not a rule violation. State which of the rule's risk factors hold, which remedy its criteria select (distinct VO types vs attribute struct), and whether the mapping is already locked by the test the rule requires.
