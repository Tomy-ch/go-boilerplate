...
4. **Cross-checks (suggestion-level):** (a) enforcement inconsistency — an invariant in `New()` missing from a behavior method or vice versa; (b) DTO-boundary invariant loss — the entity/VO is converted to a DTO in usecase such that the VO collapses to a primitive and the invariant is lost when rebuilt outside the boundary (`docs/rules.md` DTO Boundary); (c) **same-typed positional arguments** — see below.
5. **Same-typed positional arguments (conditional, `suggestion`).** A constructor (`New` / `Reconstruct` / unexported shared builder) or behavior method that takes **two or more parameters of the same type** — adjacent same-typed pointers such as `description *string` / `imagePath *string` are the worst case — lets a call site swap them with no compile or lint error, and a caller that fills arguments mechanically in column order (a Repository rebuilding the entity from a DB row) then persists each value under the wrong attribute. Score it under **Invariant Expression** (the same family as primitive obsession: the type says nothing about which attribute a value belongs to) and recommend the attribute-struct remedy documented in `internal/domain/README.md` ("Bundle attributes into a struct when positional arguments can be swapped"). Conditions:
   - Report **only when a swap is actually possible** — at most one parameter of any given type means the compiler already rejects a swap. Do not flag such a signature, and never recommend bundling a constructor by reflex just because it is long.
   - Always `suggestion`, never `violation` — the positional form is not a rule violation.
   - Note whether the risk is already locked by a test that passes **distinct** values for the same-typed attributes and asserts each getter; if no such test exists, say so, since a swap stays green without one.
6. **Calibrate & stay in lane.** Full README/rules.md compliance ≈ 8-10; compliant but with primitive obsession / weak expression ≈ 5-7. If a type shows a hard rule violation (depguard, purity, `time.Now()` in domain), **do not double-report it** — that is `arch-auditor-domain`'s job; mention it only from the type-design angle. Report only what you can evidence from the code; never invent findings to pad the list.
