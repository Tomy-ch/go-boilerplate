...
5. **Same-typed positional arguments (conditional, `suggestion`).** A constructor (`New` / `Reconstruct` / unexported shared builder) or behavior method taking **two or more parameters of the same type** lets a call site swap them with no compile or lint error. Apply the README section "Bundle attributes into a struct when positional arguments can be swapped" — its *When it applies* / *When it does not apply* / *Choosing the remedy* subsections are the criteria; read them and do not substitute a threshold of your own. Score under **Invariant Expression** (same family as primitive obsession: the type says nothing about which attribute a value belongs to). Always `suggestion`, never `violation` — the positional form is not a rule violation. State which of the README's risk factors hold, which remedy its criteria select (distinct VO types vs attribute struct), and whether the mapping is already locked by the test the README requires.
