...
- `make pin-images-apply` — lockfile を元に各 `FROM` を `image:tag@<digest>` へ固定する。**fail-closed**：lockfile 非登録の image は tag のみへ正規化*しない*（digest を剥がさない）——`未登録` エラーとして報告し非ゼロ終了する。lockfile が唯一の正で、`FROM` は lockfile に登録済みの image しか参照できない。
- `make pin-images-check` — 書き換えずに、全 `FROM` が digest 固定済みかつ lockfile 登録済みかを検証する（CI / hook）。network 不要（lockfile と Dockerfile のみ読む）。fail-closed：`未固定`（tag のみ・digest 無し）・`未登録`（digest はあるが lockfile 未登録）・drift（digest ≠ lockfile）で非ゼロ終了。CI gate は `.github/workflows/pin-images-check.yaml`。
...
lockfile を元に全 `FROM` を固定する（古い image は `@sha256:...` を付与/更新）。fail-closed：lockfile 未登録の image は tag のみへ剥がさず、`未登録` として報告し非ゼロ終了する。これが出たら、`resolve` を通さずに `FROM` へ image が到達した状態——resolve を再実行する（または不要な `FROM` を除く）。
...
- [ ] `make pin-images-apply` 実行（`未登録` エラー無し; 全 `FROM` が lockfile 登録済み）
- [ ] `make pin-images-check` + `make docker-lint` 実行・報告
- [ ] `FROM` の **tag** は不変（digest のみ）; 全 `FROM` が digest 固定済み（tag のみ無し）かつ lockfile 登録済み
