...
- **範囲は網羅されているか。** 「X だけがこうしている」という主張なら、呼び出し箇所を推測せずグラフを逆走査する（`node .claude/scripts/graph-affected.ts <symbol> --depth 2`）。各箇所を関係ラベルと `file:line` 付きで返すので、不在の主張が要求する証拠そのものになる。先にグラフの鮮度を確認し（`AGENTS.md`）、遅れているかシンボルが新しい場合は全呼出元と全兄弟の網羅的探索へフォールバックする。範囲の主張は不在の主張であり、不在こそ検索が担うもの。
...
| 4 | 影響範囲の主張が部分的な検索に依存している | グラフを逆走査する、網羅的に検索する、または覆った範囲まで主張を狭める |
...
このスキルの上流には `research` がいる。あれは未決の選択肢を比較して `決めるべきこと` で止まる。その推奨は決定ではない —— 決定に変えるのは人間の承認であり、承認された結果を追跡可能にするのがこのスキル。まだ誰も承認していない推奨を抱えた issue が来たら、それは閉じるべき穴ではなく名指しすべき穴である。決まった計画としてではなく 論点 として起票する。
