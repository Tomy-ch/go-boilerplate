...
他リポジトリの issue / PR へリンクするときは `redirect.github.com` を使う —— 素の `github.com` リンクは相手のスレッドに公開の逆参照を投稿する。その信号を意図して送るかは例外なく人間の判断: 毎回聞くこと、自分で決めないこと。`CLAUDE.md` 参照。
...
- [ ] `AskUserQuestion` 1 回で検証の深さと出力モードを確認した。
