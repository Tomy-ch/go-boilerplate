...
蓄積したコメントを、既存のレビュアが問えない 2 つの問いで判定する: **その事実を既に守っている
ものが在るか？** と **この内容はここに属するのか？**

## 使うとき

- あるパッケージのコメントが、どの行も間違っていないのに冗長・論文的に読める
- doc コメントが設計論（却下した選択肢、脅威モデル、アーキ方針）に育っている
- テストケース・lint ルール・アーキテクチャ走査・spec が既に強制していることを、コメントが言い直している
...
### 1 つ目の問い: それを既に守っているものが在るか

管轄より先に問う。これだけでコメントが片付くことがあるためである:

> **この文が偽になったら、何かが落ちるか？**

落ちるなら —— コンパイルエラー、テストケース、lint ルール、`internal/architest` の走査、再生成チェック
—— その機構こそが事実を真に保っているのであり、コメントはその二つ目の写しである。両者は対等ではない:
機構は現実が動くたびに必ず更新される（緑にならない限り先へ進めない）が、コメントは誰かが思い出したとき
にしか更新されない。だから壊れるのは写しの側で、しかも何も赤くならずに壊れる。判定は **不要**。

これを安全にする歯止めが 2 つあり、どちらを落としてもこのスキルが犯しうる最悪の編集 —— 他のどこも公表して
いない契約の削除 —— になる:

- **守っている機構を `file:line` で名指すこと。** 「読んで不要に見えた」は根拠ではない。それは写しを書かせた
  のと同じ読み方である。名指せる守り手が無い文は**守られていない**のであり、それは残す理由になる。
- **どちらの写しが消えるかは `docs/rules.md` が宣言しており、実行ごとに決めてよいものではない。** doc コメント
  は呼び出し側に向けた契約を*公表*しているので、テストや spec がそれを言い直していてもコメントが写しに
  なるわけではない。呼び出し地点から観測できないもの —— なぜ業務がそうなっているか —— だけが、コメント側
  が手放す写しである。
...
| 問い | 真実の源 |
| --- | --- |
| コメントに何を書いてよいか / 2 つの問い / 管轄条項 | `docs/rules.md` の **Comment Rules** |
| 2 つの写しのどちらが消えるか — 宣言された正本の向き | `docs/rules.md` の **Comment Rules**（契約は doc コメントが公表し、その理由は `docs/spec/**` が持つ） |
...
| 自動適用 | `--apply`、または 2 番目の選択肢 | 短縮 / 削除 / 裏取り済みの不要 / 確度 high の集約を 1 件ずつの確認なしで書き込む。文書書き込みを伴う移設は報告するだけで適用しない |
...
短縮 / 削除 / 不要 / 集約だけなら `--apply` で、移設も含めるなら確認して適用で再実行する。集約については、本体を
...
**短縮**・**削除**・**不要**・**集約**を監査エージェントの着地形のまま一括で適用し、適用した内容を報告する。
まず、その集合から次の 4 つを外す:
...
- **不要は、integrator が名指された `file:line` を開いてその事実が実在することを確認できたときだけ適用する。**
  これは下の `追記なし` 移設と同じ検査であり、理由も同じである。テスト名や spec の節を読み違えた監査
  エージェントは、契約が公表されている唯一の場所を削ってしまい、無人ではそれを捕まえる者が居ない。名指
  された出所がその事実を持っていない `不要` は適用せず報告へ回し、出所を名指していないものはそもそも
  finding ではない。
...
2. 監査エージェントが挙げた選択肢で `AskUserQuestion`（通常は 移設 / 短縮 / 削除 / 不要 / 維持 / 判断を保留）。
   **不要** については、落とすコメントだけでなく名指された `file:line` とその内容も示す — 判断すべきは
   その出所が本当にその事実を持っているかであり、コメント側だけを見て下せる判断ではない。
...
- 触れたパッケージに `make go-fix` → `make go-lint`。規約上必要な doc コメントを消していれば
...
本スキルは自分自身として依頼されるものであり、他のレビュースキルの内側から呼ばれることはない。そして**レビューの対象の 1 つでもない**: `AGENTS.md` はこれを実装の最後へ無条件に置いている。判定は検出の文脈でしか効かないためである —— まだコードを生成している実行は散文をタダで書くので、それが値したかを自分で評価できない。`/impl-review` は変更そのものを、`/test-review` はテストを監査し、その 2 つが個別に依頼される対等物であり、3 つのいずれも他を委譲しない。次のスキルの実行を提案するスキルは、主題を独立に答えられないものにし、1 つのスキルのずれが、そこを通った全ての流れから残りを黙って落とす。
...
| **`settle-comments`**（本スキル） | **コメント 1 件、パッケージの在庫全体、そしてパッケージ横断の繰り返し** | **維持 / 短縮 / 削除 / 不要 / 移設 / 集約** | **管轄 — 内容がどこに属し、どの 1 箇所が所有し、既に守っている機構が在るか** |
