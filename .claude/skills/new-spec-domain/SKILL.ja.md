...
これは**導入する側のドメイン層が在るあいだ成り立つ**。投影だけの feature——集約も `domain.md` も無く、
あるのは QueryService だけ——にはその層が無く、その語（売上・ランキング・郵便番号からの住所引き）は
放っておけば誰にも導入されない。その場合は `/glossary` が読み取り側から拾う。このスキルの担当では
ないのは、このスキルがまさに集約を作るときに走るものだからである。
