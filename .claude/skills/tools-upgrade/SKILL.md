...
- `python/*.in` — the PyPI tools, whose resolved trees are hash-pinned in `python/*.txt` ([ADR-0078 (mise-ssot-drift-gate)](../../../docs/adr/0078-mise-ssot-drift-gate.md)). Bumping one of these is a two-file change: the pin, then `make py-lock`.
...
applying the bump involves, and the user is about to approve that in step 4.
