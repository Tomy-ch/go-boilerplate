...
- `python/*.in` — PyPI のツール。解決結果は `python/*.txt` にハッシュ付きで固定される（[ADR-0075 (bridge-instrumentation-exceptions)](../../../docs/adr/0075-mise-ssot-drift-gate.ja.md)）。こちらの bump は 2 ファイルの変更になる（pin を書き換えてから `make py-lock`）。
