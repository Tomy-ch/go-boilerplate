...
- テストの監査（`/test-review`）・コメントの監査（`/settle-comments`）— 下位ステップではなく対等な別スキル
...
**本スキルが監査するのは変更そのものだけである。** テスト lens もコメント lens も持たず、他のスキルを起動もしない。テストは `/test-review` の主題であり、`AGENTS.md` の Review Phase Protocol のとおり本スキルの横で個別に依頼され個別に走る。コメント蓄積は `/settle-comments` の主題だが、それはレビューではなく実装の最後の手順なので、このレビューが依頼された時点で既に走り終えている。次のスキルの実行を提案するレビュースキルは、主題を独立に答えられないものにし、1 つのスキルの質問のずれが、そこを通った全ての流れから残りを黙って落とす。
...
`/settle-comments` の主題で、ユーザーが個別に依頼する。畳み込めば、ある主題についての判断が別の主題のために
...
コメントの内容についての指摘は `/settle-comments` の主題である。lens がついでに気づいた場合は、補足節に
...
投稿するのは本スキル自身の指摘だけである。`/test-review` と `/settle-comments` はそれぞれ自分の出力を出し、ここからそれらへ手を伸ばすことはない — 他スキルの指摘を本スキルのレビューとして投稿すれば、ある主題の監査が別の主題の中で行われたように見えてしまう。
...
- ❌ テストやコメントを監査する lens を生やす / ここから `/test-review` や `/settle-comments` を起動する。Review Phase Protocol の下では対等な別スキルであり、気づいた点は補足節に所管スキル名を添えて書く。
