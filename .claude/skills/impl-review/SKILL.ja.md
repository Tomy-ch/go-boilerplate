...
- reviewer は **read-only**（エージェント定義に Edit/Write 権限なし）— finding を返すだけ。ソースを書き換えるのはちょうど 2 箇所で、いずれも subagent ではなく **オーケストレーター**が行い、いずれも明示的な確認を経る: Step 5.5 が verify 済みのコメント指摘を適用し、Step 4.6 が委譲した `/comment-sweep` の中でユーザーが 1 件ずつ承認したものを書き込む。コード 5 lens が自動修正されることはない。
...
3. **階層 1〜2 の指摘が書き換えそうなファイルは、コメントに触れる 2 ステップの双方から保留する。**
   ...
   一覧は Step 3 の後に 1 度だけ解決する — Step 4.6 の `hold` payload と Step 5.5 の除外の両方がこれを
...
| `/comment-sweep`（Step 4.6） | 触れたファイルが**抱えている**在庫 — その管轄 |
...
Step 1 の掃引スコープが空でなく **かつ** Step 0 でユーザーが委譲を選んだときに実行する。ここに置くのは、尊重すべき tier 1–2 の指摘が Step 3 で既に確定していること、そしてこのステップが開きうる対話的な承認ループは、read-only な Step 4.5 の委譲と交錯させずその後に置くべきであるため。Step 5 より前に置くのは、その結果を後付けではなくレポートに載せるため。
...
- `hold`: 評価順の規則 3 により保留するファイル（生き残った tier 1–2 の指摘が書き換えそうなもの）。Step 5.5 が除外するのと同じ一覧である — 1 度だけ解決して両方へ渡すこと。別々に決めると保留するファイルがずれ、レポートが自己矛盾する。
...
**階層順に並べ、階層内で重大度順**、CONFIRMED を PLAUSIBLE より先に（評価順の規則 1）。上位の決着を待っている指摘は `保留` と明記し、何を待っているかを書く（規則 2）。ランタイムで何を検査し何をスキップしたかは必ず明記（黙って省くと「全部見た」と誤読される）。委譲したテスト指摘も同様に、独自の severity 語彙のまま専用節に置く。Step 4.5 を実行しなかったときはその節ごと省く（`テスト観点:` 行が既にその事実を伝えている）。掃引の節も同じで、判定は `/comment-sweep` の語彙（維持 / 短縮 / 削除 / 移設）のまま置き、Step 4.6 を実行しなかったときは節ごと省く。

## Step 5.5 — コメント指摘の適用（既定。`--no-apply` でスキップ）

本スキルが**自らの判断で**ソースを書き換えるのはここだけ。verify を通った**コメント品質**の指摘（CONFIRMED、およびユーザーがオプトインした PLAUSIBLE）は自分で適用する — `comment-reviewer` サブエージェントは決して編集しない。コード 5 lens はここでは自動修正せず、Step 6 へ回す。

掃引の finding をここで再適用しない。`confirm` モードなら Step 4.6 がユーザーの承認済みのものを既に書いており、`report` モードなら意図的に手を付けていない。このステップの対象は tier 5 の `comment-reviewer` の指摘だけである。

編集前に 1 度だけ確認する:

**先に評価順の規則 3 を適用する。** Step 3 の後に解決した保留一覧（Step 4.6 へ `hold` として渡したもの）を
...
**掃引の finding（Step 4.6）は投稿しない。**在庫は本変更が持ち込んだものではないので、diff 外のテスト指摘を留め置くのと同じ理由が、変更行に載っている分も含めて全体に及ぶ。件数はローカルレポートに書き、省略を見えるようにする。
...
- ✅ Step 0 でコメント在庫の委譲を聞き（既定: 委譲する（確認して適用））、委譲したら `scope` / `mode` / `base_ref` / `hold` を渡して Step 4.6 を実行。
...
- [ ] 委譲したときは Step 4.6 を実行（`scope` / `mode`（`apply` は渡さない）/ `base_ref` / `hold` / `claimed` を渡し、`hold` は Step 5.5 と同一の一覧）。
