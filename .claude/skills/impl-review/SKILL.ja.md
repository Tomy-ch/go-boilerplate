...
**階層順に並べ、階層内で重大度順**、CONFIRMED を PLAUSIBLE より先に（評価順の規則 1）。上位の決着を待っている指摘は `保留` と明記し、何を待っているかを書く（規則 2）。ランタイムで何を検査し何をスキップしたかは必ず明記（黙って省くと「全部見た」と誤読される）。委譲したテスト指摘も同様に、独自の severity 語彙のまま専用節に置く。Step 5 を実行しなかったときはその節ごと省く（`テスト観点:` 行が既にその事実を伝えている）。掃引の節も同じで、判定は `/comment-sweep` の語彙（維持 / 短縮 / 削除 / 移設）のまま置き、Step 6 を実行しなかったときは節ごと省く。
