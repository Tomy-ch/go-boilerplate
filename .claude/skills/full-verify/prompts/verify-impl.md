...
1. **実装の綺麗さ / 保守性（主観点）**:
   ...
   - 冗長・自明なコメント。関数に必要なのは振る舞い/契約（何をするか・前提・副作用）の説明であって、
     内部手続きの逐語的な実況や、コードを読めば自明な WHAT の繰り返しは不要。残すべきは非自明な WHY のみ。
...
table-driven の適切さ、ケース名の説明性、`t.Parallel()` の有無、前提と検証の分離
