...
問題点を指摘した Markdown 群を `tmp/skills/reviews/` 配下に生成する read-only スキル。

任意のリポジトリに対し、言語・構造・設計文書の有無を**スキル自身が検出して適応**する。
このリポジトリ専用ではなく、編集なしに別リポジトリへコピーして起動できることを目標にしている。

- **コードを一切変更しない。** 削除・権限変更・外部送信もしない。読み取りと `tmp/skills/reviews/` への md 生成のみ。
...
- **構造表現生成** → `tmp/skills/reviews/_structure/`: tree / 公開シグネチャ（best-effort grep）/ 依存グラフ /
...
- **Pass 2 実装検証** → `tmp/skills/reviews/mod_<id>.md`（モジュール単位。`architecture.md` を前提文脈に渡す）。
- **Pass 3 集約** → `tmp/skills/reviews/_index.md`（設計起因 / 局所実装を分離・重大度別）。**全モジュール完了後のみ。**
...
echo "pid=$!  進捗: tail -f tmp/skills/reviews/run.log"
```
...
| `--out <dir>` | `tmp/skills/reviews` | 出力先ディレクトリ上書き。別クラスのレビューを分離（例 `tmp/skills/reviews-config`） |
...
> 出力先 `tmp/skills/reviews/` は `tmp/` 配下＝既定で `.gitignore` 済み（バージョン管理対象外）。
> `tmp/` の外を指す `--out` を使う場合のみ、別途 ignore すること。

## 冪等性・再開

- 状態は **`tmp/skills/reviews/mod_<id>.md` の有無/中身だけ**で表現する（`_progress.md` はそこから都度導出する
