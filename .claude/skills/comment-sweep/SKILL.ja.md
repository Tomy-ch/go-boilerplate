...
**連鎖された実行もファイル単位のままである。** 呼び手の関心は diff だが本スキルの関心はそうではない。
payload が運ぶのがまさにその理由で**ファイル全体**であり、両方のパスはそのまま走る。ここから 2 つのことが
従い、それが「委譲が静かに 2 度目の diff レビューへ退化する」のを防いでいる:

- 監査エージェントは常にファイルを**丸ごと**受け取る。読ませる範囲を変更箇所や未 claim のコメントへ
  狭めないこと — 断片で渡されたファイルは総体として判定できず、Pass 2 が探している重複はまさにその
  断片の隙間に住んでいる。
- `hold` と `claimed` は **finding に対する integrator 側のフィルタ**であって、監査エージェントの入力に
  掛けるものではない。監査エージェントは全部読んで全部報告し、そのあとで integrator が呼び手の所管分を
  落とす。どちらかのフィルタを上流へ押し上げれば、内側から diff スコープが復活する — 呼び手がここへ
  委譲したのは、まさにそれを避けるためである。
...
**集約は複数の `path:line` にまたがるため、両方のフィルタに「1 つの単位として扱う」規則が要る**:

- `hold` — 集約は 1 つのファイルの中にあるので、そのファイルが保留されていれば finding ごと落とす。
  これから書き換わるコードへコメントを集約するのは、この payload が避けようとしている二度手間そのものである。
- `claimed` — メンバーの**いずれか**が未 claim なら finding を残し、claim 済みのメンバーをレポートに
  明記して呼び手が重なりを見えるようにする。claim 済みのメンバーだけを個別に落とせば、集約を成立させて
  いる site が欠けた集約が残る。1 件 claim されただけで finding ごと落とせば、呼び手側の per-comment
  レビュアが構造上生成できない判定を失う。全メンバーが claim 済みのときは落とす。

payload 下では、レポートは単体の成果物として描画するのではなく、呼び手が埋め込む前提で返す。判定語彙は
本スキルのもの（維持 / 短縮 / 削除 / 移設 / 集約）のままにする — 呼び手はこれを読み替えない。

Step 2 の監査エージェントは **Step 0 のあらゆる選択から独立**である。常に両方のパスを走らせて全 finding を
全文で生成し、モードの差はすべて、間引く側であるこの integrator にある。モードも判定粒度も
`references/audit-prompt.md` へ押し下げないこと。2 体の監査エージェントが「何を finding とみなすか」で
静かに食い違うのは検出手段の無いドリフトであり、そもそも節約にもならない — ファイル単位のパスは何も
読み直さないので、切っても作業は戻らず、レポートが短くなるだけである。

## 既存レビュアとの関係

| | 判定の単位 | 判定 | 管轄 |
| --- | --- | --- | --- |
| `comment-reviewer`（`impl-review` 経由） | diff 上のコメント 1 件 | 削除 / 書換 / 加筆 | 生成時ゲート。新たなノイズの流入を止める |
