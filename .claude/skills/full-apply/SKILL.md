...
allowed-tools: Read, Grep, Glob, Edit, Write, AskUserQuestion, Bash(grep:*), Bash(git rev-parse:*), Bash(gofmt:*), Bash(go build:*), Bash(go vet:*), Bash(go test:*), Bash(make fix:*), Bash(make lint:*), Bash(make test:*), Bash(mise install:*), Bash(mise exec:*)
---
...
2. **Check go toolchain connectivity** (this repository is Go / mise-managed). `make fix` / `make
   lint` / `make test` are the canonical paths. If `go`/`make` fails due to a broken GOROOT or version
