...
- `make md-skill-lint` が通る。これは前項の存在部分を強制する検査で、片側の環境にしか無いスキルは、`scripts/skill-lint.mjs` の `PLATFORM_ONLY_SKILLS` へ理由付きで登録されていない限り落ちる。意図的にプラットフォーム専用とするスキルはそこへ登録し、移植したら登録を削除する。
