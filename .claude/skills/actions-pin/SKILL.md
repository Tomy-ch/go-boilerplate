...
- **The age `resolve` gates on is the newer of the release `published_at` and the resolved commit's date** (why, in `docs/design/security.md`, Build inputs). Consequence for this skill: **an old `published_at` no longer implies the candidate is aged.** As of 2026-07-31, `anthropics/claude-code-action@v1` is such a case — its `v1` release was published 2025-08-26 while the tag's head commit is 2026-07-25, so `resolve` quarantines it despite the release being nearly a year old.
