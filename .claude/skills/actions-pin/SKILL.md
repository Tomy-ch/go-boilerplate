...
- **The age `resolve` gates on is the newer of the release `published_at` and the resolved commit's date.** A release object is bound to the tag *name*, so a re-pointed moving tag keeps an old publication date while pointing at brand-new code; taking the newer of the two closes that hole. Consequence for this skill: **an old `published_at` no longer implies the candidate is aged.** `anthropics/claude-code-action@v1` is the live example — its `v1` release was published 2025-08-26 while the tag's head commit is 2026-07-25, so `resolve` quarantines it despite the release being nearly a year old.
...
- The scan covers `.github/workflows/*.{yml,yaml}` plus `.github/actions/**/action.{yml,yaml}` **recursively**, so a composite action placed one directory deeper than the usual layout is pinned like any other. A one-level glob dropped such a file with no warning.
...
| --- | --- | --- |
| `lockfile に解釈できない行があります` (with a line number) | A line in `.github/actions-pin.toml` is neither blank, a comment, nor a `"key" = "<40-hex>"` assignment. It used to be skipped silently, which made the entry look unregistered. | Run `make pin-actions-resolve`, or delete the reported line. |
| `lockfile にキーの重複があります` | The same `owner/repo@tag` is assigned twice. Which SHA wins used to depend on line order. Most often the residue of resolving a merge conflict in the lockfile mechanically. | Run `make pin-actions-resolve`, or delete the duplicate line. |
...
| `固定対象として解釈できない記法の uses: があります` | A `uses:` is written in YAML flow mapping (`- {name: Checkout, uses: actions/checkout@v4}`). The pinner rewrites only block notation, so this reference would silently never be pinned. | Rewrite the step in block notation (`- uses: ...` on its own line). Do not suppress the check. |

The last one is why "nothing was flagged" is no longer the same as "there is nothing to flag": before this gate existed, an unpinnable notation produced zero matches and `check` reported success.
...
- **A release date is not the age of the code it names.** The quarantine takes the newer of `published_at` and the resolved commit date precisely because a re-pointed tag keeps its old release date. This buys time against automated takeover; it is not proof the date is honest. Catching the re-point itself is still step 6's job.
