...
本スキルは自分自身として依頼されるものであり、他のレビュースキルの内側から呼ばれることはない。`/impl-review` は変更そのものを監査する。2 つは `AGENTS.md` の Review Phase Protocol の下で対等であり、それぞれ個別に依頼され、どちらも他を委譲しない。コメント在庫は第 3 の対等物ではない —— `/settle-comments` は実装の最後の手順として無条件に走るので、レビューが依頼される時点では既に済んでいる。次のスキルの実行を提案するレビュースキルは、主題を独立に答えられないものにし、1 つのスキルのずれが、そこを通った全ての流れから残りを黙って落とす。
