...
まず `AskUserQuestion` を 1 回の batched call で 2 質問:
...
2 回目の batched `AskUserQuestion`。選択した layer ごとにフリーテキスト 1 問。skip 判定が必要とするのは
...
2. chain context として feature 名**とその layer のパッケージパス**を渡し、child のパス `AskUserQuestion` をスキップ可能にする — layer 固有 identity（aggregate 名 / interface 名）のみ child が質問
...
- [ ] feature 名 + layer 選択 + layer ごとのパッケージパスを `AskUserQuestion` で確認
