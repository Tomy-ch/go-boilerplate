...
Generate a Go unit test file for an existing function or method, applying the canonical pattern abstractly extracted from `internal/domain/user/user_domain_test.go` (parallel + nested `t.Run` + Japanese case names + 正常系 / 異常系 outer grouping, no table-driven for-loops by default).
...
2. **Multiple subjects in one `TestXxx` are the exception.** If the test-perspective subagent (or the user) proposes bundling multiple functions / methods into one main case (e.g. a unified `TestEntity_Accessors` covering every getter), invoke `AskUserQuestion`:
...
- Any approved exception (bundled multi-subject `TestXxx` or table-driven block) with its recorded rationale.
...
`AskUserQuestion` invocations for **the multi-subject bundling exception** and **the table-driven exception** remain mandatory even in chained mode — they encode rules the parent does not know about.
