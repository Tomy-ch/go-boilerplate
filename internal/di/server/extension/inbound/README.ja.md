...
- **Timeout は Pre ミドルウェア（priority 2）** — `uri` の外側に置き、per-request の deadline budget が全 `Use`・OpenAPI 検証・ハンドラ・DB を覆うようにする。deadline は `ctx` で伝播し、下流（pgx, `httpclient`）が尊重する。これが deadline budget（M1）の入口で、各境界に独立 timeout を置く代わりに単一予算へ集約する
