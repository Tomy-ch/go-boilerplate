...
- クリーンアップ処理（`stop`）は、成功・失敗・タイムアウトに関わらず常に呼び出されます。その失敗（OTel flush / DB pool close 等）はジョブ結果と **Join**（`xerrors.Join`）され呼出側へ伝播するため、ジョブ自体が成功でも後始末が失敗すれば非ゼロ exit code になります。
