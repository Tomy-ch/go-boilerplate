...
func TestOS_Output(t *testing.T) {
	...
	t.Run("正常系_標準出力を返す", func(t *testing.T) {
		t.Parallel()
		out, err := cliexec.OS{}.Output(context.Background(), "", "echo", []string{"hello"})
	...
	t.Run("異常系_存在しないコマンドはエラー", func(t *testing.T) {
		t.Parallel()
		_, err := cliexec.OS{}.Output(context.Background(), "", "definitely-not-a-real-command-xyz", nil)
		require.Error(t, err)
	})

	t.Run("異常系_非ゼロ終了はエラー", func(t *testing.T) {
		t.Parallel()
		_, err := cliexec.OS{}.Output(context.Background(), "", "false", nil)
		require.Error(t, err)
	})
}
