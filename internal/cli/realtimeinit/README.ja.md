...
`ENDPOINT_REALTIME_PUBSUB` 上に `REALTIME_TOPIC_ARN` が名指す fan-out topic を作ります。冪等な one-shot で、
...
|`TopicName(arn)`|topic 名、すなわち `REALTIME_TOPIC_ARN` を `:` で区切った最後の要素。無ければ `ErrTopicARNInvalid`|
...
`dynamodbclient.EnsureTable` を渡し、topic 名を `infrastructure/realtime` の SNS クライアントの `CreateTopic` に
