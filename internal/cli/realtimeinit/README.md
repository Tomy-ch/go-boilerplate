...
`ENDPOINT_REALTIME` point at, then the fan-out topic that `REALTIME_TOPIC` names on the SNS-compatible
...
|`TopicName(arn)`|The topic name, i.e. the last `:`-separated element of `REALTIME_TOPIC`; `ErrTopicARNInvalid` when there is none|
...
`dynamodbclient.EnsureTable` to a real client, and binds the topic name to `realtime.EnsureTopic`
