...
// PgxAdmin は実 Postgres への薄い pgx アダプタで、分岐は接続失敗/存在チェックの委譲のみ。純粋な単体
// テストでは実 DB を要するため、これらは Pool レベルの統合フロー（scaffold-integration-test / runtime
// 検証）でカバーする方針とし、ここでは 1:1 規約に従い Skip を明示する。
