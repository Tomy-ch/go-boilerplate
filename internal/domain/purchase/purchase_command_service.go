...
type CommandService interface {
	...
	// （不存在の検証は呼び出し側の責務です）。
	LockProducts(ctx context.Context, productIDs []uuid.UUID) ([]LockedProduct, error)
	...
	LockPurchase(ctx context.Context, id uuid.UUID) (*Purchase, error)
	// CancelPurchase は、キャンセルに伴う在庫復元（明細分の加算）と購入の状態遷移（→ キャンセル）を、
	// 渡された ctx のトランザクション内で原子的に実行します。在庫加算は相対更新で売り越しを生みません。
	CancelPurchase(ctx context.Context, p *Purchase) error
}
