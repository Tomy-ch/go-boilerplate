// Package status は、購入ステータスマスタドメインを定義します。ID・名称・コード・表示順（sortKey）の不変条件を持つ Status エンティティと Repository インターフェースを提供します。
package status
...
// Status は、購入ステータスを表すドメインエンティティです。
type Status struct {
	id      uuid.UUID
	name    string
	code    int
...
// Attributes は、購入ステータスの属性一式です。Code と SortKey が同型のため構造体で受けます
...
// New は、購入ステータスエンティティの検証と生成を行います。Code・SortKey は 1〜32767 の整数である
