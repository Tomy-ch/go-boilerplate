...
func TestNew(t *testing.T) {
	...
	expectedName := "未処理"
	// Code と SortKey は同型かつ同じ有効範囲のため、同じ値を与えると取り違えても検証を通過する。
	expectedCode := 1
	expectedSortKey := 5
	...
	t.Run("正常系", func(t *testing.T) {
		...
		t.Run("名前が最小文字数ちょうど(1)の場合、生成される", func(t *testing.T) {
			t.Parallel()
			name := strings.Repeat("字", minNameLength)
			actual, err := New(expectedUUID, Attributes{Name: name, Code: expectedCode, SortKey: expectedSortKey})
			require.NoError(t, err)
			assert.Equal(t, name, actual.name)
		})

		t.Run("名前が最大文字数ちょうど(100)の場合、生成される", func(t *testing.T) {
			t.Parallel()
			name := strings.Repeat("字", maxNameLength)
			actual, err := New(expectedUUID, Attributes{Name: name, Code: expectedCode, SortKey: expectedSortKey})
			require.NoError(t, err)
			assert.Equal(t, name, actual.name)
		})
	...
	t.Run("異常系", func(t *testing.T) {
		...
		t.Run("IDがゼロ値の場合、ErrInvalidIDエラーが返される", func(t *testing.T) {
			t.Parallel()
		...
		t.Run("無効な購入ステータス名の場合、ErrInvalidNameエラーが返される", func(t *testing.T) {
			t.Parallel()
...
func TestStatus_ID(t *testing.T) {
	...
	t.Run("正常系", func(t *testing.T) {
		...
		t.Run("IDメソッドは正しいIDを返す", func(t *testing.T) {
			...
			status, err := New(id, Attributes{Name: "キャンセル", Code: 6, SortKey: 2})
			require.NoError(t, err)
...
func TestStatus_Name(t *testing.T) {
	...
	t.Run("正常系", func(t *testing.T) {
		...
		t.Run("Nameメソッドは正しい名前を返す", func(t *testing.T) {
			...
			assert.Equal(t, "キャンセル", status.Name())
		})
	})
}
...
func TestStatus_SortKey(t *testing.T) {
	...
	t.Run("正常系", func(t *testing.T) {
		...
		t.Run("SortKeyメソッドは正しい表示順を返す", func(t *testing.T) {
			...
			assert.Equal(t, 2, status.SortKey())
		})
	})
}
