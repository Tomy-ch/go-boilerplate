...
const (
	...
	// StatusCodeDelivered は、購入配達済みステータスのコードです。配達遷移の遷移先で、発送済みからのみ
	// 到達し、発送・支払い・キャンセルはいずれも不可能な終端状態です。ステータスの UUID はドメインに
	// 焼き込まず、永続化時に code から解決します（seed との二重管理を避けるため）。
	// code 値（9）も到達順序を意味しないため、遷移判定は等値比較のみで行う。
