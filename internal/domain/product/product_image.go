...
// SortKey は、同一商品内での表示順を返します。
func (i Image) SortKey() int { return i.sortKey }

// validateImages は、商品が保持する画像の集合が満たすべき不変条件を検証します。
// ID が未設定、画像パスが空、表示順が保持できる範囲外の場合はそれぞれ ErrInvalidID /
// ErrInvalidImagePath / ErrInvalidImageSortKey を、同一商品内で表示順が重複する場合は
// ErrDuplicateImageSortKey を返します。画像を持たない商品は許容します。
func validateImages(images []Image) error {
	...
	for _, img := range images {
		...
		if img.sortKey < minImageSortKey || img.sortKey > maxImageSortKey {
			return xerrors.Wrap(
				ErrInvalidImageSortKey,
				fmt.Sprintf("image sortKey must be between %d and %d, got %d", minImageSortKey, maxImageSortKey, img.sortKey),
			)
		}
		if _, ok := seen[img.sortKey]; ok {
			return xerrors.Wrap(ErrDuplicateImageSortKey, fmt.Sprintf("image sortKey %d is duplicated", img.sortKey))
		}
		seen[img.sortKey] = struct{}{}
	}
	return nil
}

// sortImagesBySortKey は、画像を表示順の昇順に並べた新しいスライスを返します。
// 集約の構築時に一度だけ通すことで、どの読み出し経路から取り出しても並びが同じになります。
func sortImagesBySortKey(images []Image) []Image {
	sorted := make([]Image, len(images))
	copy(sorted, images)
	slices.SortFunc(sorted, func(a, b Image) int { return a.sortKey - b.sortKey })
