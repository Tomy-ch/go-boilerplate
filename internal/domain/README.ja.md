...
- `internal/domain/<aggregate>/` 配下に **Entity / ValueObject / Repository(IF)** を配置する。
  **Domain Service** は置かない。集約を跨ぐものであり、そのどれか 1 つの内側には住めないためである
...
複数の集約に跨るルールは **Domain Service** に属する。Usecase ではない。
...
#### 集約横断の Domain Service をどこに置くか

`internal/domain/service/<name>/` 配下、つまりどの集約パッケージの外にも属さない場所に置く。集約に跨る
サービスは、そのどれか 1 つの内側には置けないためである（ドメインパッケージは他の集約を import できない）。
...
これが無ければ、ルールを集約の外へ出しても相手の集約へは届かず、上の配置ルールは従いようがない。

名前は業務の語彙にする。何についてのルールかを表す名であり、`common` / `shared` / `util` のように
何も名指さず、したがって何も拒めない名は用いない。

**入場基準は狭く、depguard の例外は招待状ではない。** 次の 3 つがすべて成り立つときにだけここへ置く。

1. ルールが集約に跨る——片方の集約の状態だけでは判定できない。
2. どちらの集約の自然な責務でもない。どちらかに収まるならそちらへ置く。
3. ステートレスで、かつ導出である（上の *Domain Service か Usecase か* を参照）。2 つの集約を読んで
   並べるだけなら写像であり、写像は Usecase に留まる。

ここのサービスは I/O を持たない。Repository も `context.Context` も受け取らず、Usecase が読み込み済みの
状態を受け取ってドメインのエラーを返す。その状態の取得・呼び出し順序・トランザクションの所有は
Usecase の責務のままである。

**現在の占有者は [`service/membership`](service/membership) である。** 一つの不変条件を両側から
