...
var (
	...
	ErrInvalidMessageID = xerrors.Wrap(errInvalid, "message id failed")
	// ErrEmptyBody は、メッセージ本文が空の場合のエラーです（422）。
	ErrEmptyBody = xerrors.Wrap(errInvalid, "message body is empty")
	// ErrBodyTooLong は、メッセージ本文が上限文字数を超える場合のエラーです（422）。
	ErrBodyTooLong = xerrors.Wrap(errInvalid, "message body is too long")
	// ErrInvalidSequence は、問い合わせ内の位置が正の整数でない場合のエラーです（422）。
	ErrInvalidSequence = xerrors.Wrap(errInvalid, "message sequence failed")
	// ErrInvalidAuthorKind は、送り手の種別が既知の 2 値でない場合のエラーです（422）。
	ErrInvalidAuthorKind = xerrors.Wrap(errInvalid, "message author kind failed")
	// ErrInvalidAuthorSubject は、送り手の主体 ID の検証に失敗した場合のエラーです（422）。
	ErrInvalidAuthorSubject = xerrors.Wrap(errInvalid, "message author subject failed")
)
