...
func Test_store_Claim(t *testing.T) {
	t.Parallel()
	...
	t.Run("正常系", func(t *testing.T) {
		...
		t.Run("新規キーのclaimはtrueを返しclaimed状態の行を作成する", func(t *testing.T) {
			t.Parallel()

			txm.WithinTx(func(ctx context.Context) {
				const scope, key = "test-scope-claim-new", "key-1"
		...
		t.Run("同一キーの再claimはfalseを返す", func(t *testing.T) {
			t.Parallel()

			txm.WithinTx(func(ctx context.Context) {
				const scope, key = "test-scope-claim-again", "key-1"
				params := idempotencybndry.ClaimParams{
					Scope: scope, Key: key, Method: "POST", Path: "/v1/users",
					Fingerprint: newFingerprint(0xAC), ExpiresAt: time.Now().Add(24 * time.Hour),
				}
		...
		t.Run("claim後の業務クエリは復元済みlock_timeoutで動き3s超のロック待ちでも55P03にならない", func(t *testing.T) {
			...
			assertClaimLockTimeoutRestored(t, s, testDB)
		})
	})
	...
	t.Run("異常系", func(t *testing.T) {
		...
		t.Run("ロック競合タイムアウト時はErrLockTimeoutを返す", func(t *testing.T) {
			...
			assertClaimLockContentionTimeout(t, s, testDB)
		})
		...
		t.Run("キャンセル済みコンテキストではErrCanceledへ正規化して返す", func(t *testing.T) {
			...
			_, err := s.Claim(ctx, idempotencybndry.ClaimParams{
...
// assertClaimLockTimeoutRestored は、claim 時の SET LOCAL lock_timeout(3s) が業務フェーズへ
// 波及していないことを、保持側 tx の advisory lock を claim の 3s 上限を超えて待たせても
// 55P03 にならない、という振る舞いで検証するテストヘルパーです。WithinTx は txLock で
// 直列化するため使わず、同一 TransactionManager の Do を 2 本並行させます
// （Do ごとに別コネクションの tx を張る）。
func assertClaimLockTimeoutRestored(t *testing.T, s *store, testDB driver.DatabaseDriver) {
	t.Helper()
...
// assertClaimLockContentionTimeout は、同一 (scope,key) を claim する 2 本の業務 tx を
// 並行競合させ、競合側が 55P03 → ErrLockTimeout へ正規化されることを検証するテストヘルパーです。
// testkit の WithinTx は txLock で直列化するため再現できないので、TransactionManager を
// 2 本（= 2 コネクション）並行で走らせて再現します。
func assertClaimLockContentionTimeout(t *testing.T, s *store, testDB driver.DatabaseDriver) {
	t.Helper()
...
func Test_store_Get(t *testing.T) {
	...
	t.Run("正常系", func(t *testing.T) {
		...
		t.Run("未claimのキーはnilを返す", func(t *testing.T) {
			t.Parallel()

			txm.WithinTx(func(ctx context.Context) {
				rec, err := s.Get(ctx, "test-scope-get-missing", "key-1")
				require.NoError(t, err)
				assert.Nil(t, rec)
			})
		})

		t.Run("claim済みのキーはclaimed状態で結果未保存の記録を返す", func(t *testing.T) {
			t.Parallel()

			txm.WithinTx(func(ctx context.Context) {
				const scope, key = "test-scope-get-claimed", "key-1"
				fp := newFingerprint(0xAD)
		...
		t.Run("complete済みのキーはcompleted状態と保存結果を返す", func(t *testing.T) {
			t.Parallel()

			txm.WithinTx(func(ctx context.Context) {
				const scope, key = "test-scope-get-completed", "key-1"

				claimed, err := s.Claim(ctx, idempotencybndry.ClaimParams{
					Scope: scope, Key: key, Method: "POST", Path: "/v1/users",
					Fingerprint: newFingerprint(0xAE), ExpiresAt: time.Now().Add(24 * time.Hour),
				})
				require.NoError(t, err)
				require.True(t, claimed)
	...
	t.Run("異常系", func(t *testing.T) {
		...
		t.Run("キャンセル済みコンテキストではErrCanceledへ正規化して返す", func(t *testing.T) {
			...
			_, err := s.Get(ctx, "ctx-cancel", "key-1")
			require.ErrorIs(t, err, apperror.ErrCanceled)
		})
	})
}

func Test_store_Complete(t *testing.T) {
	t.Parallel()
	...
	t.Run("正常系", func(t *testing.T) {
		...
		t.Run("claim済みキーのcompleteでcompleted状態と結果が保存される", func(t *testing.T) {
			t.Parallel()

			txm.WithinTx(func(ctx context.Context) {
				const scope, key = "test-scope-complete", "key-1"

				claimed, err := s.Claim(ctx, idempotencybndry.ClaimParams{
					Scope: scope, Key: key, Method: "POST", Path: "/v1/users",
					Fingerprint: newFingerprint(0xAF), ExpiresAt: time.Now().Add(24 * time.Hour),
				})
				require.NoError(t, err)
				require.True(t, claimed)
...
func Test_store_DeleteExpired(t *testing.T) {
	...
	t.Run("正常系", func(t *testing.T) {
		...
		t.Run("失効行がlimitを超える場合はlimit件で頭打ちになる", func(t *testing.T) {
			t.Parallel()

			txm.WithinTx(func(ctx context.Context) {
				const scope = "test-scope-gc-limit"
				keys := []string{"key-1", "key-2", "key-3"}

				for i, key := range keys {
					claimed, err := s.Claim(ctx, idempotencybndry.ClaimParams{
						Scope: scope, Key: key, Method: "POST", Path: "/v1/users",
						Fingerprint: newFingerprint(byte(0x10 + i)), ExpiresAt: time.Now().Add(-1 * time.Hour),
					})
					require.NoError(t, err)
					require.True(t, claimed)
				}

				// 失効行が 3 件以上ある状態で limit=2 なら、削除は 2 件で頭打ちになる。
				deleted, err := s.DeleteExpired(ctx, time.Now(), 2)
				require.NoError(t, err)
				assert.Equal(t, int64(2), deleted)

				// 削除順は expires_at 昇順で共有DB上の他の失効行が先に消え得るため、
				// 「3 件作成し 2 件しか消えない以上、この scope に 1 件以上残る」ことで頭打ちを検証する。
				remaining := 0
	...
	t.Run("異常系", func(t *testing.T) {
		...
		t.Run("キャンセル済みコンテキストではErrCanceledへ正規化して返す", func(t *testing.T) {
			...
			_, err := s.DeleteExpired(ctx, time.Now(), 100)
			require.ErrorIs(t, err, apperror.ErrCanceled)
		})
	})
}
