...
// FindByID は、ID から単一の商品ステータスエンティティを取得します。未存在は NotFound を返します。
func (r *repository) FindByID(ctx context.Context, id uuid.UUID) (*status.Status, error) {
	ctx, endSpan := r.tracer.Start(ctx)
	defer endSpan()

	db := gen.New(driver.New(ctx, r.db))
	row, err := db.GetProductStatusByID(ctx, id)
	if err != nil {
		return nil, pgerror.NormalizeError(err)
	}

	return rowToProductStatus(row.ID, row.Name, row.Code, row.SortKey)
}
