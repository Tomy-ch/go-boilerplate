...
<!-- sample-api:begin -->
The `sqs` branch — the only branch besides `http` — is wiring from the removable sample set (see [ADR-0049](../../../docs/adr/0049-broker-sdk-isolation-measured-as-coupling.md)); after `make setup-remove-sample-api` only the HTTP branch remains, while the SQS adapter itself stays as an unwired reference implementation.
<!-- sample-api:end -->
