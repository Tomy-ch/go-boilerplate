...
- Only the delivery channels a publisher implementation actually serves may be relayed; `VerifyChannel` fails relay startup when this package's implementations are wired for any channel but `http`, so a row is never handed to a substrate that cannot carry it. The `realtime` channel is served by `internal/infrastructure/realtime` instead, and the DI module (`OutboxRelayModule`) chooses the publisher module per channel and rejects unknown ones.
