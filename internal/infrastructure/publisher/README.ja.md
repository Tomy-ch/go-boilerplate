...
- publisher 実装が実際に配送できるチャネルだけを relay できる。この package の実装が `http` 以外のチャネル向けに結線された場合、`VerifyChannel` が relay の起動を失敗させ、運べない substrate へ行が渡らないようにする。`realtime` チャネルは代わりに `internal/infrastructure/realtime` が担い、DI モジュール（`OutboxRelayModule`）がチャネルごとに publisher モジュールを選び、未知のチャネルを拒否する。
