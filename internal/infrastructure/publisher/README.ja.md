...
<!-- sample-api:begin -->
`http` 以外の唯一の分岐である `sqs` 分岐は、削除可能なサンプル群からの配線です（[ADR-0049](../../../docs/ja/adr/0049-broker-sdk-isolation-measured-as-coupling.ja.md) を参照）。`make setup-remove-sample-api` の後は HTTP 分岐だけが残り、SQS adapter 自体は未配線の参照実装として残ります。
<!-- sample-api:end -->
