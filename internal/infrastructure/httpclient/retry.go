...
// isRetrySafe は、req が retry してよいリクエストかを返します（冪等性ガード A-2）。
...
// isRetryableOutcome は、試行結果が retry 対象かを返します（分類 A-3）。
...
// computeBackoff は、attempt（1 起算）に対する指数バックオフ + full jitter の待機時間を返します（A-4）。
...
// retryAfter は、Retry-After ヘッダ（delay-seconds 形式 / HTTP-date 形式）を解釈し待機時間を返します（B-7）。
...
// jitter は thundering herd 回避が目的で暗号強度は不要なため math/rand/v2 を用います。
