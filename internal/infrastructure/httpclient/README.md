...
- A downstream whose missing profile must not fall back unnoticed contributes its key through `RequiredDownstream()` to the `required_downstreams` fx group (`internal/di/module/httpclient.go`): an unregistered required key fails DI startup instead of silently taking `DefaultProfile`.
...
- Transport / status events are normalized into `apperror` sentinels (`ErrUnavailable` / `ErrCanceled` / `ErrInvalidArgument` etc.); callers branch on sentinels rather than on raw status (the one exported exception is the retry verdict below — [ADR-0024](../../../docs/adr/0024-outbound-http-resilience.md)).
- `RetryableOutcome` is exported as the substrate's sole retry verdict, for a caller that must tell a permanent failure from a transient one (the outbox relay's dead-letter decision is the current one). It exists because the sentinels alone cannot carry that distinction: an outcome that is deterministic for a reason internal to the substrate — a response exceeding `MaxResponseBytes` — keeps `ErrUnavailable` so the HTTP vocabulary stays honest, and is separated from the retryable 503s only here. A caller re-deriving a status table would silently lose that case and split the rule across two packages.
