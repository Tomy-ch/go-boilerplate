...
|`dynamodb/table.go`|`TableSpec` — partition key `ticket_hash`, TTL on `expires_at`, GSI `by_subject_destination` (partition `subject`, sort `destination`, `KEYS_ONLY`)|
...
| seam | DynamoDB |
| --- | --- |
| item | `ticket_hash` (S, partition key), `subject`, `destination`, `scope`, `initial_cursor` (N), `issued_at` (RFC 3339 nano), `expires_at` (N, epoch **seconds** — it is also the TTL attribute, so expiry is second-precise) |
...
- The item mapping (`toItem` / `fromItem`) is unit-tested without a connection.
