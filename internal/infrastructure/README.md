...
- Repository と QueryService の分割は「検索かどうか」ではなく、その読み取りが何を対象にするかで決める。
  ...
  QueryService。[ADR-0032 (lightweight-cqrs)](../../docs/adr/0032-lightweight-cqrs.md) を参照
...
|---|---|---|---|
|`auth/`|認証基盤（環境別 Authenticator 実装）|Usecase boundary|[README](auth/README.md)|
|`authz/`|認可基盤（Authorizer 実装。本番以外はデフォルトの `allowall`）|Usecase boundary|[README](authz/README.md)|
|`awsclient/`|`objectstorage/s3` と `queue/sqs` が共用する AWS 資格情報の解決|—（substrate、domain/usecase IF なし）|[README](awsclient/README.md)|
|`httpclient/`|resilient な HTTP client substrate（retry / circuit breaker / tracing）。`webapi/` と `publisher/` が共用する driver 相当の基盤|—（substrate、domain/usecase IF なし）|[README](httpclient/README.md)|
...
|`publisher/`|transactional outbox の publish 先（`boundary.Publisher` の HTTP 実装）|Usecase boundary|[README](publisher/README.md)|
...
|`rdb/`|RDB サブシステム（Repository / QueryService / driver / sqlc 等）|Domain / Usecase|[README](rdb/README.md)|
|`system/`|システム依存処理（時刻取得等）|Usecase boundary|[README](system/README.md)|
|`token/`|OS の乱数源による不透明なトークン生成（`boundary/token.Generator` の実装）|Usecase boundary|[README](token/README.md)<!-- sample-api:line -->|
|`webapi/`|外部 Web API gateway（`boundary.Gateway` の実装）|Usecase boundary|[README](webapi/README.md)|
