...
- `local` — 署名検証なし。トークン文字列から subject を抽出する。CI / test 専用のスタブ。
- `jwt` — JWT 検証（デファクト標準コア）。署名鍵は固定公開鍵または JWKS エンドポイントから解決。本番向けの方式。
...
|---|---|
|`local`|開発用スタブ — 署名検証なし|
|`jwt`|JWT 検証（標準コア）。鍵は固定公開鍵または JWKS|

環境 → 方式の対応づけは DI で適用されます（「DI への登録」参照）。CI / test は `local` スタブを使い、`jwt` はローカル開発（mock 認証サーバーの実 JWT を検証）と実トークン検証を配線する環境を担います。
...
`jwt` は、アクセストークン（JWT）を検証し、デファクト標準の検証コアを扱います。署名鍵は**固定 RSA 公開鍵**（`New`）または **JWKS エンドポイントからの `kid` 解決**（`NewJWKS`）のいずれかで解決し、クレーム検証ロジックは共通です。

ここでは次を行います。

- 署名検証（非対称、アルゴリズム allowlist。`alg=none` / `HS256` は拒否）
- 鍵解決（固定公開鍵、または JWKS の `kid` 参照 / TTL キャッシュ。`go-jose` でパースし `httpclient` substrate 経由で遅延取得）
...
**検証方式**が選択されます（例: CI / test は `local` スタブ、local / development は `jwt`）。
