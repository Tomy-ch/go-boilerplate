...
- 再取得は、それを誘発したリクエストから切り離して走ります。鍵集合の更新は全リクエストが共有する作業であり、たまたま誘発したリクエストの寿命に支配させると、切断された試行が記録されないまま残る —— 意図的に切断する呼び出し元は、cooldown が許すより速く再取得を誘発できてしまいます。呼び出し元は自分の予算で離脱し（キャンセルは `499`、予算切れは `503`）、取得は substrate 自身のタイムアウトで完走して結果を記録するため、切断された試行は捨てられず次のリクエストのためにキャッシュを温めます。
