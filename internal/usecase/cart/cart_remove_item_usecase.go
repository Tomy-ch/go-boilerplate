...
func (u *usecase) RemoveItem(ctx context.Context, params RemoveItemParams) error {
	ctx, endSpan := u.tracer.Start(ctx)
	defer endSpan()

	if params.ProductID.IsNil() {
		return nil
	}

	now := u.clock.Now()
	return u.txm.Do(ctx, func(ctx context.Context) error {
		c, found, err := u.resolveCart(ctx, params.Subject, now)
		if err != nil {
			return err
		}
		if !found {
		...
		if rerr := locked.RemoveItem(params.ProductID); rerr != nil {
			return rerr
		}
