...
- **書き込み** — インフラが書き込みを実行し、その後 Usecase が対象集約を Repository 経由で読み直し、
  集約自身に検証させます（[ADR-0027](../../docs/ja/adr/0027-lightweight-cqrs.ja.md)）。
...
QueryService 経由の読み取りにはこの検証が及びません。返るのは射影であり、突き合わせる対象の
エンティティが存在しないためです。その経路の基準はレビューだけが支えており、これは意図した割り切り
です。検証しようとすれば、射影が避けるために存在している集約の再構築をやることになります。
