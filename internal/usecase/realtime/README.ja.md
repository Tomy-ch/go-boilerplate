...
| `Health.Check()` / `ObserveFanout(err)` / `FanoutDegraded()` | この機構がまだ配信を続けられるかどうか。2 つの依存を別々の方法で見るのは、答え方が違うからである。EventLog は都度読みに行けば分かる（誰も書かない stream の読み取りで、「無い」が健全な答え。エラーだけが不健全）。fan-out は受信を試みた者にしか分からないので、consumer engine が試行のたびに `ObserveFanout` で報告し、その判定を持ち回る。起動時の probe・`/ready`・新規 SSE 接続の可否という 3 つの呼び手が同じ 1 つの値を見るので、「到達できる」の意味が起動時と稼働中でずれない（[設計 §2.6](../../../docs/design/realtime-delivery.ja.md)）。縮退は双方向で、fan-out が回復すればフラグは戻る — 片道の掛け金にすると、一時的な失敗のあと接続を断り続けることになる | 通知が届いていないときは `ErrFanoutUnreachable`（`apperror.ErrUnavailable` を包む）。store の失敗はそのまま通す |
