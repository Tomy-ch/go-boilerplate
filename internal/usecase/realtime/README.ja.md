...
本 package の外: replay の読み取りと catch-up（stream handler が接続状態と一緒に持つ）、heartbeat の*ループ*
（`internal/controller/realtime` が serve lifecycle のスケジュールで `LeaseKeeper.Beat` を駆動する）、
orphan cleanup の引き受け（cleanup job）。
