...
func Test_decodePurchaseCursor(t *testing.T) {
	...
	t.Run("正常系", func(t *testing.T) {
		...
		t.Run("有効な2キーのカーソルはpurchaseCursorへ復号される", func(t *testing.T) {
			t.Parallel()
			encoded := paging.EncodeCursor(validTime.Format(time.RFC3339Nano), validID.String())
			cursor, err := paging.NewCursor(&encoded, nil)
			require.NoError(t, err)

			actual, decErr := decodePurchaseCursor(cursor)
			require.NoError(t, decErr)
			require.NotNil(t, actual)
			assert.True(t, validTime.Equal(actual.orderedAt))
	...
	t.Run("異常系", func(t *testing.T) {
		...
		t.Run("ordered_atがRFC3339Nanoでない場合はErrInvalidArgumentを返す", func(t *testing.T) {
			t.Parallel()
