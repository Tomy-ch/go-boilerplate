...
- **満杯だが進捗ゼロ → 必ず待機**のルールが load-bearing：全件失敗の満杯バッチを待機ゼロで再 claim すると、既に停止している下流に対してホットループし、まだ直しようのない失敗に DB とネットワークの容量を費やす。stall した満杯バッチは常に待機へ落とす（行ごとのバックオフにより再 claim 自体が起こりにくいが、ループの平静をそれに依存させない）。
