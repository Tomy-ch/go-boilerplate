...
- 設計上冪等です。対象集合は年齢述語と参照照合で決まるため、再実行しても既に条件を満たすものだけが回収され、リトライは安全です。排他制御は持たず、並行制御はスケジューラの責務です（[ADR-0109 (scheduled-job-concurrency-delegated)](../../../../docs/adr/0109-scheduled-job-concurrency-delegated.md)）。
