...
- Idempotent by design: the target set is defined by an age predicate and a reference check, so re-running only reclaims what is already eligible and retries are safe. There is no exclusive locking — concurrency is the scheduler's concern ([ADR-0100](../../../../docs/adr/0100-scheduled-job-concurrency-delegated.md)).
