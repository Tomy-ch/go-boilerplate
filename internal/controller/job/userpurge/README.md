...
4. 失敗時は、伝播する前に同じ 2 つの件数を **Warn** でログ出力します（理由は [job/README.md § GC / バッチジョブ](../README.md) を参照）。エラー自体はそのまま返します（Runner / CLI に伝播し、exit code は呼び出し側が決定します。ジョブは `os.Exit()` を呼びません）。
...
- 設計上べき等です。対象は経過時間の述語で決まるため、再実行しても既に条件を満たすユーザーを消すだけで、リトライは安全です。排他制御は持たず、多重起動の制御はスケジューラの関心事です（[ADR-0105 (scheduled-job-concurrency-delegated)](../../../../docs/adr/0105-scheduled-job-concurrency-delegated.md)）。
