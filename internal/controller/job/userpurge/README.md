...
- Idempotent by design: the target set is defined by an age predicate, so re-running only erases users that are already eligible and retries are safe. There is no exclusive locking — concurrency is the scheduler's concern ([ADR-0105 (scheduled-job-concurrency-delegated)](../../../../docs/adr/0105-scheduled-job-concurrency-delegated.md)).
