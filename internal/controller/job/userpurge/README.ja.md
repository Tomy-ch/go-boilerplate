...
- 設計上べき等です。対象は経過時間の述語で決まるため、再実行しても既に条件を満たすユーザーを消すだけで、リトライは安全です。排他制御は持たず、多重起動の制御はスケジューラの関心事です（[ADR-0102](../../../../docs/adr/0102-scheduled-job-concurrency-delegated.md)）。
