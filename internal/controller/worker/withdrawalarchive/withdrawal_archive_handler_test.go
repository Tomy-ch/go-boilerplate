...
// newHandlerUnderTest は、mock を差した worker から Handler を取り出します。
func newHandlerUnderTest(t *testing.T) (workerbd.Handler, *mock_user.MockArchiveUsecase) {
	t.Helper()

	archive := mock_user.NewMockArchiveUsecase(gomock.NewController(t))
	w := withdrawalarchive.New(
		nil,
		nil,
		archive,
...
func TestHandler_Handle(t *testing.T) {
	...
	t.Run("正常系", func(t *testing.T) {
		...
		t.Run("退会イベントの payload をそのまま保存へ渡す", func(t *testing.T) {
			...
			const body = `{"userId":"u1","deletedAt":"2026-07-29T12:00:00Z"}`
			archive.EXPECT().
				ArchiveWithdrawal(gomock.Any(), user.ArchiveWithdrawalParams{
					UserID:  "u1",
					Payload: []byte(body),
				}).
				Return("withdrawals/u1.json", nil)
	...
	t.Run("異常系", func(t *testing.T) {
		...
		t.Run("入力不正による保存失敗を Permanent として分類する", func(t *testing.T) {
			...
			err := h.Handle(t.Context(), withdrawnMessage(`{"userId":""}`))
		...
		t.Run("保存の一時失敗は分類せずそのまま返す", func(t *testing.T) {
			...
			err := h.Handle(t.Context(), withdrawnMessage(`{"userId":"u1"}`))
