...
// Run の多数の挙動シナリオを正常系/異常系のサブテストとして束ねる構造上、認知的複雑度が
// 閾値を超えるが、各サブテストは独立しており分割は subject 単位の規約に反するため抑止する。
//
//nolint:gocognit // Run の全シナリオを 1 subject=1 テストへ集約するため（各サブテストは独立）
