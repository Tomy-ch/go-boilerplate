...
SQS 参考 adapter（`internal/infrastructure/queue/sqs`）は**削除可能なサンプル群**の一部として配線してよい。broker SDK の分離は adapter を未配線にすることではなく、`make setup-remove-sample-api` の**後**に検証する — E3'（[`docs/adr/0049`](../../../docs/ja/adr/0049-broker-sdk-isolation-verified-after-sample-removal.ja.md)）を参照。
