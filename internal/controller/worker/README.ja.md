...
SQS 参考 adapter（`internal/infrastructure/queue/sqs`）は**削除可能なサンプル群**の一部として配線してよい。broker SDK の分離はリンクではなく**結合**で測る。具体的な broker を名指すのは、その adapter のパッケージとそれを選ぶ配線だけである — E3（[ADR-0052 (broker-sdk-isolation-measured-as-coupling)](../../../docs/adr/0052-broker-sdk-isolation-measured-as-coupling.ja.md)）を参照。
