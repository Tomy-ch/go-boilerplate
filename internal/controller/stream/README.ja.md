...
- **fan-out は通知を、この package の connection registry が実装すべき sink へ渡す**（Phase 7、#1414）: `controller/realtime.WakeupSink.Wake(ctx, streamID, upTo)` — その stream の接続のうち cursor が `upTo` より手前のものを起こす（冪等。重複は正常であり、receive batch をまたぐ coalescing は registry の責務）— と `controller/realtime.RevocationSink.Revoke(ctx, subject, destination)` — その subject の接続を `STOP` で閉じる。どちらも consumer loop 上で同期的に呼ばれるので、印を付けるか通知するだけにとどめる。serve lifecycle の停止順は drain → consumer 停止 → instance queue の teardown → HTTP shutdown（`internal/di/server/hook`）であり、registry はその drain を `hook.Drainer` participant（新規接続を拒否し、`RECONNECT` / `SERVER_DRAINING` を送り、接続が閉じるまで待つ）として realtime の DI module から供給する。
