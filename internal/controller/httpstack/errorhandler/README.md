...
- **Commit state** — an already-committed response skips the write entirely, and a failing write is logged without committing a second time. Reproduce the failure with a `ResponseWriter` whose `Write` always errors. The fallback `WriteHeader(500)` nested inside that failure needs the *`server.ResponseOf` degradation* viewpoint layered on top: install that same writer through `c.SetResponse` as well, so `responseCommitted` stays false across the failure and the fallback runs. Give the JSON write a status other than 500 — a validation error works — because the writer records the status the JSON path sends before it fails, and only a differing status proves the trailing 500 came from the fallback.
