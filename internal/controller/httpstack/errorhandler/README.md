...
- **Log gating** — `ObservabilityConfig.TargetStatusCodeSet()` decides whether anything is logged at all, and the 500 boundary decides `Error` vs `Warn`. Exercise a status inside and outside the set, plus both sides of the boundary — each side taken from a status the set actually contains, since the gate runs first and a status outside it never reaches the level switch. The adjacent pair around the boundary (499 / 500) is deliberately not pinned: 499 is in no configured set, so reaching it would take a `config` setter existing only for this test, and the mutations the boundary admits are already caught by the 500 case. Assert on the observed entry's message (`errorhandler.server_error` / `errorhandler.client_error`) — that string is what alerting keys on, not the level alone.
