...
// detailsPropertyName は、error レスポンススキーマにおける details プロパティ名です。
// このプロパティを持つスキーマ(ErrorResponseWithDetails)を宣言した operation だけが details を返せます。
...
type DetailPolicy interface {
	...
	// ルート不一致・operation 未解決・未 opt-in はいずれも false(fail-closed)。
	Allows(req *http.Request) bool
}

// openAPIDetailPolicy は、OpenAPI spec から details 公開を許可された operationId を前計算し、
// リクエストごとに router で operation を解決して判定します。
...
func responseHasDetailsProperty(respRef *openapi3.ResponseRef) bool {
	...
	_, ok := mediaType.Schema.Value.Properties[detailsPropertyName]
	return ok
}
