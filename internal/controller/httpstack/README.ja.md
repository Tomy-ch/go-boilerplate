...
`httpstack` はアプリケーション全体で使われる Echo ミドルウェアおよびサーバー設定ヘルパーのカタログです。各サブパッケージは 1 つの関心事（リクエスト ID / ロギング / リカバリ / CORS / セキュリティヘッダ / レートリミット 等）を担い、薄い `Middleware(...)` または `New(...)` コンストラクタを公開します。ミドルウェアの **登録** は意図的に別の場所（`internal/di/server/extension`）で行い、本ディレクトリは fx や Echo インスタンス依存を含めない設計にしています。これにより各ユニットは独立して単体テスト可能・再利用可能になります。
