...
- `mock/mock_ip_rate_limiter.go` に GoMock ベースのモックが生成されています。ミドルウェアのユニットテストではこのモックを注入して `AllowRequest` の挙動を制御できます。
- パッケージ内に `*_test.go` があり、実装の動作例と検証が含まれています。テストを参照して期待挙動を確認してください。

必要なら、この README に設定のサンプル（`config.IPRateLimitConfig` の例）や `Cleanup()` を定期実行する systemd/ticker の例を追加します。
