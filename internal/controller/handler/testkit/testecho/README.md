...
## EchoTestClient の基本（`echotest.go`）
...
本 README は `echotest.go`（リクエスト生成）に焦点を当てています。レスポンスの JSON 比較やルート検証などのアサーションに関するヘルパーが別ファイルで提供されている場合は、そちらを参照してください（プロジェクト構成により別ディレクトリに分かれていることがあります）。
