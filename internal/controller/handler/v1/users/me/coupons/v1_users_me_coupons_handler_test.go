...
func TestBindHandler(t *testing.T) {
	...
	assert.Equal(t, "/v1/users/me/coupons", routes[0].Path)
}

func Test_server_GetUsersMeCoupons(t *testing.T) {
	t.Parallel()

	t.Run("正常系", func(t *testing.T) {
		t.Parallel()

		t.Run("保有クーポンを200で返す", func(t *testing.T) {
			t.Parallel()
			...
			resp, err := s.GetUsersMeCoupons(authnContext(t), gen.GetUsersMeCouponsRequestObject{})
			require.NoError(t, err)

			actual, ok := resp.(gen.GetUsersMeCoupons200JSONResponse)
			require.True(t, ok)
			require.Len(t, actual.Coupons, 1)
			assert.Equal(t, view.ID.ToPrimitive(), actual.Coupons[0].Id)
		})

		t.Run("1枚も無い場合は空配列を返す", func(t *testing.T) {
			t.Parallel()
	...
	t.Run("異常系", func(t *testing.T) {
		...
		t.Run("authnが無い場合、認証エラーを返しユースケースを呼ばない", func(t *testing.T) {
			...
			resp, err := s.GetUsersMeCoupons(context.Background(), gen.GetUsersMeCouponsRequestObject{})
		...
		t.Run("ユースケースの失敗をそのまま伝播する", func(t *testing.T) {
			...
			s, uc := newServer(t)
			uc.EXPECT().ListMyCoupons(gomock.Any(), gomock.Any()).Return(nil, apperror.ErrCanceled)
...
func Test_toCouponResponses(t *testing.T) {
	t.Parallel()
	...
	t.Run("正常系", func(t *testing.T) {
		...
		t.Run("順序を保って全件を写す", func(t *testing.T) {
			...
			got := toCouponResponses([]couponuc.CouponView{a, b})

			require.Len(t, got, 2)
			assert.Equal(t, a.ID.ToPrimitive(), got[0].Id)
			assert.Equal(t, b.ID.ToPrimitive(), got[1].Id)
		})

		t.Run("空の場合は空を返す", func(t *testing.T) {
			t.Parallel()

			assert.Empty(t, toCouponResponses(nil))
		})
	})
}
