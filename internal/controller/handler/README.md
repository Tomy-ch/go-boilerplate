...
Echo に登録するルートは、必ず OpenAPI spec の operation（メソッド + パス）と対応していなければなりません。例外のための許可リストは持ちません。許可リスト自体がドリフト源になるためです。規約そのものの正本は [OpenAPI-first ルール](../../../docs/rules.md#openapi-first-ルール)にあり、この節はハンドラを書くうえで何を意味するかを扱います。
