...
|`closed-loop/send/`|Send the windows that have closed but never reached their feedback issue: the observation block as the issue body, and the turns a deterministic pass selected as worth reading as a comment. Creating the issue and posting the comment are separate failures — an issue with no comment is retried for its comment alone, because folding them together would let a half-sent window read as done. Runs on the host: the transcripts it reads live under the user's home, which the tool-runner container cannot see.|`make closed-loop-send` / SessionStart hook|
|`closed-loop/weekly/`|Collect the feedback issues created in a period, cluster them by the classification labels they carry, and score each cluster on frequency / impact / human intervention / recurrence so a retrospective has an order to work through. Recurrence weighs heaviest: a problem that reappears across windows is a foundation problem rather than one person's slip. Also names the windows whose merge wait exceeded their implementation time, since working faster does not shorten those.|`make closed-loop-weekly FROM=<date> TO=<date>`|
