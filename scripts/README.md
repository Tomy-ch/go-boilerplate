...
|`pnpm-cooldown/`|Check the `minimumReleaseAgeExclude` entries in every `pnpm-workspace.yaml`. Unlike `go-cooldown` and `tool-cooldown` this is not the window's guard: pnpm's resolver enforces the window itself and re-verifies the whole lockfile on every install, `--frozen-lockfile` included. What had no guard is the exemption. An entry must carry `expires: YYYY-MM-DD` and `issue: <N>` in its trailing comment plus a reason, and the check fails when that metadata is missing or unreadable, when the deadline has passed, when it reaches beyond three months, or when the sibling `pnpm-lock.yaml` no longer resolves the named version. A deadline arrives without the declaration changing, so this runs on a schedule as well as on the pull request — the same reason its siblings do. The expiry lives in a comment because a YAML list item is a string: pnpm reads the entry, the comment is where a date can sit without pnpm rejecting it.|`make pnpm-cooldown-check`|
