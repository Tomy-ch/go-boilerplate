...
├── pr-comment-fence-lint.mjs   # Reject a fixed-length Markdown fence around a PR comment body
├── actions-cutoff-lint.mjs     # Require a job timeout and a PR comment that survives a cut-off
...
|`pr-comment-fence-lint.mjs`|Fail when a workflow's `run:` block emits a fixed-length Markdown fence around a PR comment body, and when the duplicated `fence_for` helpers stop agreeing with each other. Dependency-free ESM. Enforces a rule `actionlint` cannot express — see [`.github/workflows/README.md`](../.github/workflows/README.md) for why a fence must be sized from the text it wraps. Reach: literal fences in an `echo`, and textual equality between the helper implementations; whether a given body is attacker-controlled is not decidable here and is left to the rule.|`make actions-lint` / `make actions-comment-fence-lint`|
|`actions-cutoff-lint.mjs`|Fail when a job carries no `timeout-minutes`, and when a step calling `./.github/actions/upsert-pr-comment` has an `if:` that a cancelled job cannot reach. Dependency-free ESM. Enforces a rule `actionlint` cannot express — see [`.github/workflows/README.md`](../.github/workflows/README.md) for what a cut-off has to leave behind. Reach: `always()` / `cancelled()` in the condition, `failure()` deliberately not counting since it is false for a cancelled job; jobs calling a reusable workflow are skipped because the key is invalid there. A condition that negates its own reachability (`!always()`) is writable and not statically caught — the rule is what holds.|`make actions-lint` / `make actions-cutoff-lint`|
