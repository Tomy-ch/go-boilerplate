...
├── reset-mock-auth-users.mjs   # Reset the mock-auth user fixture to a neutral default
...
|`reset-mock-auth-users.mjs`|Overwrite the mock-auth user fixture (`docker/mock-auth-server/fixtures/users.json`) with a neutral default. The file itself is never deleted, so the mock still starts. `make setup-remove-sample-api` calls it to replace the demo identities (John Doe and friends) with a single neutral user.|
