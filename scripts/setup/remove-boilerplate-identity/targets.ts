 ...
 * いる規則（`docs/rules.md` の *No premise the document will outlive*）は一般形なので作成先が
 * 継承しますが、検査が探す言い回し（`this template` / `adopters` / `このテンプレート`）は上流
 * 固有の実例でしかありません。作成先にはその前提が無いので、残しても永久に緑のままの検査が
 * 増えるだけで、赤くなったときに何を意味するのかも読めません。
 *
 * `marker-baseline` も同じです。あれが守るのは撤去マーカーを**書く側**で、書く場面は上流に
 * しかありません。作成先が受け取るのはマーカーが解決し終えたツリーなので、見張る対象が居ません。
 *
 * `docs/plan` は上流が着手していないリリース線の要件です。ロードマップ本体と違い作成先で
