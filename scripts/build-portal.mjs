...
//
// 本スクリプトは docs/portal/src/main.jsx を起点に依存を node_modules から解決し、生成物を docs/portal/dist/ へ書き出す。
// これにより外部ネットワーク依存とブラウザ内 Babel を廃止し、手書き（src/）と生成物（dist/）をディレクトリで明確に分離する。
// mermaid / highlight.js は src 側の遅延ロードで初期表示から外す。
//
