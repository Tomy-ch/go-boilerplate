...
func TestParseFrom(t *testing.T) {
	...
	t.Run("正常系", func(t *testing.T) {
		...
		t.Run("image:tag@digest は digest を捨て image:tag を返す", func(t *testing.T) {
			t.Parallel()
			r, ok := parseFrom("golang:1.26-alpine@" + digestGolang)
			require.True(t, ok)
		...
		t.Run("registry ホスト付き image も最後のコロンで tag を分離する", func(t *testing.T) {
			t.Parallel()
			r, ok := parseFrom("ghcr.io/foo/bar:v1.0")
			require.True(t, ok)
		...
		t.Run("registry ポート付きでも tag を正しく分離する", func(t *testing.T) {
			t.Parallel()
			r, ok := parseFrom("localhost:5000/app:v2")
			require.True(t, ok)
