...
//      合格条件から `failure()` を外すのは、それが cancelled で false になるためである。「ステータス
//      関数を持つか」で書くと、関数はあるのに打ち切り時は沈黙するステップを取り逃がす。
//
...
// この検査はその近似にあたる。構造は YAML パーサではなく桁で読む。ブロックスカラーの中身は必ず
// 親より深い桁に来るという YAML の規約が成り立つ前提で、これは同じ `actions-lint` の中で先に走る
// actionlint が担保する。node の標準ライブラリのみに依存し、ホストでもコンテナでも動く。
...
// ステップ単位に切り出す。`if:` / `title:` はそのステップのものだけを見る必要があるため、
// ジョブ本文を `- ` 始まりで区切る。
...
function titleOf(step) {
  const line = step.lines.find(({ text }) => STEP_KEY_TITLE.test(text))
  if (!line) return null
  return { line: line.number, value: line.text.match(STEP_KEY_TITLE)[1].trim() }
}
...
for (const rel of workflowFiles) {
  ...
  const jobs = splitJobs(source.split("\n"))
  ...
  for (const job of jobs) {
    ...
    for (const step of splitSteps(job)) {
      ...
      } else if (!REACHES_CANCELLED.test(condition.value)) {
        report(
          rel,
          condition.line,
      ...
      } else if (!CUT_OFF_HEADING.test(title.value)) {
        report(
          rel,
          title.line,
