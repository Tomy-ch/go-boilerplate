...
func main() {
	log.SetFlags(0)
	if len(os.Args) < minArgs || os.Args[1] != "audit" {
		log.Fatalf("❌ usage: npm-cooldown audit [--base=<git-ref>] [--summary-out=<path>] [--github]")
	}
	...
	findings, audited, err := auditAll(context.Background(), root, locks, *base)
	if err != nil {
		log.Fatalf("❌ %v", err)
	}
	...
	if *summaryOut != "" {
		if err := os.WriteFile(*summaryOut, []byte(summary(findings, audited, *base)), summaryPerm); err != nil {
			log.Fatalf("❌ write summary: %v", err)
		}
	}
	if out := os.Getenv("GITHUB_OUTPUT"); out != "" {
		if err := appendOutput(out, len(findings), audited); err != nil {
			log.Fatalf("❌ write GITHUB_OUTPUT: %v", err)
		}
	}
	// finding があっても 0 で終える（パッケージ doc の設計意図）。
}
...
func auditAll(ctx context.Context, root string, locks []string, base string) ([]finding, int, error) {
	var findings []finding
	audited := 0

	for _, lock := range locks {
		fs, n, err := auditLock(ctx, root, lock, base)
		if err != nil {
			return nil, 0, err
		}
...
func auditLock(ctx context.Context, root, lock, base string) ([]finding, int, error) {
	...
	times, err := publishTimes(ctx, entries)
	if err != nil {
		return nil, 0, err
	}
...
func publishTimes(ctx context.Context, entries []entry) (map[string]time.Time, error) {
	...
	for name, es := range byName {
		...
		go func(name string, es []entry) {
			...
			times, err := packumentTimes(ctx, name)
			mu.Lock()
			defer mu.Unlock()
			if err != nil {
				if firstEr == nil {
...
func packumentTimes(ctx context.Context, name string) (map[string]time.Time, error) {
