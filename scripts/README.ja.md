...
|`pnpm-cooldown/`|各 `pnpm-workspace.yaml` の `minimumReleaseAgeExclude` を検査する。`go-cooldown` / `tool-cooldown` と違い、これは窓の番人ではない。窓そのものは pnpm の解決器が強制しており、しかも解決時だけでなく install のたびに lockfile 全体を照らし直す（`--frozen-lockfile` の再生も含む）。番人が居なかったのは免除のほう。エントリは行末コメントに `expires: YYYY-MM-DD` と `issue: <N>`、加えて理由を持つ必要があり、そのメタが欠けているか読めないとき、期限が切れているとき、上限の 3 ヶ月を越えているとき、同じディレクトリの `pnpm-lock.yaml` が名指しの版をもう解決していないときに失敗する。期限は宣言が変わらなくても訪れるので、pull request だけでなくスケジュールでも走る——兄弟がそうしているのと同じ理由。期限をコメントに置くのは YAML のリスト項目が文字列だから。pnpm が読むのはエントリ本体で、日付が pnpm に拒否されずに座れる場所がコメントしかない。|`make pnpm-cooldown-check`|
