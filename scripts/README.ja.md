...
|`remove-doc-language/`|ドキュメント / スキルの対訳ペアを 1 言語へ畳む（`--lang en\|ja\|both`）。`en` は `*.ja.md` を全削除し、`ja` は対訳を正本の名前へ改名して正本のフロントマターを移植する（`SKILL.md` が読み込み可能なまま保たれる）。`both` は対訳を残してマーカーだけを解決する —— 選び終えたツリーに宣言が残ると、まだ選べると読み違えるためである。対訳規約を説明している散文は機械的に畳まず、`doc-pair` マーカーか `language-manifest.ts` の宣言が持つ。宣言の無い行に当たると 1 バイトも書かずに中止する。|
