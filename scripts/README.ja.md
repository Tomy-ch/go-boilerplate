...
├── mise-cooldown/              # mise.toml のツール固定をクールダウン窓に照らして検査 / 監査（Go）
...
- **判定を検査し、その外側のシェルは検査しない。** どのツールも、エラーを終了コードへ変えるだけの
  `main`、フラグを解釈して振り分ける `run`、そして判定を担う純粋関数へ分かれる。`run` は不純な依存
  ——作業ディレクトリ・HTTP クライアント・現在時刻・カバレッジ総量・コマンド実行器——を引数で受け、
  振り分けそのものをテストから触れるようにする。`main` は意図的に唯一の未検査シンボルであり、
  だからこそそこに分岐を置いてはならない。
