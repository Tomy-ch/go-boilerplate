...
-- 衝突時に user_id を同じ値で書き戻すのは、DO NOTHING では RETURNING が行を返さないためであり、
-- あわせて DO UPDATE が衝突した行にロックを取る。返ったカートはこのトランザクションが握っているので、
-- 呼び出し側は別途ロックを取らずに触ってよい（DO NOTHING へ変えるとこの保証は失われる）。
INSERT INTO carts (
    id,
    user_id,
    session_token,
