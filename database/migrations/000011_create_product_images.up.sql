...
-- 一意性は生存行にだけ課す。UNIQUE (product_id, sort_key, deleted_at) では NULL 同士が相異なる値として
-- 扱われるため、deleted_at IS NULL の行が同じ (product_id, sort_key) で並存でき、一意性が消える。
CREATE UNIQUE INDEX product_images_product_id_sort_key_unique ON product_images (
    product_id, sort_key
)
...
COMMENT ON COLUMN product_images.sort_key IS '順序';
