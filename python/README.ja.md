...
他のツールのバージョンはすべて `mise.toml` にあります（[ADR-0075](../docs/adr/0075-mise-ssot-drift-gate.md)）。PyPI のツールだけが例外なのは、バージョンを固定してもほとんど何も固定できないからです。依存は install 時に解決されるため、同じ pin でも日によって違うツリーが入り、バージョンの pin を lockfile として読めるスキャナも存在しません。
