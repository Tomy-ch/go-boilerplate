...
スタイル体系が定めているのは拡張点（`print:` variant / `@media print`）だけで、最小実装を同梱するかは実装フェーズの判断でした。これがその実装です。
...
PDF をサーバで組む場合は backend の責務です。ここが担うのは印刷可能な HTML と CSS を出すところまでで、その先は境界として切ります。
