...
gen-mock-auth-oapi-ci:
	cd mock-auth-server && PATH="/app/scripts/node_modules/.bin:$$PATH" pnpm run gen

# Redoc は redocly（mise グローバル）で生成する。出力は repo の docs/openapi/mock-auth-server/ 配下。
gen-mock-auth-oapi-docs-ci:
	cd mock-auth-server && pnpm run gen:docs

lint-mock-auth-oapi-ci:
	cd mock-auth-server && PATH="/app/scripts/node_modules/.bin:$$PATH" pnpm run lint:oapi
