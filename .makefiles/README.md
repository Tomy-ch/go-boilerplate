...
| `make pnpm-cooldown-check` | Fails on a `minimumReleaseAgeExclude` entry whose deadline has passed, reaches beyond three months, is missing the machine-readable `expires` / `issue` metadata, or names a version the sibling `pnpm-lock.yaml` no longer resolves. | Runs on the host. There is no `gate` counterpart because pnpm's resolver already enforces the window on every install; what this checks is the exemption, whose deadline arrives without the declaration changing. |
