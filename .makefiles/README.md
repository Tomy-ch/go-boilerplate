...
| Command | Description | Notes |
| --- | --- | --- |
| `make actions-lint` | Lints workflow definitions with actionlint, shellchecks the `run:` scripts of every composite action, then runs the three node checks: no job posting a PR comment receives a secret, no comment body is wrapped in a fixed-length fence, and every job defines what happens when it is cut off. | The one lint group whose stages span two tool-runners: it invokes `make actions-actionlint-ci` and `make actions-shellcheck-ci` in `go_tool_runner` and the three `*-lint-ci` node stages in `node_tool_runner`, rather than one `-ci` target in one container, because actionlint / the shellcheck runner are Go tools and the rest node scripts. |
...
| `make actions-lint-ci` | Runs all five stages directly. | CI target. |
