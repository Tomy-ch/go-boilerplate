...
| `make md-premise-lint` | Checks only that no document which outlives a fork rests on a premise that lapses with it. | Invokes `make md-premise-lint-ci` inside the `node_tool_runner` container. |
