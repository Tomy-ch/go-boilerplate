...
#
# 窓の数は git worktree の数で数える。スロットのリースではなく worktree を数えるのは、スロット
# 取得が opt-in で「窓はあるがスロットは取っていない」状態が普通にあるためで、CPU を食うのは窓のほう。
#
# 判定そのものは scripts/load-band（テストの当たる Go 側）が持ち、ここは設定値を渡して結果を
# 受け取るだけにする。負荷帯を決め損ねると「ホストを飽和させる」「検査を丸ごと飛ばす」の
# どちらへも黙って倒れるため、判定をシェルに置かない。
#
