...
プール（`cmd/db-pool`・コア `internal/cli/dbpool`）は per-worktree のデータベース（`wt<N>_local` / `wt<N>_test`）を起動**後**に
動的作成するため、同じ拡張と timezone を自身でブートストラップします。この設定は `internal/cli/dbpool`（`PgxAdmin.SetupDatabase`）
