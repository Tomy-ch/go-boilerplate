...
`github.com/shopspring/decimal` をラップした exact-decimal 値オブジェクトです。金額・レートなどの正確な十進量を `float64` 誤差なく表現します。
...
業務意味論は一切持ちません。通貨・非負・最小単位の選択は上位層の値オブジェクト（`internal/domain/kernel/money`）が所有します。本パッケージは純粋な十進算術・丸め・スケール変換と DB / ワイヤ境界だけを担います。
