...
# ADR-0028: マイグレーションを追記専用かつイミュータブルとして扱う

English canonical: [0028-append-only-immutable-migrations.md](0028-append-only-immutable-migrations.md)
...
- `migration-check.yaml`が検証するのは**採番のみ**である（バージョンの重複・欠番・`up`と`down`の対応。その規律については[ADR-0029](0029-sequential-migration-ids.ja.md)参照）。イミュータビリティ自体にCIゲートは無い。
