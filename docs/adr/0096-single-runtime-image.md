...
# ADR-0096: A single runtime image with command override (no purpose-specific images)
...
[ADR-0095](0095-single-multi-command-binary.md)), deployments require not only the HTTP
...
- One image digest to sign, attest, and verify per deployment. Supply-chain integrity
  checks apply uniformly to all roles (see [ADR-0100](0100-release-image-supply-chain.md)).
