...
**ファインダーステージ（並行）:** 4 つのコードレンズ（`correctness`、`security`、`architecture`、`runtime-gap`）がそれぞれ独立した `adversarial-reviewer` サブエージェントとして実行される。コメント品質はここでのレンズでは**ない** —— それは `settle-comments` スキルのものであり、同スキルは実装の最後に無条件で走り、専用の `comment-reviewer` サブエージェントを自ら fan-out する。すべてのファインダーが並行実行される。オーケストレーターは、レビュアーエージェントが実装者とは異なるモデルで実行されることを保証する — レビュアーエージェントはデフォルトで `sonnet` であり、セッションモデルが一致する場合はオーケストレーターがオーバーライドする。
