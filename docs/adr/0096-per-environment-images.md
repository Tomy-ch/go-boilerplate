...
# ADR-0096: Per-environment images (.env matrix x APP_ENV build-arg, fixed at build time)
...
build time via `go:embed` (see ADR-0043). Because the embedded file is baked in at compile
...
- Each image is independently verifiable and auditable: the embedded config is part of
  the signed artifact (see [ADR-0098](0098-release-image-supply-chain.md)).
...
- The embedded-config mechanism itself is documented in ADR-0043.
