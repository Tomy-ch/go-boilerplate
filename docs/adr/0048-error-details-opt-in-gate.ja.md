...
# ADR-0048: スキーマ分割によるエラー details の opt-in ゲート

English canonical: [0048-error-details-opt-in-gate.md](0048-error-details-opt-in-gate.md)

## ステータス

accepted

[ADR-0047](0047-error-metadata-code-message-details.ja.md) を精緻化する。

## コンテキスト

[ADR-0047](0047-error-metadata-code-message-details.ja.md) は `apperror.Meta` を導入し、エラー
...
2. **builder 型。** 型生成用エンドポイント(`GenerateErrorSchema`)が `ErrorResponseWithDetails`
   ...
   (builder は ADR-0047 どおり request 非依存のまま)。
