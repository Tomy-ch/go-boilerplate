...
# ADR-0030: マスターデータをマイグレーション経由で投入する。トランザクショナルシードを本番から除外する

English canonical: [0030-master-data-via-migration.md](0030-master-data-via-migration.md)
...
- マスターデータの値を修正する（例：都道府県名のスペルミス）には、[ADR-0028](0028-append-only-immutable-migrations.ja.md)と一致して単純な行更新ではなく新しいマイグレーションファイルが必要である。
...
- 関連: [ADR-0028](0028-append-only-immutable-migrations.ja.md)（不変性）；[ADR-0029](0029-sequential-migration-ids.ja.md)（採番）。
