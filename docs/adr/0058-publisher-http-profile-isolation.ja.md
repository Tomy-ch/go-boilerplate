...
# ADR-0058: パブリッシャーの非標準 HTTP プロファイルをリレー内に隔離する

English canonical: [0058-publisher-http-profile-isolation.md](0058-publisher-http-profile-isolation.md)
...
1. **`MaxAttempts = 1`** — トランスポートレベルのリトライは無効にしなければならない。リレーのポールループが at-least-once リトライ機構であるため（[ADR-0053](0053-at-least-once-outbox-poll.ja.md) 参照）、両方を有効にするとリトライが二重増幅してしまう（決定 D10）。
...
3. **`AllowPrivateNetwork = false`** — 受信エンドポイントは外部サービスである。プライベートアドレスやループバックアドレスへの配信を拒否することで SSRF リスクを抑制する（耐障害性と SSRF クライアントポリシーについては ADR-0022、ADR-0023 を参照）。
...
- 耐障害性と SSRF クライアントポリシー: [ADR-0022](0022-outbound-http-resilience.ja.md)、
  [ADR-0023](0023-egress-ssrf-guard.ja.md)。
- 関連 ADR: [ADR-0053](0053-at-least-once-outbox-poll.ja.md)、[ADR-0059](0059-relay-resident-gc-oneshot.ja.md)。
