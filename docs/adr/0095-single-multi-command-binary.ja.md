...
# ADR-0095: すべてのロールを 1 つのマルチコマンドバイナリに集約する

English canonical: [0095-single-multi-command-binary.md](0095-single-multi-command-binary.md)
...
- バージョン管理、プッシュ、プルのための 1 つのビルド成果物。同じイメージをコマンドオーバーライドで任意のロールとして実行できる（[ADR-0096](0096-single-runtime-image.ja.md) 参照）。
...
- コマンドロジックは [ADR-0094](0094-cli-humble-object-split.ja.md) に従って `internal/cli/<command>/` に置かれる。
