...
# ADR-0087: 多層セキュリティスキャン——報告とゲートを分離し、ハードニングされたランナー上で行う

English canonical: [0087-multi-layer-security-scanning.md](0087-multi-layer-security-scanning.md)
...
| cooldown より新しい依存バージョン | Go cooldown + tool cooldown（[ADR-0089](0089-malicious-package-detection-via-cooldown.ja.md)。pnpm は依存解決の時点で自前の窓を課す） | `go.mod` / `mise.toml` / `python/*.in` の変更 / 週次 | 同一ワークフロー（direct な Go の require とツール宣言） |
...
[ADR-0088](0088-sha-pinned-actions.ja.md) の SHA pin と隔離期間を通す。
...
- ここに挙げたスキャナのうち pre-commit でも走るのは zizmor だけ。フックと CI が呼ぶ `make`
  ...
  （[ADR-0083](0083-local-hooks-mirror-ci.ja.md)）。開発者が
...
- 追加したアクションが通るサプライチェーンの隔離期間とアクション pin:
  [ADR-0088](0088-sha-pinned-actions.ja.md)
- これらのスキャンとは別の関心事であるリリースイメージの完全性:
  [ADR-0099](0099-release-image-supply-chain.ja.md)
- `.golangci-full.yaml` の `gosec` linter は静的解析時のインプロセスチェックとして追加的に機能する
  （[ADR-0082](0082-two-layer-golangci-config.ja.md)）
