...
project's outbox-first design ([ADR-0049](0049-transactional-outbox.md)). A related ambiguity
...
the check and the commit ([ADR-0031](0031-ordered-pessimistic-row-locks.md)). "It spans aggregates
...
1. **Decomposition (default).** Ask the requirements two things: whether the consequence for the
   ...
   [ADR-0049](0049-transactional-outbox.md)), and whether a condition read from another aggregate
...
2. **Guard (synchronous row lock; still a regular usecase).** Does a **guard** need to hold for the
   ...
   order ([ADR-0031](0031-ordered-pessimistic-row-locks.md)); where the rule spans aggregates it
...
- **Why.** Evans's argument is about *change*: the hazard in a wide boundary is mutating several
  ...
  ([ADR-0027](0027-lightweight-cqrs.md) § Derivation). What both branches refuse is the option a
...
- Branch 2 costs throughput on the guarded row: the operation now blocks for the duration of any
  concurrent writer of that row ([ADR-0031](0031-ordered-pessimistic-row-locks.md)).
...
([ADR-0049](0049-transactional-outbox.md)). Rejected.
...
holds — the interleaving [ADR-0031](0031-ordered-pessimistic-row-locks.md) documents. It also makes
...
([ADR-0031](0031-ordered-pessimistic-row-locks.md) § 5), not to a write-optimization construct.
...
[ADR-0031](0031-ordered-pessimistic-row-locks.md) records: the isolation level is a property of every
...
- **Relationship to [ADR-0031](0031-ordered-pessimistic-row-locks.md)**: this ADR decides *whether* a
  cross-aggregate condition needs to be held; ADR-0031 decides *how* it is held — lock order,
...
- Related: [ADR-0028](0028-system-cqrs-dml-category.md) (`system_cqrs` category that carries
  the outbox DML, outside the CQRS split); [ADR-0049](0049-transactional-outbox.md) (the
