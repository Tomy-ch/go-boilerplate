...
# ADR-0085: CI は実際の Postgres に対して実際の fx グラフを起動する（スタートアップ検証）

English canonical: [0085-ci-real-graph-boot-check.md](0085-ci-real-graph-boot-check.md)
...
- 関連: [ADR-0038](0038-uber-fx-di.ja.md) — このチェックが実行する Uber fx DI コンテナのグラフ。
- 関連: [ADR-0048](0048-broker-agnostic-worker-scaffold.ja.md) — エントリポイントが検証されるワーカーサブシステム。
