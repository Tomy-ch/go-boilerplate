...
# ADR-0088: Multi-layer security scanning, splitting reporting from gating, on hardened runners
...
| Dependency versions younger than the cooldown | Go cooldown + tool cooldown ([ADR-0090](0090-malicious-package-detection-via-cooldown.md); pnpm enforces its own at resolution time) | `go.mod` / `mise.toml` / `python/*.in` changes / weekly | same workflow (direct Go requirements and tool declarations) |
...
go through the SHA pinning and quarantine of [ADR-0089](0089-sha-pinned-actions.md).
...
- zizmor is the only scanner here that also runs pre-commit. The hook and CI call different
  ...
  ([ADR-0084](0084-local-hooks-mirror-ci.md)). It earns that
...
- Action pinning and the supply-chain quarantine the added actions go through:
  [ADR-0089](0089-sha-pinned-actions.md).
- Release-image integrity, a separate concern from these scans:
  [ADR-0100](0100-release-image-supply-chain.md).
- The `gosec` linter in `.golangci-full.yaml` provides an additional in-process security
  check during static analysis (see [ADR-0083](0083-two-layer-golangci-config.md)).
