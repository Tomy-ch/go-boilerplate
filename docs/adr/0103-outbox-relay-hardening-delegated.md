...
single transaction ([ADR-0051], [ADR-0052]). Any such design is bounded by an impossibility
...
- **Tx-retry republish**: a serialization-failure/deadlock retry ([ADR-0031]) re-runs the whole
  claim → publish → mark function, re-sending already-delivered messages within one poll. The
  relay is the sole sanctioned exception to ADR-0031's rule that external side effects must live
...
2. **Publish outside the transaction**: claim and mark become short txs; tx dwell, vacuum pinning,
   ...
   effect inside any transaction, the relay satisfies ADR-0031's general rule and its sanctioned
...
- Related ADRs: [ADR-0031] (tx-retry idempotency contract; the shipped relay is its sole
  sanctioned exception, which blueprint layer 2 removes), [ADR-0051] (at-least-once poll),
  [ADR-0052] (SKIP LOCKED claim), [ADR-0053] (message-id / Idempotency-Key propagation),
  [ADR-0054] (dead-lettering after max attempts).
- Full ADR set and ordering: [the ADR log](README.md).

[ADR-0031]: 0031-transaction-retry-idempotent-callers.md
[ADR-0051]: 0051-at-least-once-outbox-poll.md
[ADR-0052]: 0052-skip-locked-outbox-relay.md
[ADR-0053]: 0053-message-id-idempotency-propagation.md
[ADR-0054]: 0054-outbox-dead-after-max-attempts.md
[ADR-0100]: 0100-no-in-app-rate-limiter.md
[ADR-0101]: 0101-scheduled-job-concurrency-delegated.md
