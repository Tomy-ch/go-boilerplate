...
# ADR-0100: すべてのロールを 1 つのマルチコマンドバイナリに集約する
...
- バージョン管理、プッシュ、プルのための 1 つのビルド成果物。同じイメージをコマンドオーバーライドで任意のロールとして実行できる（[ADR-0101](0101-single-runtime-image.ja.md) 参照）。
...
- コマンドロジックは [ADR-0099](0099-cli-humble-object-split.ja.md) に従って `internal/cli/<command>/` に置かれる。
