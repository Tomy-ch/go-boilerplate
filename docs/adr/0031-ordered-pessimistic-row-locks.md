...
# ADR-0031: Serialize contended writes with ordered pessimistic row locks taken before the guarded condition
...
5. **The lock query acquires the row and returns state; it does not author the criterion.** A
   ...
   ([ADR-0027](0027-lightweight-cqrs.md) § Derivation), never as its author.
...
6. **A collision between the caller's own lifecycle state and the requested operation is a
   ...
   ([ADR-0042](0042-apperror-protocol-agnostic-errors.md)). 404 is reserved for hiding the
...
- The invariant holds under concurrency rather than probabilistically: the interleaving above
  ...
  real transactions against the database ([ADR-0087](0087-rollback-integration-tests.md)).
- Deadlock is avoided by construction rather than absorbed by the transaction retry
  ([ADR-0030](0030-transaction-retry-idempotent-callers.md)), so the retry budget stays available
...
outbox relay's `SKIP LOCKED` choice ([ADR-0051](0051-skip-locked-outbox-relay.md)) is a different
