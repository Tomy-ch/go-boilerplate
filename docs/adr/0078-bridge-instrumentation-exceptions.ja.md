...
ライブラリ選定ポリシー（[ADR-0077](0077-library-selection-policy.ja.md)）は、責任が単一の
...
- otel-contrib 系（`otelhttp` / `otelzap`）は**otel-contrib の月次リリーストレイン**で
  ...
  （`github.com/labstack/echo-opentelemetry`、MIT — [ADR-0022](0022-echo-http-framework.ja.md) を参照）、
...
- 親ポリシー: [ADR-0077](0077-library-selection-policy.ja.md)。関連ゲーティング決定: [ADR-0071](0071-config-driven-observability-gating.ja.md)。
