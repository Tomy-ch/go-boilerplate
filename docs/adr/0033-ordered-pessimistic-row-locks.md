...
# ADR-0033: Serialize contended writes with ordered pessimistic row locks taken before the guarded condition
...
5. **The lock query acquires the row and returns state; it does not author the criterion.** A
   ...
   ([ADR-0029](0029-lightweight-cqrs.md) § Derivation), never as its author.
...
6. **A collision between the caller's own lifecycle state and the requested operation is a
   ...
   ([ADR-0044](0044-apperror-protocol-agnostic-errors.md)). 404 is reserved for hiding the
...
- The invariant holds under concurrency rather than probabilistically: the interleaving above
  ...
  real transactions against the database ([ADR-0089](0089-rollback-integration-tests.md)).
- Deadlock is avoided by construction rather than absorbed by the transaction retry
  ([ADR-0032](0032-transaction-retry-idempotent-callers.md)), so the retry budget stays available
...
outbox relay's `SKIP LOCKED` choice ([ADR-0053](0053-skip-locked-outbox-relay.md)) is a different
...
- Related: [ADR-0029](0029-lightweight-cqrs.md) (Repository vs CommandService, and the Derivation
  rule that binds a restated guard), [ADR-0031](0031-commandservice-atomicity-criterion.md) (the
  ...
  serialization),
  [ADR-0032](0032-transaction-retry-idempotent-callers.md) (serialization-failure retry),
  [ADR-0044](0044-apperror-protocol-agnostic-errors.md) (`ErrConflict` → 409),
  [ADR-0053](0053-skip-locked-outbox-relay.md) (the contrasting claim-a-queue-row profile),
  [ADR-0089](0089-rollback-integration-tests.md) (integration tests against a real database).
