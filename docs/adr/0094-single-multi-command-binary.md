...
# ADR-0094: All roles are one multi-command binary
...
- One build artifact to version, push, and pull. The same image can run any role via command
  override (see [ADR-0095](0095-single-runtime-image.md)).
...
- Command logic lives in `internal/cli/<command>/` per [ADR-0093](0093-cli-humble-object-split.md).
