...
# ADR-0102: Delegate outbox-relay duplicate-window hardening to production copies
...
single transaction ([ADR-0050], [ADR-0051]). Any such design is bounded by an impossibility
...
mirroring the reasoning of [ADR-0099] and [ADR-0100].
...
`Idempotency-Key` ([ADR-0052]) and the bundled idempotency middleware deduplicates on the receiving
...
- Related ADRs: [ADR-0030] (tx-retry idempotency contract; the shipped relay is its sole
  sanctioned exception, which blueprint layer 2 removes), [ADR-0050] (at-least-once poll),
  [ADR-0051] (SKIP LOCKED claim), [ADR-0052] (message-id / Idempotency-Key propagation),
  [ADR-0053] (dead-lettering after max attempts).
- Full ADR set and ordering: [the ADR log](README.md).

[ADR-0030]: 0030-transaction-retry-idempotent-callers.md
[ADR-0050]: 0050-at-least-once-outbox-poll.md
[ADR-0051]: 0051-skip-locked-outbox-relay.md
[ADR-0052]: 0052-message-id-idempotency-propagation.md
[ADR-0053]: 0053-outbox-dead-after-max-attempts.md
[ADR-0099]: 0099-no-in-app-rate-limiter.md
[ADR-0100]: 0100-scheduled-job-concurrency-delegated.md
