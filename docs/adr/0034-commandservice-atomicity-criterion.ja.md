...
- **クーポンの引き換え — 同じ集約に対する、分解できる書き込み。** 購入確定でクーポンを 1 枚適用する
  操作は `purchases` と `coupons` の両方を書くが、Repository 呼び出しで構成する通常のユースケースに
  留まる。書く行が識別子で名指しできるためで、呼び出し側がクーポン ID を渡す以上その行はロックでき、
  書き込みは `coupon.Repository.LockByID` + `coupon.Repository.UpdateUsed` と
  `purchase.Repository.Create` へ分解できる。上の判定手順における「ガード付きの分岐 1」である。
