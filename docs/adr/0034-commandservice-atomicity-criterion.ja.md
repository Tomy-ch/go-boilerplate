...
（[ADR-0054](0054-transactional-outbox.ja.md)）と矛盾する。関連する曖昧さは ADR-0032
...
- **購入作成 — 分岐 2 と分岐 3 の同時適用。** 在庫の検証と減算は購入確定と原子的で
  ...
  [ADR-0054](0054-transactional-outbox.ja.md) のとおり同一トランザクションに参加する。
...
（[ADR-0054](0054-transactional-outbox.ja.md)）と矛盾する。却下。
...
- 関連: [ADR-0033](0033-system-cqrs-dml-category.ja.md)（CQRS 分割の外側で outbox の
  ...
  [ADR-0054](0054-transactional-outbox.ja.md)（デフォルトの分解経路が依存する
  outbox パターン）、
