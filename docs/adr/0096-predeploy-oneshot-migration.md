...
# ADR-0096: Migrations run as a pre-deploy one-shot; do NOT auto-migrate at application startup
...
[ADR-0093](0093-single-runtime-image.md)) precisely to support the pre-deploy pattern via
command override.
...
achieves the same result without the overhead (see [ADR-0093](0093-single-runtime-image.md)).
