...
# ADR-0052: Measure broker-SDK isolation as coupling, not as linkage

## Status

accepted (supersedes [ADR-0051](0051-sqs-adapter-opt-in.md))

## Context

[ADR-0051](0051-sqs-adapter-opt-in.md) kept the SQS reference adapter out of the default `cmd`
...
ADR-0051 was accepted. `github.com/aws/aws-sdk-go-v2/service/s3` — and with it the SDK core
(`aws`, `credentials`, `signer/v4`, `retry`, `transport/http`) and `smithy-go` — is now linked into
the default binary. ADR-0051's context ("every binary — including `serve`, which never consumes a
...
The concern behind ADR-0051 is better stated as **coupling**, not linkage: the repository must not
...
form stand literally. Rejected for now: ADR-0051 already rejected build tags for this purpose, and a binary
...
- Supersedes [ADR-0051](0051-sqs-adapter-opt-in.md). Parent decision:
  [ADR-0049](0049-broker-agnostic-worker-scaffold.md). Principle: [ADR-0001](0001-avoid-lock-in.md).
- [ADR-0050](0050-out-of-scope-push-streaming-brokers.md) is unaffected: push-type and
