...
# ADR-0089: Accept a publication cooldown as the primary defence against malicious packages, without a dedicated detector
...
| --- | --- | --- |
| Node packages | 7 days | `minimumReleaseAge` in each `pnpm-workspace.yaml`, refused at resolution time ([ADR-0106](0106-pnpm-as-the-only-node-resolver.md)) |
...
| CLI tools installed from PyPI | 7 days | `scripts/tool-cooldown`, reading the `python/*.in` declarations ([ADR-0078](0078-mise-ssot-drift-gate.md)) |
...
- Related: [ADR-0088](0088-sha-pinned-actions.md) (SHA pinning),
  [ADR-0087](0087-multi-layer-security-scanning.md) (the scanning layers this sits beside),
