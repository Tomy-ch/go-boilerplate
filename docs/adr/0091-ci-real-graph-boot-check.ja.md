...
# ADR-0091: CI は実際の Postgres に対して実際の fx グラフを起動する（スタートアップ検証）
...
プロジェクトには 4 つの独立したエントリポイントがある——HTTP サーバー、バックグラウンドワーカー、単発ジョブ、
outbox relay——それぞれに独自の DI グラフを持つ。アプリケーションコードに触れるすべてのプルリクエストで
4 つすべてを検証しなければならない。
...
ディスパッチロジックに到達したことを検証する、エントリポイントごとに 1 つの専用 CI ワークフロージョブを
導入する:
...
**Outbox relay 起動チェック**（`outbox-relay-boot-check.yaml`）:
relay はデーモンであり、未知の名前を渡して弾かれることを確認する経路を持たない。代わりに
`outbox-relay --channel=realtime` を起動し、その出力を fx event logger の `"Application started"`
が現れるまでポーリングしてから SIGTERM を送り、`"Application stopped"` とゼロ終了を要求する。起動できる
ことと drain できることの両方が契約の一部であり、起動はするが停止できない relay はこのチェックに失敗する。
対象を `http` チャネルではなく `realtime` チャネルとするのは、CI で結線されているのがこちらだからである。
`http` チャネルは `ENDPOINT_OUTBOX` を必要とするが、`env/.env.ci` は意図的にこれを空にしている。

4 つのジョブはすべて `services:` ブロック経由で実際の Postgres 18 コンテナを起動し、実行前に
...
単一のジョブがすべてのエントリポイントを順番に起動することもできる。一方の失敗が他方をマスクしないように、
また CI レポートが明確に帰属されるように、エントリポイントごとに 1 つの独立したジョブを選択した。同じ理由
から、新しいエントリポイントのチェックは既存のワークフローファイル内の追加ジョブではなく、それぞれ独自の
ワークフローファイルとして持たせている。
...
- relay チェックのセンチネルは `internal/di/fx_event_logger.go` に由来する。これらのログメッセージを
  リネームすると、このチェックはプロセスが生きていることしか証明しない弱いチェックへ静かに劣化する。
- 関連: [ADR-0040](0040-uber-fx-di.ja.md) — このチェックが実行する Uber fx DI コンテナのグラフ。
- 関連: [ADR-0050](0050-broker-agnostic-worker-scaffold.ja.md) — エントリポイントが検証されるワーカーサブシステム。
