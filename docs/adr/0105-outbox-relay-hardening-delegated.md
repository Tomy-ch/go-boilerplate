...
# ADR-0105: Ship a balanced outbox relay; delegate hardening to operational evidence
...
single transaction ([ADR-0053], [ADR-0054]). Any such design is bounded by an impossibility
...
- **Tx-retry republish**: a serialization-failure/deadlock retry ([ADR-0033]) re-runs the whole
  claim → publish → mark function, re-sending already-delivered messages within one poll. The
  relay is the sole sanctioned exception to ADR-0033's rule that external side effects must live
...
than by structural exclusion: `message_id` propagates as `Idempotency-Key` ([ADR-0055]) and the
...
2. **Publish outside the transaction**: claim and mark become short txs; tx dwell, vacuum pinning,
   ...
   effect inside any transaction, the relay satisfies ADR-0033's general rule and its sanctioned
...
- Related ADRs: [ADR-0033] (tx-retry idempotency contract; the shipped relay is its sole
  sanctioned exception, which blueprint layer 2 removes), [ADR-0053] (at-least-once poll),
  [ADR-0054] (SKIP LOCKED claim), [ADR-0055] (message-id / Idempotency-Key propagation),
  [ADR-0056] (dead-lettering after max attempts).
- Full ADR set and ordering: [the ADR log](README.md).

[ADR-0033]: 0033-transaction-retry-idempotent-callers.md
[ADR-0053]: 0053-at-least-once-outbox-poll.md
[ADR-0054]: 0054-skip-locked-outbox-relay.md
[ADR-0055]: 0055-message-id-idempotency-propagation.md
[ADR-0056]: 0056-outbox-dead-after-max-attempts.md
[ADR-0102]: 0102-no-in-app-rate-limiter.md
[ADR-0103]: 0103-scheduled-job-concurrency-delegated.md
