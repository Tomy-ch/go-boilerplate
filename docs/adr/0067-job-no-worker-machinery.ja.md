...
# ADR-0067: ジョブにはブローカー・サーキットブレーカー・ドレイン・ヘルス機構を意図的に設けない

English canonical: [0067-job-no-worker-machinery.md](0067-job-no-worker-machinery.md)
...
ジョブサブシステムとワーカーサブシステムはどちらもユースケースレイヤーへの非同期エントリポイントであり、同じ Fx DI コンテナパターンを共有する。ワーカーエンジン（[ADR-0048](0048-broker-agnostic-worker-scaffold.ja.md)）は豊富な運用レイヤーを提供する。インテーク側にプルアック型ブローカーシーム、3 状態サーキットブレーカー、シャットダウン時のドレインフェーズ（インフライトメッセージの待機）、そして HTTP ヘルスリスナー（`/healthz`、`/readyz`）である。
...
- [ADR-0048](0048-broker-agnostic-worker-scaffold.ja.md)（ワーカー: ブローカーシーム、サーキットブレーカー、ドレイン、ヘルスリスナー）との対比。
