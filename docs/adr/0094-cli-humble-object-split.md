...
# ADR-0094: CLI humble-object split (thin cmd/ shell + testable internal/cli core)
...
The coverage gate ([ADR-0085](0085-coverage-hard-gate.md), 90%+ branch coverage) must apply to command logic, but `cmd/` shell files
...
with the single-binary decision (see [ADR-0095](0095-single-multi-command-binary.md)).
