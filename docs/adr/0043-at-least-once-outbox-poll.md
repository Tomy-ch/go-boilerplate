...
# ADR-0043: At-least-once delivery via polling (transport-level retry disabled)
...
- `attempts` and `last_error` provide direct, unambiguous observability of what failed
  and how many times.
- Eventual dead-lettering (see [ADR-0046](0046-outbox-dead-after-max-attempts.md)) is
...
- At-least-once semantics mean the receiver must be idempotent on the `Idempotency-Key`
  header (see [ADR-0045](0045-message-id-idempotency-propagation.md)).
...
- Related ADRs: [ADR-0042](0042-transactional-outbox.md), [ADR-0044](0044-skip-locked-outbox-relay.md),
