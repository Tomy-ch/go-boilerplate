...
# ADR-0050: Measure broker-SDK isolation as coupling, not as linkage

## Status

accepted (supersedes [ADR-0049](0049-sqs-adapter-opt-in.md))

## Context

[ADR-0049](0049-sqs-adapter-opt-in.md) kept the SQS reference adapter out of the default `cmd`
...
ADR-0049 was accepted. `github.com/aws/aws-sdk-go-v2/service/s3` — and with it the SDK core
(`aws`, `credentials`, `signer/v4`, `retry`, `transport/http`) and `smithy-go` — is now linked into
the default binary. ADR-0049's context ("every binary — including `serve`, which never consumes a
...
The concern behind ADR-0049 is better stated as **coupling**, not linkage: the repository must not
...
form stand literally. Rejected for now: ADR-0049 already rejected build tags for this purpose, and a binary
...
- Supersedes [ADR-0049](0049-sqs-adapter-opt-in.md). Parent decision:
  [ADR-0047](0047-broker-agnostic-worker-scaffold.md). Principle: [ADR-0001](0001-avoid-lock-in.md).
- [ADR-0048](0048-out-of-scope-push-streaming-brokers.md) is unaffected: push-type and
