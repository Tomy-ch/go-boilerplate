...
# ADR-0017: OpenAPI is the wire contract, not the domain rule; request is subset of domain, domain is subset of response
...
- The request constraint may be *stricter* than the domain rule. The request-validation
  middleware (see [ADR-0015](0015-spec-driven-request-validation.md)) rejects out-of-range
...
[ADR-0011](0011-openapi-first.md) OpenAPI-first principle (the spec is authored first, not
...
- The request validation that enforces the wire-contract side is described in
  [ADR-0015](0015-spec-driven-request-validation.md).
- Builds on [ADR-0016](0016-validation-value-authority.md): the domain is the authority for
