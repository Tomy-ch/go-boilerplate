...
# ADR-0051: SQS アダプターはオプトインであり、デフォルトバイナリにリンクしない

English canonical: [0051-sqs-adapter-opt-in.md](0051-sqs-adapter-opt-in.md)

## ステータス

superseded by [ADR-0052](0052-broker-sdk-isolation-measured-as-coupling.ja.md)

## 背景

ワーカースキャフォールド（[ADR-0049](0049-broker-agnostic-worker-scaffold.ja.md)）は AWS SQS 向けのリファレンスブローカーアダプターを同梱している。このアダプターをデフォルトビルドに組み込むと、キューを消費しない `serve` を含むすべてのバイナリが `aws-sdk-go-v2` をリンクし、ロックイン回避原則（[ADR-0001](0001-avoid-lock-in.ja.md)）に反して依存性サーフェスが拡大する。
...
- 親決定: [ADR-0049](0049-broker-agnostic-worker-scaffold.ja.md)。原則: [ADR-0001](0001-avoid-lock-in.ja.md)。
