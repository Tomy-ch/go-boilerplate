...
# ADR-0090: 悪意あるパッケージへの主防御として専用の検出器を持たず公開クールダウンを受け入れる

English canonical: [0090-malicious-package-detection-via-cooldown.md](0090-malicious-package-detection-via-cooldown.md)
...
| 対象 | 窓 | 機構 |
| --- | --- | --- |
| Node パッケージ | 7 日 | 各 `pnpm-workspace.yaml` の `minimumReleaseAge`。依存解決の時点で拒否される（[ADR-0107](0107-pnpm-as-the-only-node-resolver.ja.md)） |
...
| PyPI から入れる CLI ツール | 7 日 | `scripts/tool-cooldown` が `python/*.in` の宣言を読む（[ADR-0079](0079-mise-ssot-drift-gate.ja.md)） |
...
- 関連: [ADR-0089](0089-sha-pinned-actions.ja.md)（SHA ピン留め）、[ADR-0088](0088-multi-layer-security-scanning.ja.md)（これが隣り合うスキャンのレイヤー群）、[ADR-0001](0001-avoid-lock-in.ja.md)（SaaS 検出器が対象外である理由）。
