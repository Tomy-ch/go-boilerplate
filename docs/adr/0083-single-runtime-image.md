...
# ADR-0083: A single runtime image with command override (no purpose-specific images)
...
[ADR-0082](0082-single-multi-command-binary.md)), deployments require not only the HTTP
...
- One image digest to sign, attest, and verify per deployment. Supply-chain integrity
  checks apply uniformly to all roles (see [ADR-0087](0087-release-image-supply-chain.md)).
