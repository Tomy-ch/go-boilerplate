...
# ADR-0098: Migrations run as a pre-deploy one-shot; do NOT auto-migrate at application startup
...
[ADR-0095](0095-single-runtime-image.md)) precisely to support the pre-deploy pattern via
command override.
...
achieves the same result without the overhead (see [ADR-0095](0095-single-runtime-image.md)).
