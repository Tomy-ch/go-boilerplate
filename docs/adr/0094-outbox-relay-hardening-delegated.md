...
# ADR-0094: Delegate outbox-relay duplicate-window hardening to production copies
...
single transaction ([ADR-0043], [ADR-0044]). Any such design is bounded by an impossibility
...
- **Tx-retry republish**: a serialization-failure/deadlock retry ([ADR-0029]) re-runs the whole
  claim → publish → mark function, re-sending already-delivered messages within one poll. The
  relay is the sole sanctioned exception to ADR-0029's rule that external side effects must live
...
mirroring the reasoning of [ADR-0091] and [ADR-0092].
...
`Idempotency-Key` ([ADR-0045]) and the bundled idempotency middleware deduplicates on the receiving
...
2. **Publish outside the transaction**: claim and mark become short txs; tx dwell, vacuum pinning,
   ...
   effect inside any transaction, the relay satisfies ADR-0029's general rule and its sanctioned
...
- Related ADRs: [ADR-0029] (tx-retry idempotency contract; the shipped relay is its sole
  sanctioned exception, which blueprint layer 2 removes), [ADR-0043] (at-least-once poll),
  [ADR-0044] (SKIP LOCKED claim), [ADR-0045] (message-id / Idempotency-Key propagation),
  [ADR-0046] (dead-lettering after max attempts).
- Full ADR set and ordering: [the ADR log](README.md).

[ADR-0029]: 0029-transaction-retry-idempotent-callers.md
[ADR-0043]: 0043-at-least-once-outbox-poll.md
[ADR-0044]: 0044-skip-locked-outbox-relay.md
[ADR-0045]: 0045-message-id-idempotency-propagation.md
[ADR-0046]: 0046-outbox-dead-after-max-attempts.md
[ADR-0091]: 0091-no-in-app-rate-limiter.md
[ADR-0092]: 0092-scheduled-job-concurrency-delegated.md
