...
# ADR-0064: 決定論的リプレイを可能にするためレスポンスボディを JSON で永続化する（PII トレードオフを許容）

English canonical: [0064-idempotency-response-persistence.md](0064-idempotency-response-persistence.md)
...
- PII 保持期間を制限する 24 時間 TTL は ADR-0063 で決定されている。
