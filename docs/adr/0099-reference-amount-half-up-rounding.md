...
# ADR-0099: reference_amount is computed in integers with half-up rounding at a single point
...
`GET /v1/exchange-rates` returns, when `display_currency` is requested, a `reference_amount`
— a display-currency (JPY) figure derived from the base amount and the fetched rate. Money
must not accumulate `float` error, so the internal computation for `reference_amount` must be
...
The `reference_amount.amount` field (a JPY-yen integer) is computed with **half-up rounding
...
the reference_amount computation path only**, not to this input multiplier or to the generic
`converted` output.

Half-up is chosen because `reference_amount` is an advisory, non-persistent display figure not
...
reference_amount integerization, and later `purchases` bring their own stored USD-cent value
...
Weighed for accounting neutrality, but `reference_amount` is advisory and non-persistent, so
