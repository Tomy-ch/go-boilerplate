...
# ADR-0098: Per-environment images (.env matrix x APP_ENV build-arg, fixed at build time)
...
build time via `go:embed` (see ADR-0045). Because the embedded file is baked in at compile
...
- Each image is independently verifiable and auditable: the embedded config is part of
  the signed artifact (see [ADR-0100](0100-release-image-supply-chain.md)).
...
- The embedded-config mechanism itself is documented in ADR-0045.
