...
# ADR-0104: Cross-aggregate value objects live in a curated domain shared kernel
...
Introducing `money.Price` ([ADR-0101](0101-two-scale-money-model.md)) surfaced a gap in the
...
Cross-aggregate, business-semantic value objects live in a **curated domain shared kernel** at
`internal/domain/kernel/` (DDD *Shared Kernel*). Other domain packages **may** import the kernel;
the kernel itself depends only on `pkg/**` and `internal/apperror`, never on an aggregate.

Admission to `kernel/` is deliberately narrow — it is a shared kernel, **not** a `shared` /
`common` / `util` junk drawer. A type belongs there only when **all** hold:
...
- adding it is a **jointly-owned** decision — a kernel change ripples to every aggregate that
...
for domain files, with an explicit allow for `internal/domain/kernel`. So domain→kernel is
...
- The path itself signals intent: `internal/domain/kernel/money` reads as "shared, importable",
  so a cross-aggregate import no longer looks like a violation (the review confusion that
  prompted this ADR).
- depguard now both **permits** the shared kernel and **forbids** ad-hoc aggregate-to-aggregate
...
- A shared kernel is a coupling point: a change to `kernel/money` can affect every dependent
  aggregate, so it must be evolved conservatively (this is the cost the admission bar manages).
- One more placement concept for contributors to learn (aggregate vs. kernel), documented here,
  in `docs/rules.md`, and in `internal/domain/kernel/README.md`.
...
Rejected: a name whose only admission bar is "used in more than one place" attracts unrelated
code and becomes a junk drawer. `kernel` names the DDD Shared Kernel — a deliberately minimal,
jointly-owned core with the strict admission criterion above.
...
[ADR-0102](0102-exact-decimal-pkg-wrap.md)) belongs in `pkg/`.

## Notes

- Kernel package: `internal/domain/kernel/money` (`Price`).
- Enforcement: depguard `maintain_a_sound_domain` in `.golangci-full.yaml` (deny
  `internal/domain/`, allow `internal/domain/kernel`).
- Admission bar: `internal/domain/kernel/README.md`; layer rule: `docs/rules.md`.
- Two-scale money model this enables: [ADR-0101](0101-two-scale-money-model.md).
