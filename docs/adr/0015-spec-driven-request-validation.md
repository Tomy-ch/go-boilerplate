...
# ADR-0015: Validate requests and enforce auth from the spec at runtime; do not validate responses
...
[ADR-0011](0011-openapi-first.md) makes the OpenAPI spec the single source of truth for
...
kept in sync through code generation (see [ADR-0013](0013-oapi-codegen-strict-server.md)),
unnecessary.
...
construction: because handler code is generated from the same spec (ADR-0013), a
