...
# ADR-0041: Opt-in gate for error-response details via schema split

## Status

accepted

Refines [ADR-0040](0040-error-metadata-code-message-details.md).

## Context

[ADR-0040](0040-error-metadata-code-message-details.md) introduced `apperror.Meta`, letting
...
2. **Builder type.** The internal type-generation endpoint (`GenerateErrorSchema`) references
   ...
   downstream, not by the builder (which stays request-agnostic, per ADR-0040).
