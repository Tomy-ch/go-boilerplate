...
# ADR-0052: At-least-once delivery via polling (transport-level retry disabled)
...
- `attempts` and `last_error` provide direct, unambiguous observability of what failed
  and how many times.
- Eventual dead-lettering (see [ADR-0055](0055-outbox-dead-after-max-attempts.md)) is
...
- At-least-once semantics mean the receiver must be idempotent on the `Idempotency-Key`
  header (see [ADR-0054](0054-message-id-idempotency-propagation.md)).
...
- Related ADRs: [ADR-0051](0051-transactional-outbox.md), [ADR-0053](0053-skip-locked-outbox-relay.md),
