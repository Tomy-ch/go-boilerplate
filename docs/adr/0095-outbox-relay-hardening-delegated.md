...
# ADR-0095: Delegate outbox-relay duplicate-window hardening to production copies
...
single transaction ([ADR-0044], [ADR-0045]). Any such design is bounded by an impossibility
...
mirroring the reasoning of [ADR-0092] and [ADR-0093].
...
`Idempotency-Key` ([ADR-0046]) and the bundled idempotency middleware deduplicates on the receiving
...
- Related ADRs: [ADR-0029] (tx-retry idempotency contract; the shipped relay is its sole
  sanctioned exception, which blueprint layer 2 removes), [ADR-0044] (at-least-once poll),
  [ADR-0045] (SKIP LOCKED claim), [ADR-0046] (message-id / Idempotency-Key propagation),
  [ADR-0047] (dead-lettering after max attempts).
- Full ADR set and ordering: [the ADR log](README.md).

[ADR-0029]: 0029-transaction-retry-idempotent-callers.md
[ADR-0044]: 0044-at-least-once-outbox-poll.md
[ADR-0045]: 0045-skip-locked-outbox-relay.md
[ADR-0046]: 0046-message-id-idempotency-propagation.md
[ADR-0047]: 0047-outbox-dead-after-max-attempts.md
[ADR-0092]: 0092-no-in-app-rate-limiter.md
[ADR-0093]: 0093-scheduled-job-concurrency-delegated.md
