...
# ADR-0082: All roles are one multi-command binary
...
- One build artifact to version, push, and pull. The same image can run any role via command
  override (see [ADR-0083](0083-single-runtime-image.md)).
...
- Command logic lives in `internal/cli/<command>/` per [ADR-0081](0081-cli-humble-object-split.md).
