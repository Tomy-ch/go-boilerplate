...
# ADR-0106: outbox relay はバランス型で出荷し、ハードニングは運用で得た事実に委ねる

English canonical: [0106-outbox-relay-hardening-delegated.md](0106-outbox-relay-hardening-delegated.md)
...
mark する（[ADR-0054]、[ADR-0055]）。この種の設計はすべて不可能性の結果に縛られる: 外部副作用
...
ために可用性とコストを払うおそれがある — [ADR-0103] と [ADR-0104] と同じ論法である。
...
`Idempotency-Key` として伝搬（[ADR-0056]）し、受信側の同梱冪等性ミドルウェアが dedup するので、
...
2. **publish の tx 外化**: claim と mark は短 tx になる。tx 滞留・vacuum ピン留め・
   ...
   含まれなくなるため、relay は ADR-0034 の一般則を満たし、公認例外そのものが消滅する。
...
- 関連 ADR: [ADR-0034]（tx リトライ冪等性契約。出荷状態の relay はその唯一の公認例外であり、
  設計図の第 2 層がこの例外を除去する）、[ADR-0054]（at-least-once poll）、[ADR-0055]
  （SKIP LOCKED claim）、[ADR-0056]（message-id / Idempotency-Key 伝搬）、[ADR-0057]
  （max attempts 到達での dead 化）。
- ADR 全体の一覧と順序: [ADR ログ](README.ja.md)。

[ADR-0034]: 0034-transaction-retry-idempotent-callers.ja.md
[ADR-0054]: 0054-at-least-once-outbox-poll.ja.md
[ADR-0055]: 0055-skip-locked-outbox-relay.ja.md
[ADR-0056]: 0056-message-id-idempotency-propagation.ja.md
[ADR-0057]: 0057-outbox-dead-after-max-attempts.ja.md
[ADR-0103]: 0103-no-in-app-rate-limiter.ja.md
[ADR-0104]: 0104-scheduled-job-concurrency-delegated.ja.md
