...
# ADR-0087: ローカル開発環境はプロファイルで分離されたサービスを持つ Docker Compose で提供する
...
ツール実行はマシン間で再現可能でなければならない（[ADR-0083](0083-containerized-pinned-toolchain.ja.md) 参照）。
...
- コンテナベース再現性の根拠:
  [ADR-0083](0083-containerized-pinned-toolchain.ja.md)。
