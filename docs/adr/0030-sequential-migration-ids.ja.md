...
いずれかのチェックに違反するPRはCIが失敗しマージできない。このナンバリング規律が支援する不変性ルールについては[ADR-0029](0029-append-only-immutable-migrations.ja.md)参照。
...
- 関連: [ADR-0029](0029-append-only-immutable-migrations.ja.md)（不変性）。
