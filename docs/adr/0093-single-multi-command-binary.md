...
# ADR-0093: All roles are one multi-command binary
...
- One build artifact to version, push, and pull. The same image can run any role via command
  override (see [ADR-0094](0094-single-runtime-image.md)).
...
- Command logic lives in `internal/cli/<command>/` per [ADR-0092](0092-cli-humble-object-split.md).
