...
# ADR-0090: Accept a publication cooldown as the primary defence against malicious packages, without a dedicated detector
...
| --- | --- | --- |
| Node packages | 7 days | `minimumReleaseAge` in each `pnpm-workspace.yaml`, refused at resolution time ([ADR-0107](0107-pnpm-as-the-only-node-resolver.md)) |
...
| CLI tools installed from PyPI | 7 days | `scripts/tool-cooldown`, reading the `python/*.in` declarations ([ADR-0079](0079-mise-ssot-drift-gate.md)) |
...
- Related: [ADR-0089](0089-sha-pinned-actions.md) (SHA pinning),
  [ADR-0088](0088-multi-layer-security-scanning.md) (the scanning layers this sits beside),
