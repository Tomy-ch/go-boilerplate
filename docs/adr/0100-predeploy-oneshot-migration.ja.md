...
`runtime` イメージはコマンドオーバーライードを介してデプロイ前パターンをサポートするために、完全な `migrate-up` サブコマンドをシップする（[ADR-0097](0097-single-runtime-image.ja.md) 参照）。
...
第 2 の成果物の管理が必要となる。シングルイメージのコマンドオーバーライードアプローチがオーバーヘッドなしに同じ結果を達成する（[ADR-0097](0097-single-runtime-image.ja.md) 参照）。
