...
# ADR-0032: Introduce system_cqrs as a fourth DML category outside the CQRS split
...
[ADR-0031](0031-lightweight-cqrs.md). It serves infrastructure concerns, not application
business logic.

`make gen-query` processes all four categories through the same merge-dml and sqlc pipeline
(see [ADR-0027](0027-merged-dml-schema-as-sqlc-input.md)), so system_cqrs participates
...
- Related: [ADR-0031](0031-lightweight-cqrs.md) (CQRS split that system_cqrs falls
  outside of); [ADR-0027](0027-merged-dml-schema-as-sqlc-input.md) (merge-dml pipeline).
