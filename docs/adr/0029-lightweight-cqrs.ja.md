...
`persistenceModule` に登録され、Uber Fx で注入される（[ADR-0037](0037-uber-fx-di.ja.md)を参照）。
...
fail-closed な二重の網として言い直したものである（[ADR-0033](0033-ordered-pessimistic-row-locks.ja.md)）。
...
- 関連: [ADR-0030](0030-system-cqrs-dml-category.ja.md)（CQRS の分割の外に位置する第 4 の
