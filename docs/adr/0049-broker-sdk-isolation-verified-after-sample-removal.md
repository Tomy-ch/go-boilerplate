...
# ADR-0049: Broker-SDK isolation is verified after sample removal, not by leaving the adapter unwired

## Status

accepted (supersedes [ADR-0048](0048-sqs-adapter-opt-in.md))

## Context

[ADR-0048](0048-sqs-adapter-opt-in.md) kept the SQS reference adapter out of the default `cmd`
...
ADR-0048 was accepted. `github.com/aws/aws-sdk-go-v2/service/s3` — and with it the SDK core
(`aws`, `credentials`, `signer/v4`, `retry`, `transport/http`) and `smithy-go` — is now linked into
the default binary. ADR-0048's context ("every binary — including `serve`, which never consumes a
...
The concern behind ADR-0048 is better stated as **coupling**, not linkage: the repository must not
...
literally. Rejected for now: ADR-0048 already rejected build tags for this purpose, and a binary
...
- Supersedes [ADR-0048](0048-sqs-adapter-opt-in.md). Parent decision:
  [ADR-0046](0046-broker-agnostic-worker-scaffold.md). Principle: [ADR-0001](0001-avoid-lock-in.md).
- [ADR-0047](0047-out-of-scope-push-streaming-brokers.md) is unaffected: push-type and
