...
# ADR-0033: Reserve CommandService for multi-aggregate writes that require single-transaction atomicity
...
[ADR-0031](0031-lightweight-cqrs.md) defines the *structure* of the CommandService
...
returns DTOs, and it is registered in `persistenceModule` and injected via DI. What ADR-0031
...
project's outbox-first design ([ADR-0053](0053-transactional-outbox.md)). A related ambiguity
exists in ADR-0031 itself: its consequences note that CommandService "can freely optimize
...
the check and the commit ([ADR-0035](0035-ordered-pessimistic-row-locks.md)). "It spans aggregates
...
- **Why.** Evans's argument is about *change*: the hazard in a wide boundary is mutating several
  ...
  ([ADR-0031](0031-lightweight-cqrs.md) § Derivation). What both branches refuse is the option a
...
- **Purchase creation — branch 2 and branch 3 together.** Stock validation and decrement must be
  ...
  that same transaction as usual per [ADR-0053](0053-transactional-outbox.md). The outbox insert is
...
- Branch 2 costs throughput on the guarded row: the operation now blocks for the duration of any
  concurrent writer of that row ([ADR-0035](0035-ordered-pessimistic-row-locks.md)).
...
([ADR-0053](0053-transactional-outbox.md)). Rejected.
...
holds — the interleaving [ADR-0035](0035-ordered-pessimistic-row-locks.md) documents. It also makes
...
([ADR-0035](0035-ordered-pessimistic-row-locks.md) § 5), not to a write-optimization construct.
...
[ADR-0035](0035-ordered-pessimistic-row-locks.md) records: the isolation level is a property of every
...
- **Relationship to [ADR-0031](0031-lightweight-cqrs.md)**: complementary, not conflicting.
  ADR-0031 defines the construct's structure (interface placement, implementation directory,
  DTO returns, DI registration); this ADR supplies the placement criterion ADR-0031 left
  open. One clarification: ADR-0031's consequence that CommandService "can freely optimize
  ...
  governs placement going forward. ADR-0031 is not edited or superseded.
- **Relationship to [ADR-0035](0035-ordered-pessimistic-row-locks.md)**: this ADR decides *whether* a
  cross-aggregate condition needs to be held; ADR-0035 decides *how* it is held — lock order,
...
- Related: [ADR-0032](0032-system-cqrs-dml-category.md) (`system_cqrs` category that carries
  the outbox DML, outside the CQRS split); [ADR-0053](0053-transactional-outbox.md) (the
  outbox pattern that the default decomposition path relies on);
  [ADR-0034](0034-transaction-retry-idempotent-callers.md) (retry semantics of the single
