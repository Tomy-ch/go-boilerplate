...
# ADR-0025: sqlcによる型安全なSQLアクセスの生成

English canonical: [0025-sqlc-type-safe-sql.md](0025-sqlc-type-safe-sql.md)
...
SQLファーストの方針（[ADR-0024](0024-sql-first-data-access.ja.md)）を前提とすると、手書きのSQLをGoに**コンパイル時の型安全な**コードとして届ける必要があるが、ORMのランタイム抽象や暗黙的なクエリ生成を再導入したくない。
...
- [ADR-0024](0024-sql-first-data-access.ja.md)（SQLファースト）を前提とする。
