...
ジョブサブシステムとワーカーサブシステムはどちらもユースケースレイヤーへの非同期エントリポイントであり、同じ Fx DI コンテナパターンを共有する。ワーカーエンジン（[ADR-0050](0050-broker-agnostic-worker-scaffold.ja.md)）は豊富な運用レイヤーを提供する。インテーク側にプルアック型ブローカーシーム、3 状態サーキットブレーカー、シャットダウン時のドレインフェーズ（インフライトメッセージの待機）、そして HTTP ヘルスリスナー（`/healthz`、`/readyz`）である。
...
- [ADR-0050](0050-broker-agnostic-worker-scaffold.ja.md)（ワーカー: ブローカーシーム、サーキットブレーカー、ドレイン、ヘルスリスナー）との対比。
