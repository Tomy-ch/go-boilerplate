...
# ADR-0049: ブローカー非依存のプル・アック型ワーカースキャフォールド

English canonical: [0049-broker-agnostic-worker-scaffold.md](0049-broker-agnostic-worker-scaffold.md)
...
- ポートは意図的に狭い（プル・アック型のみ）。プッシュ型とストリーミングログモデルは適合しないため除外されている（ADR-0050 として別途記録、Phase 4 で具体化）。
...
却下: このリポジトリに前例がなく、シングルバイナリではモジュール分離が不十分。`cmd` からアダプターをインポートしないことで、タグなしで分離が達成できる（[ADR-0051](0051-sqs-adapter-opt-in.ja.md) を参照）。

## 補足

- プッシュ/ストリーミング除外: ADR-0050（Phase 4）。SQS アダプターのオプトイン分離: [ADR-0051](0051-sqs-adapter-opt-in.ja.md)。
