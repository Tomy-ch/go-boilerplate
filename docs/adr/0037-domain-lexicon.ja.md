...
# ADR-0037: 集約横断の値オブジェクトはキュレートされたドメイン lexicon に置く

English canonical: [0037-domain-lexicon.md](0037-domain-lexicon.md)
...
`money.Price`（[ADR-0036](0036-two-scale-quantity-model.ja.md)）の導入により、レイヤールールの欠落が表面化した。`Price` はビジネス上の意味を持つ値オブジェクト（非負性、最小単位への換算）であり、**複数の集約**で共有される。
...
却下: `pkg/` はビジネスロジックを禁じ、コンテキスト非依存でなければならない。通貨 / 非負性 / 最小単位はドメインのセマンティクスである。`pkg/` に属するのは汎用の decimal コンテナ（`pkg/decimal`、[ADR-0036](0036-two-scale-quantity-model.ja.md)）だけである。
...
- これが可能にする 2 スケール数量モデル: [ADR-0036](0036-two-scale-quantity-model.ja.md)。
