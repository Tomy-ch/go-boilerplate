...
# ADR-0084: Multi-layer security scanning, splitting reporting from gating, on hardened runners
...
go through the SHA pinning and quarantine of [ADR-0085](0085-sha-pinned-actions.md).
...
- zizmor is the only scanner here that also runs pre-commit. The hook and CI call different
  ...
  ([ADR-0080](0080-local-hooks-mirror-ci.md)). It earns that
...
- Action pinning and the supply-chain quarantine the added actions go through:
  [ADR-0085](0085-sha-pinned-actions.md).
- Release-image integrity, a separate concern from these scans:
  [ADR-0096](0096-release-image-supply-chain.md).
- The `gosec` linter in `.golangci-full.yaml` provides an additional in-process security
  check during static analysis (see [ADR-0079](0079-two-layer-golangci-config.md)).
