...
# ADR-0057: Isolate the publisher's non-standard HTTP profile inside the relay
...
1. **`MaxAttempts = 1`** — transport-level retry must be disabled because the relay poll
   loop is the at-least-once retry mechanism (see [ADR-0052](0052-at-least-once-outbox-poll.md)).
...
3. **`AllowPrivateNetwork = false`** — the receiver endpoint is an external service;
   ...
   ADR-0021 and ADR-0022 for the resilience and SSRF client policies).
