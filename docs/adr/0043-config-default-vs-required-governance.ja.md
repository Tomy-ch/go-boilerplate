...
# ADR-0043: ガバナンス: コードデフォルト（不変）対ファイル必須（可変）

English canonical: [0043-config-default-vs-required-governance.md](0043-config-default-vs-required-governance.md)
...
**ファイル必須（可変）:** `envspec.go` で `required` とマークされたフィールドには埋め込みデフォルトがない。このような変数はすべて `env/.env`（ローカルデフォルト）とすべてのパーエンバイロメントファイル（`env/.env.<env>`）に存在しなければならない。必須フィールドが欠けている場合、`env.ParseAs` は失敗して起動を中断する（[ADR-0044](0044-immutable-fail-fast-config.ja.md) のフェイルファスト原則に沿う）。
...
- フェイルファスト起動の動作: [ADR-0044](0044-immutable-fail-fast-config.ja.md)。
