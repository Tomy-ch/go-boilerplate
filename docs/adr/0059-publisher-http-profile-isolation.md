...
# ADR-0059: Isolate the publisher's non-standard HTTP profile inside the relay
...
1. **`MaxAttempts = 1`** — transport-level retry must be disabled because the relay poll
   loop is the at-least-once retry mechanism (see [ADR-0054](0054-at-least-once-outbox-poll.md)).
...
3. **`AllowPrivateNetwork = false`** — the receiver endpoint is an external service;
   ...
   ADR-0023 and ADR-0024 for the resilience and SSRF client policies).
...
- Resilience and SSRF client policies: [ADR-0023](0023-outbound-http-resilience.md),
  [ADR-0024](0024-egress-ssrf-guard.md).
- Related ADRs: [ADR-0054](0054-at-least-once-outbox-poll.md),
  [ADR-0060](0060-relay-resident-gc-oneshot.md).
