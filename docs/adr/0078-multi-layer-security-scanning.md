...
# ADR-0078: Multi-layer security scanning (reachability-filtered govulncheck + scheduled CodeQL SAST + secret + fs scans)
...
- The `gosec` linter in `.golangci-full.yaml` provides an additional in-process security
  check during static analysis (see [ADR-0073](0073-two-layer-golangci-config.md)).
