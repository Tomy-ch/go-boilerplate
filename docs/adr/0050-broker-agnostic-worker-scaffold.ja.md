...
却下: このリポジトリに前例がなく、シングルバイナリではモジュール分離が不十分。`cmd` からアダプターをインポートしないことで、タグなしで分離が達成できる（[ADR-0052](0052-sqs-adapter-opt-in.ja.md) を参照）。

## 補足

- プッシュ/ストリーミング除外: ADR-0051（Phase 4）。SQS アダプターのオプトイン分離: [ADR-0052](0052-sqs-adapter-opt-in.ja.md)。
