...
# ADR-0048: Isolate the publisher's non-standard HTTP profile inside the relay
...
1. **`MaxAttempts = 1`** — transport-level retry must be disabled because the relay poll
   loop is the at-least-once retry mechanism (see [ADR-0043](0043-at-least-once-outbox-poll.md)).
...
- Related ADRs: [ADR-0043](0043-at-least-once-outbox-poll.md),
  [ADR-0049](0049-relay-resident-gc-oneshot.md).
