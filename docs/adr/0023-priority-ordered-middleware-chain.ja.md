...
このプロジェクトは依存性注入に [ADR-0040](0040-uber-fx-di.ja.md)（Uber fx）を使用している。各ミドルウェアは `internal/di/server/extension/` 配下の独自の `*_di.go` ファイルに存在するため、コールシーケンスが記述された単一の場所がない。各 `*_di.go` が `e.Use(...)` を直接呼び出すと、実際のチェーン順序は Go モジュールの初期化と fx ワイヤリング順序の新興的な結果になり、明示的でも保証されたものでもない。
