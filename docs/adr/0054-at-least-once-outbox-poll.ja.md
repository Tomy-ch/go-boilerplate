...
# ADR-0054: ポーリングによる少なくとも1回のデリバリー（トランスポートレベルのリトライを無効化）

English canonical: [0054-at-least-once-outbox-poll.md](0054-at-least-once-outbox-poll.md)
...
- 最終的なデッドレター処理（[ADR-0057](0057-outbox-dead-after-max-attempts.ja.md) を参照）が予測可能になる。`attempts` はポールごとに最大 1 回しか進まない。

### ネガティブな影響

- 失敗後のデリバリーレイテンシは `PollInterval`（デフォルト 1 秒）によって制限され、即時のトランスポートリトライよりも遅い。
- 少なくとも1回のセマンティクスにより、レシーバーは `Idempotency-Key` ヘッダー（[ADR-0056](0056-message-id-idempotency-propagation.ja.md) を参照）に対して冪等でなければならない。
...
- 関連 ADR: [ADR-0053](0053-transactional-outbox.ja.md)、[ADR-0055](0055-skip-locked-outbox-relay.ja.md)、[ADR-0057](0057-outbox-dead-after-max-attempts.ja.md)。
