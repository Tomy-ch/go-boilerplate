...
**ファイル必須（可変）:** `envspec.go` で `required` とマークされたフィールドには埋め込みデフォルトがない。このような変数はすべて `env/.env`（ローカルデフォルト）とすべてのパーエンバイロメントファイル（`env/.env.<env>`）に存在しなければならない。必須フィールドが欠けている場合、`env.ParseAs` は失敗して起動を中断する（[ADR-0045](0045-immutable-fail-fast-config.ja.md) のフェイルファスト原則に沿う）。
...
- フェイルファスト起動の動作: [ADR-0045](0045-immutable-fail-fast-config.ja.md)。
- サブシステム構造体のレイアウト: [ADR-0043](0043-subsystem-typed-config-loaders.ja.md)。
