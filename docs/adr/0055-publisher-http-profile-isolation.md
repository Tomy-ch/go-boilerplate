...
# ADR-0055: Isolate the publisher's non-standard HTTP profile inside the relay
...
1. **`MaxAttempts = 1`** — transport-level retry must be disabled because the relay poll
   loop is the at-least-once retry mechanism (see [ADR-0050](0050-at-least-once-outbox-poll.md)).
...
3. **`AllowPrivateNetwork = false`** — the receiver endpoint is an external service;
   ...
   ADR-0019 and ADR-0020 for the resilience and SSRF client policies).
...
- Related ADRs: [ADR-0050](0050-at-least-once-outbox-poll.md),
  [ADR-0056](0056-relay-resident-gc-oneshot.md).
