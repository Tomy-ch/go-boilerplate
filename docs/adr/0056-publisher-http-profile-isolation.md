...
# ADR-0056: Isolate the publisher's non-standard HTTP profile inside the relay
...
1. **`MaxAttempts = 1`** — transport-level retry must be disabled because the relay poll
   loop is the at-least-once retry mechanism (see [ADR-0051](0051-at-least-once-outbox-poll.md)).
...
- Related ADRs: [ADR-0051](0051-at-least-once-outbox-poll.md),
  [ADR-0057](0057-relay-resident-gc-oneshot.md).
