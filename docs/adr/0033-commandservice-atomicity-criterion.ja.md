...
# ADR-0033: CommandService は単一トランザクション原子性を要する複数集約書き込みに限定する

English canonical: [0033-commandservice-atomicity-criterion.md](0033-commandservice-atomicity-criterion.md)
...
[ADR-0031](0031-lightweight-cqrs.ja.md) は CommandService という構成要素の
...
`persistenceModule` に登録して DI で注入する。一方で ADR-0031 が意図的に空けたままに
...
（[ADR-0053](0053-transactional-outbox.ja.md)）と矛盾する。関連する曖昧さは ADR-0031
...
並行する書き込みがそれを無効化し得る（[ADR-0035](0035-ordered-pessimistic-row-locks.ja.md)）。
...
- **理由。** Evans の議論は *変更* についてのものである。広い境界の害は、読み込んだ
  ...
  ドメインが著述する（[ADR-0031](0031-lightweight-cqrs.ja.md) § Derivation）。
...
- **購入作成 — 分岐 2 と分岐 3 の同時適用。** 在庫の検証と減算は購入確定と原子的で
  ...
  [ADR-0053](0053-transactional-outbox.ja.md) のとおり同一トランザクションに参加する。
...
- 分岐 2 はガード対象の行のスループットを代償にする。その行に対する並行する書き手の
  ...
  （[ADR-0035](0035-ordered-pessimistic-row-locks.ja.md)）。
...
（[ADR-0053](0053-transactional-outbox.ja.md)）と矛盾する。却下。
...
[ADR-0035](0035-ordered-pessimistic-row-locks.ja.md) が記録するインターリーブである。
...
（[ADR-0035](0035-ordered-pessimistic-row-locks.ja.md) § 5）、書き込み最適化の構成要素の
ものではない。
...
[ADR-0035](0035-ordered-pessimistic-row-locks.ja.md) が記録するとおりである。分離レベルは
...
- **[ADR-0031](0031-lightweight-cqrs.ja.md) との関係**: 補完関係であり、衝突しない。
  ADR-0031 は構成要素の構造（インターフェースの配置、実装ディレクトリ、DTO 返却、
  DI 登録）を定義し、本 ADR は ADR-0031 が空けたままにした配置基準を与える。
  1 点の明確化: ADR-0031 の帰結にある CommandService は「柔軟な update や delete を
  ...
  唯一の基準であり、今後の配置は本 ADR が規律する。ADR-0031 は編集も supersede も
  しない。
- **[ADR-0035](0035-ordered-pessimistic-row-locks.ja.md) との関係**: 本 ADR は集約横断の
  条件を保持する必要が *あるか* を決め、ADR-0035 はそれを *どう* 保持するか — ロック順序、
  取得位置、ロックモード、守る条件を誰が著述してよいか — を決める。分岐 2 はその入口で
  あり、分岐 2 の記述が ADR-0035 を上書きすることはない。
...
- 関連: [ADR-0032](0032-system-cqrs-dml-category.ja.md)（CQRS 分割の外側で outbox の
  DML を担う `system_cqrs` カテゴリ）、
  [ADR-0053](0053-transactional-outbox.ja.md)（デフォルトの分解経路が依存する
  outbox パターン）、
  [ADR-0034](0034-transaction-retry-idempotent-callers.ja.md)（原子的書き込みが走る
