...
# ADR-0031: 軽量CQRSの採用 — 書き込みにRepository、読み込みにQueryService
...
`persistenceModule` に登録され、Uber Fx で注入される（[ADR-0039](0039-uber-fx-di.ja.md)を参照）。
...
fail-closed な二重の網として言い直したものである（[ADR-0035](0035-ordered-pessimistic-row-locks.ja.md)）。
...
- 関連: [ADR-0032](0032-system-cqrs-dml-category.ja.md)（CQRS の分割の外に位置する第 4 の
