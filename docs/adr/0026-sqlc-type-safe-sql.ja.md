...
# ADR-0026: sqlcによる型安全なSQLアクセスの生成

English canonical: [0026-sqlc-type-safe-sql.md](0026-sqlc-type-safe-sql.md)
...
SQLファーストの方針（[ADR-0025](0025-sql-first-data-access.ja.md)）を前提とすると、手書きのSQLをGoに**コンパイル時の型安全な**コードとして届ける必要があるが、ORMのランタイム抽象や暗黙的なクエリ生成を再導入したくない。
...
- [ADR-0025](0025-sql-first-data-access.ja.md)（SQLファースト）を前提とする。
