...
# ADR-0046: Broker-agnostic pull-ack worker scaffold
...
- The port is deliberately narrow (pull-ack only); push and streaming-log models do not fit
  and are excluded (recorded separately as ADR-0047, materialized in Phase 4).
...
[ADR-0048](0048-sqs-adapter-opt-in.md)).

## Notes

- Push / streaming exclusion: ADR-0047 (Phase 4). SQS adapter opt-in isolation: [ADR-0048](0048-sqs-adapter-opt-in.md).
