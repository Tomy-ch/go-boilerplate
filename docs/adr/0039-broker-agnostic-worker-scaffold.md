...
# ADR-0039: Broker-agnostic pull-ack worker scaffold
...
- The port is deliberately narrow (pull-ack only); push and streaming-log models do not fit
  and are excluded (recorded separately as ADR-0040, materialized in Phase 4).
...
[ADR-0041](0041-sqs-adapter-opt-in.md)).

## Notes

- Push / streaming exclusion: ADR-0040 (Phase 4). SQS adapter opt-in isolation: [ADR-0041](0041-sqs-adapter-opt-in.md).
