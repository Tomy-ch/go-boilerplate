...
# ADR-0076: ベンダー中立の OTLP 専用エクスポート（バックエンドは Collector に委譲）
...
この決定は [ADR-0075](0075-config-driven-observability-gating.ja.md) によってゲーティングされる。
...
生み出す。[ADR-0075](0075-config-driven-observability-gating.ja.md) がこの却下を詳述している。
...
- 設定駆動ゲーティング: [ADR-0075](0075-config-driven-observability-gating.ja.md)。
