...
# ADR-0094: すべてのロールを 1 つのマルチコマンドバイナリに集約する

English canonical: [0094-single-multi-command-binary.md](0094-single-multi-command-binary.md)
...
- バージョン管理、プッシュ、プルのための 1 つのビルド成果物。同じイメージをコマンドオーバーライドで任意のロールとして実行できる（[ADR-0095](0095-single-runtime-image.ja.md) 参照）。
...
- コマンドロジックは [ADR-0093](0093-cli-humble-object-split.ja.md) に従って `internal/cli/<command>/` に置かれる。
