...
# ADR-0087: Per-environment images (.env matrix x APP_ENV build-arg, fixed at build time)
...
build time via `go:embed` (see ADR-0037). Because the embedded file is baked in at compile
...
- Each image is independently verifiable and auditable: the embedded config is part of
  the signed artifact (see [ADR-0089](0089-release-image-supply-chain.md)).
...
- The embedded-config mechanism itself is documented in ADR-0037.
