...
フィルタがどの識別子を受けるかは [ADR-0031 (master-data-via-migration)](0031-master-data-via-migration.ja.md) が別途決めている。クライアントが送るのはマスタ行の `code` であり、UUID でも表示名でもない。日本語のテキストがそもそも絞り込みパラメータに載らないのは、この決定によるものである。
...
- マスタデータ絞り込みの識別子の選択: [ADR-0031 (master-data-via-migration)](0031-master-data-via-migration.ja.md)
- 上記のランタイム強制は [ADR-0016 (spec-driven-request-validation)](0016-spec-driven-request-validation.ja.md)、境界値をどの層が所有するかは [ADR-0018 (boundary-value-ownership)](0018-boundary-value-ownership.ja.md)
