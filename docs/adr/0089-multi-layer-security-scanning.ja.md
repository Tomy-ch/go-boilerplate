...
| cooldown より新しい依存バージョン | Go cooldown + tool cooldown（[ADR-0091](0091-malicious-package-detection-via-cooldown.ja.md)。pnpm は依存解決の時点で自前の窓を課す） | `go.mod` / `mise.toml` / `python/*.in` の変更 / 週次 | 同一ワークフロー（direct な Go の require とツール宣言） |
...
[ADR-0090](0090-sha-pinned-actions.ja.md) の SHA pin と隔離期間を通す。
...
- ここに挙げたスキャナのうち pre-commit でも走るのは zizmor だけ。フックと CI が呼ぶ `make`
  ...
  （[ADR-0085](0085-local-hooks-mirror-ci.ja.md)）。開発者が
...
- 追加したアクションが通るサプライチェーンの隔離期間とアクション pin:
  [ADR-0090](0090-sha-pinned-actions.ja.md)
- これらのスキャンとは別の関心事であるリリースイメージの完全性:
  [ADR-0101](0101-release-image-supply-chain.ja.md)
- `.golangci-full.yaml` の `gosec` linter は静的解析時のインプロセスチェックとして追加的に機能する
  （[ADR-0084](0084-two-layer-golangci-config.ja.md)）
