...
accepted (supersedes [ADR-0047](0047-sqs-adapter-opt-in.md))

## Context

[ADR-0047](0047-sqs-adapter-opt-in.md) kept the SQS reference adapter out of the default `cmd`
...
ADR-0047 was accepted. `github.com/aws/aws-sdk-go-v2/service/s3` — and with it the SDK core
(`aws`, `credentials`, `signer/v4`, `retry`, `transport/http`) and `smithy-go` — is now linked into
the default binary. ADR-0047's context ("every binary — including `serve`, which never consumes a
...
The concern behind ADR-0047 is better stated as **coupling**, not linkage: the repository must not
...
- Supersedes [ADR-0047](0047-sqs-adapter-opt-in.md). Parent decision:
  [ADR-0045](0045-broker-agnostic-worker-scaffold.md). Principle: [ADR-0001](0001-avoid-lock-in.md).
