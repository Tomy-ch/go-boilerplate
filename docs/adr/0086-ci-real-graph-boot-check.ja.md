...
# ADR-0086: CI は実際の Postgres に対して実際の fx グラフを起動する（スタートアップ検証）

English canonical: [0086-ci-real-graph-boot-check.md](0086-ci-real-graph-boot-check.md)
...
- 関連: [ADR-0039](0039-uber-fx-di.ja.md) — このチェックが実行する Uber fx DI コンテナのグラフ。
- 関連: [ADR-0049](0049-broker-agnostic-worker-scaffold.ja.md) — エントリポイントが検証されるワーカーサブシステム。
