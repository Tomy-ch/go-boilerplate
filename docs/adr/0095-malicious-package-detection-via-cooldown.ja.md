...
# ADR-0095: 悪意あるパッケージへの主防御として専用の検出器を持たず公開クールダウンを受け入れる
...
| 対象 | 窓 | 機構 |
| --- | --- | --- |
| Node パッケージ | 7 日 | 各 `pnpm-workspace.yaml` の `minimumReleaseAge`。依存解決の時点で拒否される（[ADR-0112](0112-pnpm-as-the-only-node-resolver.ja.md)） |
...
| PyPI から入れる CLI ツール | 7 日 | `scripts/tool-cooldown` が `python/*.in` の宣言を読む（[ADR-0084](0084-mise-ssot-drift-gate.ja.md)） |
...
- 関連: [ADR-0094](0094-sha-pinned-actions.ja.md)（SHA ピン留め）、[ADR-0093](0093-multi-layer-security-scanning.ja.md)（これが隣り合うスキャンのレイヤー群）、[ADR-0001](0001-avoid-lock-in.ja.md)（SaaS 検出器が対象外である理由）。
