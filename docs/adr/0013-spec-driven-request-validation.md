...
# ADR-0013: Validate requests and enforce auth from the spec at runtime; do not validate responses
...
- The `/metrics` auth exception (skipped from this pipeline; protected by a separate
  BasicAuth middleware) is recorded in [ADR-0016](0016-metrics-endpoint-auth-exception.md).
