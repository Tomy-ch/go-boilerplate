...
[ADR-0082](../adr/0082-scripts-in-node-go.ja.md) が述べる一般形は「各スクリプトは判定ロジックを純粋なモジュールに置き、隣にテストを持つ。ゲートの壊れ方は『何も検査しないまま `0` で終了する』方向に出るので、それを固定できるのは型検査とテストだけである」。これを慣習から規則へ押し上げたのが `scripts/setup/` 配下の 1 回限りのスクリプト群であり、その論拠は上流でしか成立しない。
