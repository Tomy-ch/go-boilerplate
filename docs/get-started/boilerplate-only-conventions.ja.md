...
[ADR-0086](../adr/0086-scripts-in-node-go.ja.md) が述べる一般形は「各スクリプトは判定ロジックを純粋なモジュールに置き、隣にテストを持つ。ゲートの壊れ方は『何も検査しないまま `0` で終了する』方向に出るので、それを固定できるのは型検査とテストだけである」。これを慣習から規則へ押し上げたのが `scripts/setup/` 配下の 1 回限りのスクリプト群であり、その論拠は上流でしか成立しない。
...
**この前提は継承されない。** テンプレートから作られたプロジェクトではセットアップスクリプトは既に走り終えており、文脈を持たない他人も、使用の瞬間で切れる CI も存在しない。残るのは ADR-0086 の一般形であり、それはゲートの壊れ方だけで独立に立つ。
