...
Three scanners here need something this repository cannot supply on its own. Two want a token for
a vendor's service — [`sonarqube.yaml`](../../.github/workflows/sonarqube.yaml),
[`codacy.yaml`](../../.github/workflows/codacy.yaml) —
and [`code-ql.yaml`](../../.github/workflows/code-ql.yaml) needs GitHub Advanced Security, which is
free for a public repository and billed for a private one. All three are free here because this
repository is public. Yours may be neither public nor willing to pay.

Nothing is broken while you decide. Each of the two checks for its token before it scans and skips
...
Register two secrets on the repository, and create the matching project on each vendor's side:
...
The script deletes all three and **commits each product separately**, so a licence you already hold
is one `git revert` away:

```bash
git revert <the "CI: Codacy のワークフローを撤去する" commit>
```

Two things to know about that undo. The README rows come off in a single final commit rather than
with each product, because the three occupy adjacent rows of the same tables and a per-product doc
...
Removing the workflows does not remove the secrets. Delete `SONAR_TOKEN` /
