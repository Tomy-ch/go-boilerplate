...
これは**最後に**実行してください。先行する各撤去は編集対象の文書に完全一致の文字列を宣言して
おり、先に言語を畳むと、その宣言が待っている本文ごと消えます。
...
**先送りせず、ここで決めてください。** この先はふつうの開発で、その 1 日ごとに両方の言語の
代償を払うことになります —— ドキュメントの修正は 2 回書くことになり、エージェントの実行は
必要のない対訳を読み飛ばし、片方だけを直した瞬間に両者はずれます。代償は繰り返し発生しますが、
判断は 1 回きり、コマンド 1 本です。この手順を後回しにするのは代償の先送りではなく、支払いです。
<!-- lang-choice:end -->
