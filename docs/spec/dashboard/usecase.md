...
> （ADR-0030 (lightweight-cqrs) 軽量CQRS / `docs/rules.md` の Repository / QueryService 境界に準拠）。既存の判断基準で
...
入力の正規化（`period` の区分解釈と、`range` 指定時の開始日・終了日の相関検証）のみを担い、集計本体・期間境界の算出は QueryService／infra に委ねる。売上はキャンセル済みの購入を除外し未払いの購入を含む（商品売上ランキングと同一の母集団）。一方ステータス別件数はキャンセル済みも 1 ステータスとして含むため、両者の母集団は一致しない。商品数は集計期間に依存しないマスタの現在値。
...
- **期間の境界は設定のタイムゾーンで解釈する。** `today` / `month` は現在時刻から、`range` は指定された暦日から、いずれも半開区間として infra が算出する（現在時刻とロケーションへの依存を infra に閉じる）。`range` は開始日・終了日の両端を含む。ロケーションは設定から構築された値を注入して用い、実行環境の `time.Local` には依存させない（コンテナの既定は UTC のため、依存させると設定と異なる暦日で集計してしまう）。
