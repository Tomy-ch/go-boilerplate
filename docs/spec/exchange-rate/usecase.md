...
> エンティティ・テーブルを持たない。そのため lean-a-spec（[ADR-0093 (lean-a-spec-scaffold)]）の「原則 domain + usecase」から
...
（`pkg/decimal.ToScaledInt64`）には焼き込まない（[ADR-0037 (two-scale-quantity-model)]）。現状の表示通貨は JPY のみで
...
パーサが IEEE754 double として復号するため、そこで精度が落ちる（[ADR-0037 (two-scale-quantity-model)]）。
...
作らず既存の継ぎ目を decorate するという判断そのものは [ADR-0105 (no-generic-cache-abstraction)]。
...
[ADR-0093 (lean-a-spec-scaffold)]: ../../adr/0093-lean-a-spec-scaffold.md
[ADR-0105 (no-generic-cache-abstraction)]: ../../adr/0105-no-generic-cache-abstraction.md
