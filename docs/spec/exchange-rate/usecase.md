...
> エンティティ・テーブルを持たない。そのため lean-a-spec（[ADR-0091]）の「原則 domain + usecase」から
...
（`pkg/decimal.ToScaledInt64`）には焼き込まない（[ADR-0035]）。現状の表示通貨は JPY のみで
...
パーサが IEEE754 double として復号するため、そこで精度が落ちる（[ADR-0035]）。
...
作らず既存の継ぎ目を decorate するという判断そのものは [ADR-0103]。
...
[ADR-0091]: ../../adr/0091-lean-a-spec-scaffold.md
[ADR-0103]: ../../adr/0103-no-generic-cache-abstraction.md
