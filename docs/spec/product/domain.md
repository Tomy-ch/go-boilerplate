...
商品集約は、商品の基本情報（名称・説明・価格・在庫）と分類の ID 参照（ステータス ID・カテゴリ ID）、および公開日時を保持するドメインエンティティ。`price` はサブセント精度を保持する価格スケール（Decimal）の値オブジェクト `money.Price` で保持する（非負は VO が担保。2 スケールモデルは ADR-0034）。生成時に必須・長さの不変条件を検証し、違反する `Product` は構築できない。

一覧取得は「公開済み（`publishedAt` 非 NULL）の商品を `(publishedAt, id)` の keyset ページネーションで返す」read-only な集約読み取りであり、すべて products 自身の列への操作のため QueryService ではなく domain `product.Repository` に委譲する（ADR-0028 / `docs/rules.md` の Repository 境界に準拠）。ステータスによる可視範囲の絞り込みは後続 PBI（#555）で対応する。
...
# pkg/decimal.Decimal（ADR-0034）。分類は ID 参照のまま VO を持たない。
```
...
- name: FindAllLowStock
  ...
  behavior: |
    ...
    usecase / controller の分岐も置かない（ADR-0028: 単一集約の自属性フィルタは Repository）。
...
- name: FindPublishedByID
  ...
  behavior: |
    ...
    可視性判断は SQL に閉じ、usecase / controller には分岐を置かない（ADR-0028: 単一集約の ID fetch は Repository）。
...
- name: LockByID
  ...
  behavior: |
    ...
    （ADR-0032: 購入と在庫補充は同じ行を同じロック規律で扱う）。
...
- name: LockByIDs
  ...
  behavior: |
    ...
    デッドロックしないためである（ADR-0032: ロック順序を単一の全域順序に固定する規律）。
...
- name: Update
  ...
  behavior: |
    ...
    この衝突は tx.Manager が透過リトライする serialization_failure（ADR-0030）と異なり、同じ内容の再送では解消しない。
