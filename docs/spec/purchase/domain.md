...
> **切り捨て**、ドメイン内 1 箇所に集約する。2 スケールの型分けそのものは [ADR-0035]。
...
初期ステータスは「未処理」（`purchase_statuses.code = 1`）。ドメインは code（安定した業務キー）を定数として持ち、`purchase_statuses` の UUID は焼き込まない（seed との二重管理を避けるため、UUID 解決は永続化時の infra 責務）。`id` / `code` は UUIDv7（[ADR-0034]）で、生成は usecase 層が行いドメインへ渡す（ドメインは乱数・時刻に直接依存しない）。

書き込み後のドメイン整合の再検証とレスポンス組み立ての取得元として、`Repository.FindByID` で永続化済みの購入を明細込みで読み出す（[ADR-0029] / [ADR-0031]）。
...
- name: Pay
  ...
  behavior: |
    購入を支払い済み状態へ遷移させる（PATCH /v1/purchases/{purchaseId}/pay の状態遷移・擬似決済。決済 seam の除外は nextjs-boilerplate ADR-0082）。
...
- name: LockByID
  ...
  behavior: |
    ...
    CommandService ではなく Repository が担う（[ADR-0031] の判定軸）。
- name: UpdatePaid
...
- name: FindFeedByUserID
  ...
  behavior: |
    ...
    （購入ステータスは購入集約に属する固定参照マスタで、[ADR-0029] の子参照マスタ例外により単一集約の
...
なお、状態遷移に伴う購入行の悲観ロック（`FOR UPDATE`）は、書き込みが集約を跨ぐかで担い手が分かれる（[ADR-0031] の判定軸）:

- **キャンセル**は在庫復元（`products`）+ 購入更新（`purchases`）の**複数集約への原子的書き込み**のため CommandService（`LockPurchase` / `CancelPurchase`）が担う（[ADR-0029] / [ADR-0031]）。
...
[ADR-0034]: ../../adr/0034-uuidv7-identifiers.md
