...
購入ステータスは購入集約に属する固定参照マスタで独立集約ではないため、[ADR-0032 (lightweight-cqrs)] の子参照マスタ例外に該当し QS ではなく Repository で JOIN する）。
...
（[ADR-0032 (lightweight-cqrs)]。子参照マスタへの JOIN で済む一覧・cancel/pay の `purchase.Detail`（Repository read）とは経路を分ける）。商品名は `products` との
