...
- **Before writing a comment, ask what fails when it turns false.** If something already fails — a compile error, a test, a `depguard` / `forbidigo` rule, an `internal/architest` scan, a regeneration check — then that mechanism is what keeps the fact true and the comment is a second copy of it. The two are not peers: the mechanism is updated whenever reality moves, because nothing proceeds until it is green again, while the comment is updated only when someone remembers. So the copy is the one that goes wrong, and it goes wrong invisibly, and a reader trusts it because a comment reads as authoritative. **Do not write it.** If nothing fails, ask next whether something could be made to — a test case, an architecture scan, an `ExampleXxx` function, a lint rule — and build that instead; the comment then has nothing left to say. What survives both questions is the comment's proper subject: **a comment records what no mechanism could be built to check.**
- **Deleting on that question requires naming where the fact now lives** — the file and line that keeps it true. "It reads as unnecessary" is not that; it is the same reading that produced the duplicate. A sentence you cannot trace to another owner is not redundant but **unguarded**, which is the case for keeping it.
- **The doc comment is canonical for the caller-facing contract; `docs/spec/**` is canonical for its reasons.** The first question decides which of two copies goes, and without a declared direction the same triplication resolves toward deleting the comment one day and the spec the next. The direction here follows from the mechanisms: `revive`'s `exported` rule makes a doc comment mandatory on every exported declaration and `make gen-godoc` renders all of them, so it is the only surface a caller in another package actually reads — it **publishes** the contract rather than copying it. What a caller can observe therefore stays here: what the declaration does, the meaning of its inputs / outputs / errors, and the order in which conditions are judged when that order is observable. Why the business works that way is not observable from the call site and belongs to `docs/spec/**`; the comment does not restate it. The same split is what the *jurisdiction* bullet below already applies to `docs/adr/` and `docs/design/**`.
- **Editing a declaration puts its whole comment under judgment, not only the lines you touched.** Apply the questions above to every sentence that is there, delete the ones that no longer hold, and **leave the surviving ones exactly as they are.** Judging is idempotent; rewriting is not. A pass that deletes and re-words rebuilds the same prose on every visit, and the second wording is no truer than the first.
...
- **Language scope**: godoc governs Go only, but this content standard is **language-agnostic** — it applies to non-Go alike (shell, `.mjs` / `.jsx`, Dockerfile, Makefile, SQL, YAML). Non-Go is **higher-risk**, not exempt: `revive` covers only Go, so for non-Go the `settle-comments` pass (and the `comment-reviewer` agent it fans out) is the *only* check. Hold non-Go comments to the same bar — no How narration, no 経緯, no restatement; a constraint whose premise sits at that site stays.
- **Enforcement split**: `revive`'s `exported` rule guarantees only the **presence** and **`Name`-prefixed format** of doc comments on exported declarations. This **content** rule (godoc-conformant contract + co-located constraint + no noise + no copy of a fact some mechanism already guards) is semantic and cannot be linted — it is enforced by the `settle-comments` skill, which applies the questions and the jurisdiction test above, drops what another mechanism already guards, and relocates content a document owns. That pass runs **unconditionally as the last step of every implementation** over the declarations the change touched (`AGENTS.md`, *Task Execution Protocol*), and over a package / layer / repo scope when the accumulated stock is the subject. It is deliberately not a review lens: the judgment has to be made in the detection context, after the code is written, rather than while the same run is generating prose for free.
...
- **Which side is the error depends on what the document does.** A doc that *describes* — a package README, a command reference, a guide — is corrected to match the code. A doc that *governs* — `docs/rules.md`, `docs/architecture.md`, `docs/adr/**`, `docs/spec/**` — is not: it states the intent the code is meant to satisfy, so a disagreement is as likely to be a defect in the code. Report the disagreement; changing a governing document to match what was built is a decision for this repository's architect or tech lead, not a drift fix.
