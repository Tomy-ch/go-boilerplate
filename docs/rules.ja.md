...
- **コメントを書く前に、それが偽になったとき何が落ちるかを問う。** 既に落ちるものが在るなら —— コンパイルエラー、テスト、`depguard` / `forbidigo` のルール、`internal/architest` の走査、再生成チェック —— その機構こそが事実を真に保っているのであり、コメントはその二つ目の写しである。両者は対等ではない: 機構は現実が動くたびに必ず更新される（緑にならない限り先へ進めないため）が、コメントは誰かが思い出したときにしか更新されない。だから壊れるのは写しの側で、しかも何も赤くならずに壊れ、コメントは断定調で読まれるため読み手はそれを信じる。**書かない。** 何も落ちないなら、次に落ちるものを作れるかを問い —— テストケース、アーキテクチャ走査、`ExampleXxx` 関数、lint ルール —— それを作る。そうすればコメントに残ることは無くなる。両方の問いを越えて残ったものがコメントの本来の主題である: **コメントとは、評価者を作れなかったことの記録である。**
- **その問いで削除するには、その事実がいまどこに在るかを名指すこと** —— それを真に保っているファイルと行番号を示す。「読んで不要に見えた」はそれではない。それは写しを書かせたのと同じ読み方である。別の持ち主に辿れない文は冗長なのではなく**守られていない**のであり、それは残す理由になる。
- **契約の正本は doc コメント、その理由の正本は `docs/spec/**`。** 第一の問いは 2 つの写しのどちらが消えるかを決めるが、向きが宣言されていなければ、同じ三重化がある日はコメントを消し別の日には spec を消す形で解決されてしまう。ここでの向きは機構から導かれる: `revive` の `exported` が全 export 宣言に doc コメントを必須とし、`make gen-godoc` がそのすべてを描画するので、他パッケージの呼び出し側が実際に読む面はここだけである —— doc コメントは契約を**公表**しているのであって写しているのではない。したがって呼び出し側が観測できるものはここに残る: その宣言が何をするか、入出力・エラーの意味、そして判定の順序が観測可能なときはその順序。なぜ業務がそうなっているかは呼び出し地点から観測できず `docs/spec/**` に属し、コメントはそれを再掲しない。同じ切り分けを、下記の*管轄*の箇条が既に `docs/adr/` と `docs/design/**` に対して行っている。
- **宣言を編集したら、その宣言のコメント全体が判定の対象になる。触った行だけではない。** 上の問いをそこに在るすべての文へ当て、成り立たなくなった文を消し、**残った文はそのままにする。** 判定は冪等だが書き直しは冪等ではない。消して書き直すパスは訪れるたびに同じ散文を作り直し、二度目の言い回しが一度目より真であるわけではない。
...
- **言語スコープ**: godoc は Go のみを統べるが、この内容基準は**言語非依存** — 非 Go（shell, `.mjs` / `.jsx`, Dockerfile, Makefile, SQL, YAML）にも等しく適用する。非 Go は**高リスク**で対象外ではない: `revive` は Go しか見ないため、非 Go では `settle-comments` のパス（とそれが fan-out する `comment-reviewer` agent）が唯一のチェックになる。同基準で扱う — How ナレーション・経緯・言い換えは NG、前提がその場にある制約は残す。
- **強制の分担**: `revive` の `exported` ルールは export 宣言の doc コメントの **有無**と **`Name` 前置の形式**しか保証しない。この **内容**ルール（godoc 準拠の契約 + 前提がその場にある制約 + ノイズなし + 既に何らかの機構が守っている事実の写しでないこと）は意味的で lint 不能 — `settle-comments` スキルが強制する。上記の問いと管轄の判定基準を適用し、他の機構が既に守っている内容を落とし、文書が管轄を持つ内容を移設する。このパスは、変更が触れた宣言に対して**すべての実装の最後に無条件で**走り（`AGENTS.md` の *タスク実行プロトコル*）、蓄積した在庫そのものが主題のときはパッケージ / レイヤ / リポジトリ単位で走る。レビューのレンズにしていないのは意図的である: この判定はコードを書き終えた検出の文脈で下される必要があり、同じ実行が散文をタダで生成している最中では効かない。
...
- **どちらが誤りかは、その文書が何をする文書かで決まる。** *記述する*文書——パッケージ README、コマンドリファレンス、ガイド——はコードに合わせて直す。*統べる*文書——`docs/rules.md`・`docs/architecture.md`・`docs/adr/**`・`docs/spec/**`——は違う。そちらはコードが満たすべき意図を述べているのだから、食い違いはコード側の欠陥である可能性が同じだけある。食い違いは報告すること。統べる文書を「作られたもの」に合わせて変えるのは、このリポジトリのアーキテクトまたはテックリードの判断であって、ドリフトの修正ではない。
