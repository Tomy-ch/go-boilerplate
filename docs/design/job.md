...
本書は job scaffold の **役割論・状態遷移・実装箇所・integrator が書く箇所・用語** を、実装を精査して 1 枚にまとめた参照資料です。概要は README、非同期の兄弟である worker は [worker.md](worker.md) を参照。
...
| **idempotencygc** | 失効した冪等性キーを掃除する同梱ジョブ（`--batch-size=N`）。[idempotency.md](idempotency.md) 参照。 |
