...
**相対更新、集合ベースの操作、ロックを取らずに原子性を得る操作**。注文キャンセル時に引き当て数量を戻す
のがその形である —— 在庫行に一切ロックを取らない相対更新であり、load-add-save として表現するとキャンセル
...
[ADR-0034](../adr/0034-commandservice-atomicity-criterion.ja.md) の § Departure from "1 Aggregate = 1
