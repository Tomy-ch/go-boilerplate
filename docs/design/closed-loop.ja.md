...
そこで各スキルはクラスを宣言し、書ける場合は「その窓がそれを使えたはずか」を述べる述語を持つ。使った数 / 使えたはずの数 ＝ カバレッジこそが意味のある尺度であり、述語を正直に書けたスキルについてのみ存在する。機会を観測する汎用の方法は無い。表現できないものはそう印を付け、別のもので判断する。宣言は `.agents/closed-loop/skill-meta.yaml` にある。
