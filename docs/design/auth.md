...
検証コアは [jwt README](../../internal/infrastructure/auth/jwt/README.md)、Provider の配線と設定は [docker README](../../docker/README.md) を参照。その HTTP 表面は素の OIDC Core であり、本リポジトリはその写しを持たない——他人の実装の spec は腐る spec だからである。本ページは両者を結ぶ横断ナラティブである。
...
<!-- = identity として登録した値でなければなりません。そうでないと検証は成功し、その先の identity -->
<!-- = 解決で拒まれます。**ロールはトークンに載りません。** ロールはこのサービスの DB に -->
<!-- = あり、このサービス自身の API が返します。IdP がたまたまロールを知っているのは特定デプロイの性質で -->
...
| ops-path / metrics の auth 例外 | `internal/controller/httpstack/oapi/skipper/`, ADR [0020](../adr/0020-metrics-endpoint-auth-exception.md) |
...
3. **IdP 方言を追加**する（標準コアから外れる場合）— Cognito `token_use` / `aud`→`client_id`、Azure `scp` / `roles`、EC 鍵、opaque token — は [jwt README](../../internal/infrastructure/auth/jwt/README.md) の拡張ポイントで。
