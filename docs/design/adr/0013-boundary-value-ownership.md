...
# ADR-0013: OpenAPI is the wire contract, not the domain rule; request is subset of domain, domain is subset of response
...
- The request constraint may be *stricter* than the domain rule. The request-validation
  middleware (see [ADR-0012](0012-spec-driven-request-validation.md)) rejects out-of-range
...
- The request validation that enforces the wire-contract side is described in
  [ADR-0012](0012-spec-driven-request-validation.md).
