...
# ADR-0047: Isolate the publisher's non-standard HTTP profile inside the relay
...
1. **`MaxAttempts = 1`** — transport-level retry must be disabled because the relay poll
   loop is the at-least-once retry mechanism (see [ADR-0042](0042-at-least-once-outbox-poll.md)).
...
3. **`AllowPrivateNetwork = false`** — the receiver endpoint is an external service;
   ...
   ADR-0017 and ADR-0018 for the resilience and SSRF client policies).
...
- Resilience and SSRF client policies: ADR-0017, ADR-0018 (plain text — not yet published).
- Related ADRs: [ADR-0042](0042-at-least-once-outbox-poll.md),
