...
# ADR-0086: Migrations run as a pre-deploy one-shot; do NOT auto-migrate at application startup
...
[ADR-0083](0083-single-runtime-image.md)) precisely to support the pre-deploy pattern via
command override.
...
achieves the same result without the overhead (see [ADR-0083](0083-single-runtime-image.md)).
