...
English: [context-map.md](context-map.md)
...
| outbox の受信者 | このシステムが上流 | `未確定` | publish 境界は substrate-agnostic な envelope を持つが、payload のスキーマは公開されていない。[ADR-0056](../adr/0056-message-id-idempotency-propagation.ja.md) が定めるのは転送規約のみ。[`outbox.ja.md`](outbox.ja.md) を参照 | 受信者の期待は、このシステムが公開するどの成果物にも守られていない |
