...
**例外は期限つきの債務で、その期限は両方向に効きます。** 窓が開く前に消せば install がすべて落ち、版がカットオフを越えて熟成すればエントリは不要になり、無くても lockfile は通ります。したがって各エントリには、何を免除しているか・理由・そのパッケージがどこで動くか・いつ削除できるようになるかを記録します。そしてその日付は散文ではなく機械可読にします。人しか読まない日付は、誰も読み返さない日付だからです。[`scripts/pnpm-cooldown`](../../scripts/pnpm-cooldown)（`make pnpm-cooldown-check`）はエントリの行末コメントに `expires: YYYY-MM-DD` と `issue: <N>` を要求し、期限が切れている・3 ヶ月より先を指している・lockfile がもう解決していない版を名指ししている場合に失敗します。これは `go-cooldown` / `tool-cooldown` のバイパスファイルが持つ 3 つの失敗と同じで、理由も同じです。誰も見直さない免除は一時的であることをやめ、恒久 allowlist になります。pull request だけでなくスケジュールでも走るのは、期限が `pnpm-workspace.yaml` の変更なしに訪れるためです。これは pnpm が自前で必要とした唯一の検査でした——解決器が窓を守っていた一方で免除は誰も守っておらず、このゲートができるまで 2 件が記載の削除期限を 23 日と 27 日過ぎたまま残っていました。
