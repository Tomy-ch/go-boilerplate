...
| **fan-out substrate** (`infrastructure/realtime/`) | infrastructure | SNS publish (the realtime publisher and the revocation notifier), per-instance SQS queue / subscription lifecycle, receive and delete on the instance's inbox | what a wakeup means |
| **consumer engine** (`controller/realtime/`) | controller | drain the instance's inbox, coalesce wakeups per batch, hand them and the revocations to the Streamer's sinks; the instance-lease heartbeat loop | what to do with a wakeup (the Streamer's), the transport |
...
| `internal/di/module/realtime.go` | wired only when at least one feature adapter is provided; contributes the serve-lifecycle participants (readiness probe, the lease + inbox provisioner, the consumer and heartbeat runners) that `internal/di/server/hook` runs in the order of §2.5 |
...
| Kind | Values |
| --- | --- |
| **Typed config** (deployment-dependent) | EventLog endpoint / region / table; ticket and lease tables; credentials (empty → SDK default chain); fan-out endpoint; SNS topic ARN; SQS resource prefix; DLQ ARN (empty → no redrive); **max SSE connections per instance**; **replay / catch-up concurrency** |
