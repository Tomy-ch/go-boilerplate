...
- worktree × DB スロットプールの詳細 → [db-worktree-pool.md](db-worktree-pool.md)
- o11y の送出配線 → [observability.md](../design/observability.md)、認証（疑似 OIDC）→ [auth.md](../design/auth.md)
...
| `elasticmq` | infra | `softwaremill/elasticmq-native` | `9324`（SQS API） | 開発用の SQS 互換ブローカー（テストは in-process の fake）。全 checkout で共有され、スロット単位に隔離**できない** — [`db-worktree-pool.md`](db-worktree-pool.md) 参照 |
...
- リース・ブートストラップ・`make slot-acquire` / `slot-free` / `slot-release` 等の**全詳細は正本の [db-worktree-pool.md](db-worktree-pool.md) を参照**。
...
| worktree × DB スロットプール（正本） | [db-worktree-pool.md](db-worktree-pool.md) |
| 生成物 / root 所有 / DB 落ちの復旧手順 | `repo-ops` スキル |
| Observability の送出配線 | [observability.md](../design/observability.md) |
| 認証（疑似 OIDC / JWKS） | [auth.md](../design/auth.md) |
