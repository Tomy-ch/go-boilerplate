...
- **ポートの二重性**（重要）:
  - `DB_HOST_PORT`（=`5432+N`）: **ホスト公開ポート**。compose の publish と、ホスト実行の `go test`
    が繋ぐ先（`internal/config` のテスト設定は `DB_HOST_PORT` を参照）。
  - `DB_PORT`（=`5432` 固定、`env/.env` 由来）: **コンテナ内部**の接続ポート。`go_tool_runner` 内の
    migrate/seed/gen が `database:5432` へ繋ぐ。`DB_HOST_PORT` とは別物で、export して混同させない。
...
```sh
make db-acquire      # 空きスロットをリースし DB 起動 + スキーマ再構築（この後 test/db-init は同一スロット）
make test            # ホストから DB_HOST_PORT 経由で接続
make db-pool-status  # スロット占有状況を表示
make db-release      # スロットを解放（コンテナは warm 保持）
```

`make db-acquire` を実行しなければ、従来どおりホスト 5432 / ディレクトリ由来プロジェクトで単独動作する
（プールは opt-in）。
