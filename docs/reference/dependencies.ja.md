...
- 依存採用の**ポリシー**（一責務 = 一関心）は決定であり ADR です:
  [`ADR-0075`](../adr/0075-library-selection-policy.ja.md)。
- 2 つの上流にまたがる **bridge / instrumentation** ライブラリは、そのポリシーの
  境界のある例外として受容します: [`ADR-0076`](../adr/0076-bridge-instrumentation-exceptions.ja.md)。
...
| 領域 | ライブラリ | 責務 |
| --- | --- | --- |
| Web / API | `labstack/echo/v5` | HTTP web フレームワーク（[ADR-0020](../adr/0020-echo-http-framework.ja.md) 参照） |
...
| Errors / utils | `google/uuid` | UUID 生成（UUIDv7、[ADR-0035](../adr/0035-uuidv7-identifiers.ja.md) 参照） |
...
| DI / logging / CLI | `go.uber.org/fx` | DI コンテナ（[ADR-0038](../adr/0038-uber-fx-di.ja.md) 参照） |
...
| Messaging / worker | `aws/aws-sdk-go-v2/service/sqs` | SQS クライアント（pull-ack worker）。配線は削除可能なサンプル群からのみ — [ADR-0051](../adr/0051-broker-sdk-isolation-measured-as-coupling.ja.md) 参照 |
...
typed な `OBS_*` config から明示的に構築されます（[ADR-0069](../adr/0069-config-driven-observability-gating.ja.md) 参照）。

## bridge / instrumentation 例外

以下は**独立にバージョニングされる 2 つの上流**（フレームワーク/ライブラリ × OpenTelemetry）に
またがるため「一関心・一上流」から外れ、[ADR-0076](../adr/0076-bridge-instrumentation-exceptions.ja.md)
に基づき境界のある例外として受容します。
...
- 以前この依存表は `docs/decisions.md` にインラインでしたが、ドリフトしていました
  ...
  **ポリシー**（[ADR-0075](../adr/0075-library-selection-policy.ja.md)）から分離した理由がこれで、
