...
- 依存採用の**ポリシー**（一責務 = 一関心）は決定であり ADR です:
  [`ADR-0077`](../adr/0077-library-selection-policy.md)。
- 2 つの上流にまたがる **bridge / instrumentation** ライブラリは、そのポリシーの
  境界のある例外として受容します: [`ADR-0078`](../adr/0078-bridge-instrumentation-exceptions.md)。
...
| 領域 | ライブラリ | 責務 |
| --- | --- | --- |
| Web / API | `labstack/echo/v5` | HTTP web フレームワーク（[ADR-0022](../adr/0022-echo-http-framework.md) 参照） |
...
| Errors / utils | `google/uuid` | UUID 生成（UUIDv7、[ADR-0037](../adr/0037-uuidv7-identifiers.md) 参照） |
...
| DI / logging / CLI | `go.uber.org/fx` | DI コンテナ（[ADR-0040](../adr/0040-uber-fx-di.md) 参照） |
...
| Messaging / worker | `aws/aws-sdk-go-v2/service/sqs` | SQS クライアント（pull-ack worker）。配線は削除可能なサンプル群からのみ — [ADR-0053](../adr/0053-broker-sdk-isolation-measured-as-coupling.md) 参照 |
...
typed な `OBS_*` config から明示的に構築されます（[ADR-0071](../adr/0071-config-driven-observability-gating.md) 参照）。

## bridge / instrumentation 例外

以下は**独立にバージョニングされる 2 つの上流**（フレームワーク/ライブラリ × OpenTelemetry）に
またがるため「一関心・一上流」から外れ、[ADR-0078](../adr/0078-bridge-instrumentation-exceptions.md)
に基づき境界のある例外として受容します。
...
- 以前この依存表は `docs/decisions.md` にインラインでしたが、ドリフトしていました
  ...
  **ポリシー**（[ADR-0077](../adr/0077-library-selection-policy.md)）から分離した理由がこれで、
