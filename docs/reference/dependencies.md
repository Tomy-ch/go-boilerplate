...
- The **policy** for adopting a dependency (one responsibility = one concern) is a decision:
  [`ADR-0073`](../adr/0073-library-selection-policy.md).
- **Bridge / instrumentation** libraries that straddle two upstreams are accepted as bounded
  exceptions to that policy: [`ADR-0074`](../adr/0074-bridge-instrumentation-exceptions.md).
...
| Errors / utils | `google/uuid` | UUID generation (UUIDv7, see [ADR-0033](../adr/0033-uuidv7-identifiers.md)) |
...
| DI / logging / CLI | `go.uber.org/fx` | Dependency injection container (see [ADR-0036](../adr/0036-uber-fx-di.md)) |
...
| Messaging / worker | `aws/aws-sdk-go-v2/service/sqs` | SQS client (pull-ack worker). Wired only from the removable sample set — see [ADR-0049](../adr/0049-broker-sdk-isolation-verified-after-sample-removal.md) |
...
(see [ADR-0067](../adr/0067-config-driven-observability-gating.md)).
...
exceptions per [ADR-0074](../adr/0074-bridge-instrumentation-exceptions.md).
...
- Previously the dependency table lived inline in `docs/decisions.md`; it drifted (the
  ...
  *inventory* (this file) from the *policy* ([ADR-0073](../adr/0073-library-selection-policy.md))
