...
- The UUID path param's format is already validated by the echo binding layer (a malformed UUID is rejected with 400 **before** the handler runs), so at this point the value is guaranteed valid and the conversion never fails — `conv.UUID` therefore returns no error.
- `conv` reuses the existing validated `uuid.Parse`; we deliberately do **not** add a byte-level constructor to `pkg/uuid`, because a public validation-bypassing entry point there would erode over time and get misused across layers.
- Because only the controller layer imports the OpenAPI generated types, routing the conversion through `internal/controller/conv` confines it to the request boundary — usecase / domain cannot reach it.
- Internally `conv` asserts the invariant with a `panic` on the (unreachable) parse failure: if it ever fires, that is a catastrophic broken-precondition bug, not a normal error to swallow. The panic is unit-tested via a string-input helper, so coverage stays complete.
