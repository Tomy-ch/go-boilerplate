...
# ADR-0069: 運用スクリプトは scripts/ に Node（.mjs）または Go で配置し、シェルスクリプトは使用しない

English canonical: [0069-scripts-in-node-go.md](../../adr/0069-scripts-in-node-go.md)
...
ファイル操作が十分に複雑であり、シェルはメンテナンス上の負債となる。
...
- スクリプトを呼び出す Make ターゲットエントリポイント:
  [ADR-0068](0068-make-single-entrypoint.ja.md)。
