...
タグごとにスコープを絞るが、各ハンドラーメソッドは生の `echo.Context` を受け取り、自身でバインディングとシリアライズを行う必要がある。却下：すべてのハンドラーで同じボイラープレートを再現することになり、一貫性のないエラー処理の余地が残る。この処理を各ハンドラーに書いていくだけで全体の行数が著しく増加する問題もある。
