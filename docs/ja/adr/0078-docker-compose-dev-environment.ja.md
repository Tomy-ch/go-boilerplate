...
# ADR-0078: ローカル開発環境はプロファイルで分離されたサービスを持つ Docker Compose で提供する

English canonical: [0078-docker-compose-dev-environment.md](../../adr/0078-docker-compose-dev-environment.md)
...
ツール実行はマシン間で再現可能でなければならない（[ADR-0074](0074-containerized-pinned-toolchain.ja.md) 参照）。
...
- コンテナベース再現性の根拠:
  [ADR-0074](0074-containerized-pinned-toolchain.ja.md)。
