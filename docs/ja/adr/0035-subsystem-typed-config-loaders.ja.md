...
# ADR-0035: サブシステムスコープの envPrefix 型付き設定ローダー

English canonical: [0035-subsystem-typed-config-loaders.md](../../adr/0035-subsystem-typed-config-loaders.md)
...
- SubConfig プロバイダーパターンと不変性ルールは [ADR-0037](0037-immutable-fail-fast-config.ja.md) に記録されている。
- フィールドに `envDefault` と `required` のどちらを使用するかのガバナンスルールは [ADR-0036](0036-config-default-vs-required-governance.ja.md) に記録されている。
