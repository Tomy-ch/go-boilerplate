...
# ADR-0102: outbox relay はバランス型で出荷し、ハードニングは運用で得た事実に委ねる

English canonical: [0102-outbox-relay-hardening-delegated.md](../../adr/0102-outbox-relay-hardening-delegated.md)
...
mark する（[ADR-0050]、[ADR-0051]）。この種の設計はすべて不可能性の結果に縛られる: 外部副作用
...
ために可用性とコストを払うおそれがある — [ADR-0099] と [ADR-0100] と同じ論法である。
...
`Idempotency-Key` として伝搬（[ADR-0052]）し、受信側の同梱冪等性ミドルウェアが dedup するので、
...
2. **publish の tx 外化**: claim と mark は短 tx になる。tx 滞留・vacuum ピン留め・
   ...
   含まれなくなるため、relay は ADR-0030 の一般則を満たし、公認例外そのものが消滅する。
...
- 関連 ADR: [ADR-0030]（tx リトライ冪等性契約。出荷状態の relay はその唯一の公認例外であり、
  設計図の第 2 層がこの例外を除去する）、[ADR-0050]（at-least-once poll）、[ADR-0051]
  （SKIP LOCKED claim）、[ADR-0052]（message-id / Idempotency-Key 伝搬）、[ADR-0053]
  （max attempts 到達での dead 化）。
- ADR 全体の一覧と順序: [ADR ログ](README.ja.md)。

[ADR-0030]: 0030-transaction-retry-idempotent-callers.ja.md
[ADR-0050]: 0050-at-least-once-outbox-poll.ja.md
[ADR-0051]: 0051-skip-locked-outbox-relay.ja.md
[ADR-0052]: 0052-message-id-idempotency-propagation.ja.md
[ADR-0053]: 0053-outbox-dead-after-max-attempts.ja.md
[ADR-0099]: 0099-no-in-app-rate-limiter.ja.md
[ADR-0100]: 0100-scheduled-job-concurrency-delegated.ja.md
