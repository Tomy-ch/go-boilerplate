...
# ADR-0046: ブローカー非依存のプル・アック型ワーカースキャフォールド

English canonical: [0046-broker-agnostic-worker-scaffold.md](../../adr/0046-broker-agnostic-worker-scaffold.md)
...
- ポートは意図的に狭い（プル・アック型のみ）。プッシュ型とストリーミングログモデルは適合しないため除外されている（ADR-0047 として別途記録、Phase 4 で具体化）。
...
却下: このリポジトリに前例がなく、シングルバイナリではモジュール分離が不十分。`cmd` からアダプターをインポートしないことで、タグなしで分離が達成できる（[ADR-0048](0048-sqs-adapter-opt-in.ja.md) を参照）。

## 補足

- プッシュ/ストリーミング除外: ADR-0047（Phase 4）。SQS アダプターのオプトイン分離: [ADR-0048](0048-sqs-adapter-opt-in.ja.md)。
