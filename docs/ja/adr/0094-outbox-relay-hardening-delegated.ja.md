...
# ADR-0094: outbox relay の重複窓ハードニングは本番コピー側の責務とする

English canonical: [0094-outbox-relay-hardening-delegated.md](../../adr/0094-outbox-relay-hardening-delegated.md)
...
mark する（[ADR-0043]、[ADR-0044]）。この種の設計はすべて不可能性の結果に縛られる: 外部副作用
...
2. **publish の tx 外化**: claim と mark は短 tx になる。tx 滞留・vacuum ピン留め・
   ...
   含まれなくなるため、relay は ADR-0029 の一般則を満たし、公認例外そのものが消滅する。
...
- 関連 ADR: [ADR-0029]（tx リトライ冪等性契約。出荷状態の relay はその唯一の公認例外であり、
  設計図の第 2 層がこの例外を除去する）、[ADR-0043]（at-least-once poll）、[ADR-0044]
  （SKIP LOCKED claim）、[ADR-0045]（message-id / Idempotency-Key 伝搬）、[ADR-0046]
  （max attempts 到達での dead 化）。
- ADR 全体の一覧と順序: [ADR ログ](README.ja.md)。

[ADR-0029]: 0029-transaction-retry-idempotent-callers.ja.md
[ADR-0043]: 0043-at-least-once-outbox-poll.ja.md
[ADR-0044]: 0044-skip-locked-outbox-relay.ja.md
[ADR-0045]: 0045-message-id-idempotency-propagation.ja.md
[ADR-0046]: 0046-outbox-dead-after-max-attempts.ja.md
[ADR-0091]: 0091-no-in-app-rate-limiter.ja.md
[ADR-0092]: 0092-scheduled-job-concurrency-delegated.ja.md
