...
# ADR-0039: サブシステムスコープの envPrefix 型付き設定ローダー

English canonical: [0039-subsystem-typed-config-loaders.md](../../adr/0039-subsystem-typed-config-loaders.md)
...
- SubConfig プロバイダーパターンと不変性ルールは [ADR-0041](0041-immutable-fail-fast-config.ja.md) に記録されている。
- フィールドに `envDefault` と `required` のどちらを使用するかのガバナンスルールは [ADR-0040](0040-config-default-vs-required-governance.ja.md) に記録されている。
