...
# ADR-0096: 環境別イメージ（.env マトリックス × APP_ENV ビルド引数、ビルド時に固定）

English canonical: [0096-per-environment-images.md](../../adr/0096-per-environment-images.md)
...
アプリケーションは `go:embed` を通じてビルド時にバイナリに埋め込まれた `.env` ファイルから設定を読み取る（ADR-0043 参照）。埋め込みファイルはコンパイル時にベイクインされるため、単一のバイナリは 1 つの環境の設定しか持てない。異なる環境の設定（プロダクション、ステージング、開発）はそのため別々のイメージを必要とする。
...
- 各イメージは独立して検証可能かつ監査可能である: 埋め込まれた設定は署名された成果物の一部である（[ADR-0098](0098-release-image-supply-chain.ja.md) 参照）。
...
- 埋め込み設定メカニズム自体は ADR-0043 に文書化されている。
