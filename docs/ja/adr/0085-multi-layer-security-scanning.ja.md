...
# ADR-0085: 多層セキュリティスキャン——報告とゲートを分離し、ハードニングされたランナー上で行う

English canonical: [0085-multi-layer-security-scanning.md](../../adr/0085-multi-layer-security-scanning.md)
...
[ADR-0086](0086-sha-pinned-actions.ja.md) の SHA pin と隔離期間を通す。
...
- ここに挙げたスキャナのうち pre-commit でも走るのは zizmor だけ。フックと CI が呼ぶ `make`
  ...
  （[ADR-0081](0081-local-hooks-mirror-ci.ja.md)）。開発者が
...
- 追加したアクションが通るサプライチェーンの隔離期間とアクション pin:
  [ADR-0086](0086-sha-pinned-actions.ja.md)
- これらのスキャンとは別の関心事であるリリースイメージの完全性:
  [ADR-0097](0097-release-image-supply-chain.ja.md)
- `.golangci-full.yaml` の `gosec` linter は静的解析時のインプロセスチェックとして追加的に機能する
  （[ADR-0080](0080-two-layer-golangci-config.ja.md)）
