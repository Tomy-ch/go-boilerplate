...
# ADR-0035: ガバナンス: コードデフォルト（不変）対ファイル必須（可変）

English canonical: [0035-config-default-vs-required-governance.md](../../adr/0035-config-default-vs-required-governance.md)
...
**ファイル必須（可変）:** `envspec.go` で `required` とマークされたフィールドには埋め込みデフォルトがない。このような変数はすべて `env/.env`（ローカルデフォルト）とすべてのパーエンバイロメントファイル（`env/.env.<env>`）に存在しなければならない。必須フィールドが欠けている場合、`env.ParseAs` は失敗して起動を中断する（[ADR-0036](0036-immutable-fail-fast-config.ja.md) のフェイルファスト原則に沿う）。
...
- フェイルファスト起動の動作: [ADR-0036](0036-immutable-fail-fast-config.ja.md)。
- サブシステム構造体のレイアウト: [ADR-0034](0034-subsystem-typed-config-loaders.ja.md)。
