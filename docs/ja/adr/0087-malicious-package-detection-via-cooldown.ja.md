...
# ADR-0087: 悪意あるパッケージへの主防御として専用の検出器を持たず公開クールダウンを受け入れる

English canonical: [0087-malicious-package-detection-via-cooldown.md](../../adr/0087-malicious-package-detection-via-cooldown.md)
...
却下。いずれも依存グラフをベンダーへアップロードすることを要求し、有償の外部依存を CI に持ち込む。ADR-0001 は、採用者が同じベンダー契約を持つと仮定できないテンプレートに対して、この種のロックインを排除している。
...
- 関連: [ADR-0086](0086-sha-pinned-actions.ja.md)（SHA ピン留め）、[ADR-0085](0085-multi-layer-security-scanning.ja.md)（これが隣り合うスキャンのレイヤー群）、[ADR-0001](0001-avoid-lock-in.ja.md)（SaaS 検出器が対象外である理由）。
