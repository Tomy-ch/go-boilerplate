...
# ADR-0043: go:embed で設定（.env）とマイグレーションをバンドルし、自己完結型バイナリを実現する

English canonical: [0043-embedded-self-contained-binary.md](../../adr/0043-embedded-self-contained-binary.md)
...
- `FS` を消費する設定ロードシーケンスは [ADR-0042](0042-immutable-fail-fast-config.ja.md) に記載されている。
