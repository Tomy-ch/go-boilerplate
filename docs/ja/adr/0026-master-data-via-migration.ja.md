...
# ADR-0026: マスターデータをマイグレーション経由で投入する。トランザクショナルシードを本番から除外する

English canonical: [0026-master-data-via-migration.md](../../adr/0026-master-data-via-migration.md)
...
- マスターデータの値を修正する（例：都道府県名のスペルミス）には、[ADR-0024](0024-append-only-immutable-migrations.ja.md)と一致して単純な行更新ではなく新しいマイグレーションファイルが必要である。
