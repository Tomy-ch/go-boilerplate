...
# ADR-0034: 量を 2 つのスケールで保持する — 精度は正確な十進、決済は最小単位の整数

English canonical: [0034-two-scale-quantity-model.md](../../adr/0034-two-scale-quantity-model.md)
...
**正確なスケールのデータベース列は、精度もスケールも指定しない `NUMERIC` とする。** スケールは値の性質であって設計時の定数ではないため、スキーマは、未知の将来の通貨に対して正当化できない通貨固有の十進指数を主張しない。決済スケールは素の整数列である。
...
却下: 通貨固有の十進指数を設計時の定数としてスキーマに焼き込むことになり、「スケールは値の性質」と矛盾し、見たこともない通貨に対して正当化できない前提をボイラープレートに背負わせる。
...
- 共有される業務意味を持つ金額の値オブジェクトの置き場所: [ADR-0035](0035-domain-lexicon.ja.md)。
