...
# ADR-0099: docs/ の静的コンテンツを GitHub Pages で公開（production プッシュ時にリリース）

English canonical: [0099-docs-via-github-pages.md](../../adr/0099-docs-via-github-pages.md)
...
このリポジトリは `docs/` ディレクトリをアーキテクチャドキュメント・設計上の意思決定・API リファレンス・ガイドの正典として管理している（ADR-0008 参照）。このコンテンツは、生の Markdown としてリポジトリで閲覧するだけでなく、レンダリングされた形式でブラウズできると、コントリビューターや読み手にとって価値が高い。
