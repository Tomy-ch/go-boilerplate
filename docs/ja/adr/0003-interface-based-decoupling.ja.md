...
English canonical: [0003-interface-based-decoupling.md](../../adr/0003-interface-based-decoupling.md)
...
- DI 解決（ADR-0030 参照）は起動時に実装をインターフェースに結線するため、クロスレイヤーの継ぎ目は DI コンテナがインジェクトできるインターフェースでなければならない。
...
- **Boundary インターフェース**（クロック、乱数、アウトボックス、べき等性）は `internal/usecase/boundary` で宣言され、インフラストラクチャが実装を提供する。
...
- レイヤーローカルな型間のマッピング（ドメインエンティティから DTO、DTO から OpenAPI レスポンス）が各境界横断でボイラープレートを追加する。
...
- 起動時に実装をインターフェースに結線する DI コンテナ: ADR-0030。
