...
# ADR-0087: 環境別イメージ（.env マトリックス × APP_ENV ビルド引数、ビルド時に固定）

English canonical: [0087-per-environment-images.md](../../adr/0087-per-environment-images.md)
...
- 各イメージは独立して検証可能かつ監査可能である: 埋め込まれた設定は署名された成果物の一部である（[ADR-0089](0089-release-image-supply-chain.ja.md) 参照）。
