...
- インターフェースは**ドメインレイヤー**で定義する（`internal/domain/<aggregate>/`）。Repository インターフェースと同じ場所である。CommandService が担うのは集約では表現できない書き込みであり、そこで強制する条件は当の集約の Invariant から導出される。したがって契約はドメインの語彙で述べられ、所有者もドメインになる。読み手が何を読みたいかで形が決まる QueryService とは、この点が異なる。
...
- インターフェースの所有は「形を決めるのは誰か」に従う：読み込みモデルはユースケースの関心事なので QueryService インターフェースはユースケースレイヤーに、CommandService は集約の Invariant から導出した条件を強制するのでそのインターフェースは Repository と並んでドメインレイヤーに置く。
...
- QueryService インターフェースはユースケースレイヤーにありドメインからより遠いため、ドメインコードを単独で読む際に意図が分かりにくくなることがある。
- ドメインレイヤーが永続化インターフェースを 2 つ宣言することになり、「書き込みがストレージへ届く経路は Repository だけ」がドメインパッケージを読むだけでは成り立たなくなる。後述の適格基準が、2 つ目が汎用の逃げ道になることを防ぐ役割を負う。
