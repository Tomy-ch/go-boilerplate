...
# ADR-0063: ジョブ起動ごとに新しい fx.App を構築する（ワンショットライフサイクル）

English canonical: [0063-job-fresh-fx-app-per-run.md](../../adr/0063-job-fresh-fx-app-per-run.md)
...
ジョブサブシステムはユースケースレイヤーへの CLI 駆動エントリポイントであり、HTTP ハンドラーやワーカーと同等だが、一度だけ呼び出されて終了することが期待される。常駐の `serve` コマンドや `worker` コマンドと同じ Uber Fx 依存性注入コンテナを使用する（[ADR-0035](0035-uber-fx-di.ja.md) 参照）。
...
- 関連: [ADR-0035](0035-uber-fx-di.ja.md)（Uber Fx DI）。
