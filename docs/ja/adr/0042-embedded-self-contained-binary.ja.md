...
# ADR-0042: go:embed で設定（.env）とマイグレーションをバンドルし、自己完結型バイナリを実現する

English canonical: [0042-embedded-self-contained-binary.md](../../adr/0042-embedded-self-contained-binary.md)
...
- `FS` を消費する設定ロードシーケンスは [ADR-0041](0041-immutable-fail-fast-config.ja.md) に記載されている。
