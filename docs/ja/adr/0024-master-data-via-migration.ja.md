...
# ADR-0024: マスターデータをマイグレーション経由で投入する。トランザクショナルシードを本番から除外する

English canonical: [0024-master-data-via-migration.md](../../adr/0024-master-data-via-migration.md)
...
1. **マスターデータ** — アプリケーションがいかなる環境でも正しく機能するために必要な参照テーブル（都道府県・商品ステータス・購入ステータス）。これらの行は安定していて列挙可能であり、本番環境に必要である。
2. **トランザクショナルシードデータ** — 開発・テスト環境のみで必要なユーザー・商品・デモ注文。これらの行は本番環境に絶対に到達してはならない。
...
**マスターデータ**は対応するマイグレーションファイル内のテーブルDDLと一緒に挿入される。べき等性のために`INSERT ... ON CONFLICT (id) DO NOTHING`を使用する。例：

- `000003_create_prefectures.up.sql` — `CREATE TABLE`文と一緒に日本の47都道府県すべてを挿入する。
- `000005_create_product_statuses.up.sql` — 初期の商品ステータス行を挿入する。
- `000008_create_purchase_statuses.up.sql` — 初期の購入ステータス行を挿入する。
...
- マスターデータの値を修正する（例：都道府県名のスペルミス）には、[ADR-0022](0022-append-only-immutable-migrations.ja.md)と一致して単純な行更新ではなく新しいマイグレーションファイルが必要である。
...
- 関連: [ADR-0022](0022-append-only-immutable-migrations.ja.md)（不変性）；[ADR-0023](0023-sequential-migration-ids.ja.md)（採番）。
