...
# ADR-0086: 悪意あるパッケージへの主防御として専用の検出器を持たず公開クールダウンを受け入れる

English canonical: [0086-malicious-package-detection-via-cooldown.md](../../adr/0086-malicious-package-detection-via-cooldown.md)
...
- 関連: [ADR-0085](0085-sha-pinned-actions.ja.md)（SHA ピン留め）、[ADR-0084](0084-multi-layer-security-scanning.ja.md)（これが隣り合うスキャンのレイヤー群）、[ADR-0001](0001-avoid-lock-in.ja.md)（SaaS 検出器が対象外である理由）。
