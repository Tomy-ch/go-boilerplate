...
# ADR-0071: ローカル開発環境はプロファイルで分離されたサービスを持つ Docker Compose で提供する

English canonical: [0071-docker-compose-dev-environment.md](../../adr/0071-docker-compose-dev-environment.md)
...
ツール実行はマシン間で再現可能でなければならない（[ADR-0067](0067-containerized-pinned-toolchain.ja.md) 参照）。
...
- コンテナベース再現性の根拠:
  [ADR-0067](0067-containerized-pinned-toolchain.ja.md)。
