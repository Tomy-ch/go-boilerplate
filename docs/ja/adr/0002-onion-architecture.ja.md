...
このテンプレートは、インフラストラクチャとフレームワークの選択からビジネスロジックを独立させる必要がある。インフラストラクチャが交換可能（[ADR-0001](0001-avoid-lock-in.ja.md) 参照）であり、ドメインコアが長期にわたって安定してテスト可能であり続けるためである。主要な設計目標は、保守性・構造的安全性・型安全性・交換可能なインフラストラクチャ・長期的な運用性であり、生のパフォーマンスや最小限の抽象化ではない。
...
シンプルだが、ドメインロジックとインフラストラクチャロジックが混在しがちで、このプロジェクトが優先する安定したコアが侵食される。
