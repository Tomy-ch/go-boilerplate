...
# ADR-0037: 設定は不変、起動時に 1 回だけロード、フェイルファスト

English canonical: [0037-immutable-fail-fast-config.md](../../adr/0037-immutable-fail-fast-config.md)
...
`validateConfig()` は制約（ポート範囲、CIDR フォーマット、タイムアウトの順序、bcrypt コストの境界など）を適用し、制約に違反した場合は即座にエラーを返す。アプリケーションは検証エラーや必須フィールドの欠落があれば、劣化または誤設定の状態で動作するのではなく、**起動に失敗する**。
...
- デフォルト対必須のガバナンス: [ADR-0036](0036-config-default-vs-required-governance.ja.md)。
- 埋め込み env ファイルの仕組み: [ADR-0038](0038-embedded-self-contained-binary.ja.md)。
