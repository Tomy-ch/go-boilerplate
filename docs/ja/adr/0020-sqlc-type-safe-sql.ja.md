...
# ADR-0020: sqlcによる型安全なSQLアクセスの生成

English canonical: [0020-sqlc-type-safe-sql.md](../../adr/0020-sqlc-type-safe-sql.md)
...
SQLファーストの方針（[ADR-0019](0019-sql-first-data-access.ja.md)）を前提とすると、手書きのSQLをGoに**コンパイル時の型安全な**コードとして届ける必要があるが、ORMのランタイム抽象や暗黙的なクエリ生成を再導入したくない。
...
- [ADR-0019](0019-sql-first-data-access.ja.md)（SQLファースト）を前提とする。
- 生成コードは手動編集不可 — [`docs/rules.md`](../../rules.md#generated-code-rules)の生成コードルール参照。
