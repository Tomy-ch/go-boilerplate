...
# ADR-0013: oapi-codegen の strict-server モードでタグ/ハンドラーごとに生成する

English canonical: [0013-oapi-codegen-strict-server.md](../../adr/0013-oapi-codegen-strict-server.md)
...
- 再生成にはバンドルされた `openapi.gen.yaml` が先に存在する必要がある（[ADR-0012](0012-redocly-modular-spec-pipeline.ja.md) 参照）。
...
- 仕様バンドルの前提条件: [ADR-0012](0012-redocly-modular-spec-pipeline.ja.md)。
