...
# ADR-0084: すべてのロールを 1 つのマルチコマンドバイナリに集約する

English canonical: [0084-single-multi-command-binary.md](../../adr/0084-single-multi-command-binary.md)
...
- バージョン管理、プッシュ、プルのための 1 つのビルド成果物。同じイメージをコマンドオーバーライドで任意のロールとして実行できる（[ADR-0085](0085-single-runtime-image.ja.md) 参照）。
...
- コマンドロジックは [ADR-0083](0083-cli-humble-object-split.ja.md) に従って `internal/cli/<command>/` に置かれる。
