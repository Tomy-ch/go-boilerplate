...
3 つのスケジュール（one-shot）ジョブ（`outbox-gc`、`idempotency-gc`、`usercount`）と、常駐する outbox リレープロセス（スケジュールジョブではなく常駐エンジン。[ADR-0057](0057-relay-resident-gc-oneshot.ja.md) 参照）を含んでいる。各々はアプリケーションレベルのロックなしに並行安全となるよう設計されている:
