...
# ADR-0078: 運用スクリプトは scripts/ に Node（.mjs）または Go で配置し、シェルスクリプトは使用しない

English canonical: [0078-scripts-in-node-go.md](../../adr/0078-scripts-in-node-go.md)
...
- スクリプトを呼び出す Make ターゲットエントリポイント:
  [ADR-0077](0077-make-single-entrypoint.ja.md)。
