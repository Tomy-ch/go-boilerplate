...
これは長期にわたる **テンプレート** リポジトリであり、その設計目標として交換可能なインフラストラクチャと長期的な運用性が掲げられている。二つの形態の囲い込みがその目標を脅かす:
...
下流のユーザーはこのテンプレートを予測できないベンダー環境にフォークするため、囲い込みは理論上のリスクではなく具体的なリスクである。
...
- フォークはドメイン/ユースケースに触れることなく、クラウド・ブローカー・テレメトリのバックエンドを切り替えられる。
...
却下: 初期は最速だが、ビジネスコードを特定プロバイダーの API と料金体系に結合させ、テンプレートの交換可能なインフラという目標を損なう。
