...
# ADR-0103: outbox relay はバランス型で出荷し、ハードニングは運用で得た事実に委ねる
...
ために可用性とコストを払うおそれがある — [ADR-0100] と [ADR-0101] と同じ論法である。
...
`Idempotency-Key` として伝搬（[ADR-0053]）し、受信側の同梱冪等性ミドルウェアが dedup するので、
