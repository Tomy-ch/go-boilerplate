...
# ADR-0041: スキーマ分割によるエラー details の opt-in ゲート

English canonical: [0041-error-details-opt-in-gate.md](../../adr/0041-error-details-opt-in-gate.md)

## ステータス

accepted

[ADR-0040](0040-error-metadata-code-message-details.ja.md) を精緻化する。

## コンテキスト

[ADR-0040](0040-error-metadata-code-message-details.ja.md) は `apperror.Meta` を導入し、エラー
...
2. **builder 型。** 型生成用エンドポイント(`GenerateErrorSchema`)が `ErrorResponseWithDetails`
   ...
   (builder は ADR-0040 どおり request 非依存のまま)。
