...
# ADR-0068: Make を単一のツールエントリポイントとし、.mk 登録とセルフドキュメンティングなヘルプを提供する

English canonical: [0068-make-single-entrypoint.md](../../adr/0068-make-single-entrypoint.md)
...
- 新しい `.mk` ファイルはトップレベルインクルードと `make_help.mjs` によって自動的に認識される。
  `.makefiles/` にファイルを配置する以上の登録ステップは不要である。
- CI、git フック、スキルが 1 つの安定したエントリポイントを共有する。
