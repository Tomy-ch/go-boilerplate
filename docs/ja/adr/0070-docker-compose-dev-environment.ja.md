...
# ADR-0070: ローカル開発環境はプロファイルで分離されたサービスを持つ Docker Compose で提供する

English canonical: [0070-docker-compose-dev-environment.md](../../adr/0070-docker-compose-dev-environment.md)
...
ツール実行はマシン間で再現可能でなければならない（[ADR-0066](0066-containerized-pinned-toolchain.ja.md) 参照）。
...
- コンテナベース再現性の根拠:
  [ADR-0066](0066-containerized-pinned-toolchain.ja.md)。
