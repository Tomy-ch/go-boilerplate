...
# ADR-0015: OpenAPI はワイヤー契約であってドメインルールではない。リクエストはドメインのサブセット、ドメインはレスポンスのサブセット

English canonical: [0015-boundary-value-ownership.md](../../adr/0015-boundary-value-ownership.md)
...
- リクエスト制約はドメインルールより*厳しく*てよい。リクエスト検証ミドルウェア（[ADR-0013](0013-spec-driven-request-validation.ja.md) 参照）は範囲外の入力をドメインが見る前に拒否するため、より厳しいワイヤー制限は安全である。
