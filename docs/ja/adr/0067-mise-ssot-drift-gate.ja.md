...
# ADR-0067: mise.toml を単一の情報源とし、バージョンを下流に伝播させ CI でドリフトを検知する

English canonical: [0067-mise-ssot-drift-gate.md](../../adr/0067-mise-ssot-drift-gate.md)
...
- コントリビューターは `mise.toml` を編集した後に `make sync-versions` を実行して変更結果を
  コミットすることを忘れてはならない。そうしなければ CI ゲートが PR を拒否する。
