...
# ADR-0077: 運用スクリプトは scripts/ に TypeScript または Go で配置し、シェルスクリプトは使用しない
...
慣習ではない理由がここにある。5 本は CI から一度も実行されず、いずれも利用者自身のリポジトリを書き換える
——ツリー全体の Go モジュールパス、LICENSE の権利者、CODEOWNERS の全ルールの所有者欄。置換規則が
過剰にマッチしたりファイル種別を取りこぼしたりしたとき、それに気づくのは数分前にボイラープレートを
クローンした人であり、デバッグの文脈を何も持たず、自分のリポジトリを初めて眺めている場面である。
...
- `npm-cooldown/` / `go-cooldown/` / `mise-cooldown/` — 供給網 cooldown のゲート。
...
- セットアップスクリプトは 1 回限りの使用であり、初期セットアップ完了後もリポジトリに残り続け、継続的な価値なく
  ボリュームを増やす。
- 自消滅するセットアップツールはテストも道連れにするため、そのテストが守るのはこのボイラープレート自身の
  CI であって、そこから作られたリポジトリではない。これは正しい取引である——代替案は既に存在しない
  ツールのテストスイートを同梱することだ——が、保証が使用の瞬間で終わることは事実である。
