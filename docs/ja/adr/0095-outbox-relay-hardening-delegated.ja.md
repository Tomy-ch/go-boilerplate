...
# ADR-0095: outbox relay の重複窓ハードニングは本番コピー側の責務とする

English canonical: [0095-outbox-relay-hardening-delegated.md](../../adr/0095-outbox-relay-hardening-delegated.md)
...
mark する（[ADR-0044]、[ADR-0045]）。この種の設計はすべて不可能性の結果に縛られる: 外部副作用
...
- 関連 ADR: [ADR-0029]（tx リトライ冪等性契約。出荷状態の relay はその唯一の公認例外であり、
  設計図の第 2 層がこの例外を除去する）、[ADR-0044]（at-least-once poll）、[ADR-0045]
  （SKIP LOCKED claim）、[ADR-0046]（message-id / Idempotency-Key 伝搬）、[ADR-0047]
  （max attempts 到達での dead 化）。
- ADR 全体の一覧と順序: [ADR ログ](README.ja.md)。

[ADR-0029]: 0029-transaction-retry-idempotent-callers.ja.md
[ADR-0044]: 0044-at-least-once-outbox-poll.ja.md
[ADR-0045]: 0045-skip-locked-outbox-relay.ja.md
[ADR-0046]: 0046-message-id-idempotency-propagation.ja.md
[ADR-0047]: 0047-outbox-dead-after-max-attempts.ja.md
[ADR-0092]: 0092-no-in-app-rate-limiter.ja.md
[ADR-0093]: 0093-scheduled-job-concurrency-delegated.ja.md
