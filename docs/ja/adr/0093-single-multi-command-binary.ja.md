...
# ADR-0093: すべてのロールを 1 つのマルチコマンドバイナリに集約する

English canonical: [0093-single-multi-command-binary.md](../../adr/0093-single-multi-command-binary.md)
...
- バージョン管理、プッシュ、プルのための 1 つのビルド成果物。同じイメージをコマンドオーバーライドで任意のロールとして実行できる（[ADR-0094](0094-single-runtime-image.ja.md) 参照）。
...
- コマンドロジックは [ADR-0092](0092-cli-humble-object-split.ja.md) に従って `internal/cli/<command>/` に置かれる。
