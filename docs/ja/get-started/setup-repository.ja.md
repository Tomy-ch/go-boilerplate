...
**スキャンが到達できる先に結びついた値が 2 つあり、そこは黙らず落ちる作りにしてあります。** ワークフローは ZAP を起動する前に、保護されたオペレーション 1 つ（`PROBE_PATH`）を 1 つのアイデンティティ（`SCAN_SUBJECT`）で叩き、資格情報が受理されることを確かめます。そのオペレーションまたはアイデンティティが存在しなくなったとき — 同梱 API の削除（Phase 15）がまさにそれです — 両方を自分のものへ向け直すまで実行は赤くなります。これは意図した挙動です。対象が消えた認証確認をそのままにすると、スキャンが 401 しか見ていないのに緑を報告し続けることになります。
