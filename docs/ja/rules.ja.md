...
- レスポンスで動的なエラー `code` / `details` を返す場合は、エラー発生箇所で `apperror.Meta` を付与する（`apperror.WithMeta` / `WithDetails`）。`Meta` は HTTP ステータスを運ばず、ステータスはセンチネル分類のみで解決する。`Details` には公開して安全な識別子のみ（例: 不正フィールド名）を入れ、理由文や入力値そのものを入れてはならない。理由はラップしたエラーメッセージ側に残し、ログ専用とする。理由: [ADR-0039](adr/0039-error-metadata-code-message-details.ja.md)。

## コメントルール

- コメントは **What** と **Why** を述べ、**How** は述べない:
  - **What（契約）**: その宣言が何をするか、入出力・エラーの意味。呼び出し側が依拠する情報。
  - **Why（非自明な理由）**: コードからは読み取れない判断の理由（制約・意図・非自明なトレードオフ）。非自明な場合は推奨。
  - **How はコードに委ねる**: 逐次の「どうやるか」や実装手段を書かない。What があれば実装はコードから読める。
- **What は「あるだけ」でなく品質が必要**（`revive` は有無/形式しか見ない）:
  - **正確**: 実挙動と一致していること。コードと食い違う/陳腐化した What は無コメントより有害＝最優先の指摘。
  - **充足**: 非自明な契約（エラー意味論、nil/ゼロ値の挙動、単位、境界、副作用）を網羅。呼び出し側が推測できないことを省かない。
  - **有意**: 識別子の単なる反復を超えた情報を持つこと。名前の言い換えだけは低価値。（ただし真に自明な export 宣言では簡潔な最小 What で可 — `revive` が doc を必須化するため、非自明な情報を*足せるのに足していない*場合のみ指摘する。）
- **Why は「良い Why」であること**:
  - OK（非自明な理由 / load-bearing 制約）: `// upstream がバースト時にレート制限するため 3 回までリトライする`、`runtime.Caller` のスキップ段数の警告（「このヘルパーを抽出するな — スキップ段数がずれる」）。
  - NG（開発の経緯 / メタ）: 移行履歴、障害の後日談、「なぜ移行したか」、`// テスト容易性のため`、`// 〜の登録は di 層が担う` 等のメタな組織事情 — これらは腐るので PR / commit ログに置き、コードに残さない。
