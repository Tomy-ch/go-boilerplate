...
- **README は子ディレクトリを列挙する。ファイルツリーは置かない。** ファイルまで並べた木はエディタが
  既に見せているものの再掲であり、しかも腐る（このリポジトリでも、実在するディレクトリが抜けた木や、
  実在しないディレクトリを載せた木が繰り返し見つかっている）。エディタに出せないのは「なぜそのディレクトリが
  あるか」なので、親の README は**子ディレクトリ 1 つにつき 1 行**（名前と用途）を持ち、それより下は持たない。
  中身を知りたければそのディレクトリの README を開く。**したがってディレクトリを増やすときの義務は 1 つだけ
  — 親の README にその名前があるか確かめること。** ファイルは頻繁に変わるので、どこにも列挙しない。
