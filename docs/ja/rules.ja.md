...
domain 内部を横断してよい経路は lexicon だけではありません。集約に跨るルールはどちらの集約の責務でも
ないため **`internal/domain/service/<name>/`** に置き、このパスは集約の import を許す専用の depguard
...
Usecase が読み込み済みの状態を受け取り、ドメインのエラーを返します。

2 つの例外は別々の問いに答えるものであり、互換ではありません。lexicon が受け入れるのは複数の集約が
話す **値オブジェクト** であり、ドメインサービスが持つのはどの単一集約でも判定できない **ルール** です。
片方の集約に収まるルールはその集約へ置き、2 つの集約を読んで並べるだけなら写像であり Usecase に
留まります。入場基準と現在の占有者は
[`internal/domain/README.ja.md`](../../internal/domain/README.ja.md) の「集約横断の Domain Service を
どこに置くか」を参照してください。
