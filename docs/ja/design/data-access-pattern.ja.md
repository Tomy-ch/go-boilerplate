...
[English](../../design/data-access-pattern.md) | 日本語

> このファイルは canonical な英語版 [data-access-pattern.md](../../design/data-access-pattern.md) の翻訳です。直接編集せず、更新は英語版から反映してください。
...
[ADR-0029 (lightweight-cqrs)](../adr/0029-lightweight-cqrs.ja.md)（構築物とその構造、CommandService の
eligibility と derivation の規則）、
[ADR-0030 (system-cqrs-dml-category)](../adr/0030-system-cqrs-dml-category.ja.md)（分割の外側にある
第 4 のカテゴリ）、
...
ドメイン集約との 1:1 対応が壊れる。[ADR-0030](../adr/0030-system-cqrs-dml-category.ja.md) を参照。DML は
...
1. **分解（既定）。** 他集約への帰結が結果整合でよく、そこから読んだ条件が陳腐化してよいなら、通常の
   ...
   （[ADR-0051](../adr/0051-transactional-outbox.ja.md)）。他の集約をトランザクション内に保持しない。
...
行ロックを取る（[ADR-0033](../adr/0033-ordered-pessimistic-row-locks.ja.md)）。規則が集約を跨ぐ場合は
...
[ADR-0031](../adr/0031-commandservice-atomicity-criterion.ja.md) の § Departure from "1 Aggregate = 1
