...
English: [README.md](../../design/README.md)

本ディレクトリは **サブシステム別の設計リファレンス**を収めています。各文書は 1 つのサブシステムの *役割論・状態遷移 / ライフサイクル・実装箇所・integrator が書く箇所・用語* を、**実装を精査して 1 枚に**まとめたものです。

これらはパッケージの `README` を置き換えるものではなく**補完**します。README は 1 パッケージの API / 概要、設計リファレンスはサブシステム全体を跨ぐ物語です。設計判断の ADR は [`docs/decisions.md`](../../decisions.md) にあります。
