...
it("TTLStore: take は単回使用で、2 回目は undefined を返す", () => {
  let now = 0;
...
it("TTLStore: get は値を保持し、繰り返し取得できる", () => {
  let now = 0;
  const store = new TTLStore<string>(1000, () => now);
...
it("TTLStore: TTL 経過後は get が undefined を返し、エントリを失効させる", () => {
  let now = 0;
  const store = new TTLStore<string>(1000, () => now);
  store.set("code", "value");
  now = 999;
...
it("TTLStore: 期限切れ後の take は undefined を返す", () => {
  let now = 0;
  const store = new TTLStore<string>(1000, () => now);
  store.set("code", "value");
  now = 2000;
...
it("TTLStore: sweep は期限切れのみを回収し、鮮度内は残す", () => {
  let now = 0;
  const store = new TTLStore<string>(1000, () => now);
  store.set("old", "1");
  now = 500;
...
it("TTLStore: 未知の key の delete は何も壊さない", () => {
  const store = new TTLStore<string>(1000, () => 0);
  store.set("a", "1");
...
it("sweepAll: 定期回収は鮮度内の code / session を落とさない", () => {
  resetAll();
  putLiveEntries();
  sweepAll();
...
it("resetAll: code / session 双方を初期化する", () => {
  putLiveEntries();
  resetAll();
...
it("TTLStore: deleteWhere は述語に一致しないエントリを残す", () => {
