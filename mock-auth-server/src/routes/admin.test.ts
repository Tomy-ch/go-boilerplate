...
it("正常系: admin/reset は揮発ストア（code / session）を初期化する", async () => {
  const app = createApp();
...
it("正常系: admin/reset は鍵ストアを Phase1（公開 = 署名 = PRIMARY_KID のみ）へ戻す", async () => {
  const app = createApp();
...
it("正常系: rotate の add-key → promote → retire で Phase1→2→3 を再現する", async () => {
  const app = createApp();
...
it("正常系: JWKS は公開集合の変化を反映する", async () => {
  const app = createApp();
...
it("異常系: 未知 kid の add-key は 400", async () => {
  const app = createApp();
...
it("契約: admin/keys/rotate は 501 スタブではなく実装済み（200）", async () => {
  const app = createApp();
  const [status] = await rotate(app, "add-key", ROTATION_KID);
  expect(status).toBe(200);
});

it("異常系: dev-gate 無効時は admin/reset を 404 で秘匿する", async () => {
  const app = createApp({ devEndpointsEnabled: false });
  const res = await app.request("/admin/reset", { method: "POST" });
  expect(res.status).toBe(404);
});

it("異常系: dev-gate 無効時は admin/keys/rotate を 404 で秘匿する", async () => {
  const app = createApp({ devEndpointsEnabled: false });
  const res = await app.request("/admin/keys/rotate", {
    method: "POST",
    headers: { "content-type": "application/json" },
...
it("異常系: admin/keys/rotate は JSON として壊れたボディを 400 で拒否する", async () => {
...
it("rotationFailure: RotationError は 400 応答ボディへ写像する", () => {
...
it("rotationFailure: RotationError 以外は握り潰さず投げ返す", () => {
  const unexpected = new Error("boom");
  expect(() => rotationFailure(unexpected)).toThrow(unexpected);
});

it("異常系: 空ボディの rotate は action を欠くものとして拒否する", async () => {
  const app = createApp();
