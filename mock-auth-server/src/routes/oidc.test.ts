...
describe("oidcRoutes", () => {
  describe("正常系", () => {
    it("openid を含まない scope では id_token を返さない", async () => {
      const app = createApp();
    ...
    it("authorize→token で access/id token を発行し claim が正しい", async () => {
      const app = createApp();
    ...
    it("userinfo は Bearer access token で whitelist フィールドのみ返す", async () => {
      const app = createApp();
    ...
    it("logout は登録済み post_logout_redirect_uri へ 302（state 反映）", async () => {
      const app = createApp();
    ...
    it("logout は post_logout_redirect_uri 未指定で 200 を返す", async () => {
      const app = createApp();
      const res = await logoutRequest(app, {});
      expect(res.status).toBe(200);
    ...
    it("bypass/session が session を作成し、logout(id_token_hint) が破棄する", async () => {
      const app = createApp();
    ...
    it("userinfo は fixture 外 subject に sub のみ返す", async () => {
      const app = createApp();
    ...
    it("subject を取り出せない id_token_hint でも logout は成功する", async () => {
      const app = createApp();
    ...
    it("state 無しの logout は redirect URI へそのまま 302 する", async () => {
      const app = createApp();
  ...
  describe("異常系", () => {
    it("code の再利用は 400 invalid_grant で拒否する", async () => {
      const app = createApp();
      const location = await issueCode(app);
      const code = location.searchParams.get("code") ?? "";
      const form = {
    ...
    it("code_verifier 不一致は 400 invalid_grant で拒否する", async () => {
      const app = createApp();
      const location = await issueCode(app);
      const res = await tokenRequest(app, {
        grant_type: "authorization_code",
    ...
    it("未登録 redirect_uri は 400（リダイレクト不能）で拒否する", async () => {
      const app = createApp();
      const res = await app.request(
        authorizePath({
          response_type: "code",
    ...
    it("code_challenge_method!=S256 は 302 error redirect（state を反映）", async () => {
      const app = createApp();
      const res = await app.request(
        authorizePath({
          response_type: "code",
    ...
    it("token で redirect_uri 不一致は 400 invalid_grant で拒否する", async () => {
      const app = createApp();
      const location = await issueCode(app);
      const res = await tokenRequest(app, {
        grant_type: "authorization_code",
    ...
    it("userinfo は ID Token を 401 で拒否する", async () => {
      const app = createApp();
      const { id_token } = await issueTokens(app);
      const res = await app.request("/oidc/userinfo", {
    ...
    it("userinfo は Bearer 欠如を 401 で拒否する", async () => {
      const app = createApp();
      const res = await app.request("/oidc/userinfo");
      expect(res.status).toBe(401);
    });

    it("logout は未登録 post_logout_redirect_uri を 400 で拒否する", async () => {
      const app = createApp();
      const res = await logoutRequest(app, { post_logout_redirect_uri: "http://evil.example.com" });
      expect(res.status).toBe(400);
    });

    it("authorize は response_type!=code を 302 error redirect で拒否する", async () => {
      const app = createApp();
      const res = await app.request(
        authorizePath({
          response_type: "token",
    ...
    it("authorize は scope 欠如を 302 error redirect で拒否する", async () => {
      const app = createApp();
      const res = await app.request(
        authorizePath({
          response_type: "code",
    ...
    it("authorize は code_challenge 欠如を 302 error redirect で拒否する", async () => {
      const app = createApp();
      const res = await app.request(
        authorizePath({
          response_type: "code",
    ...
    it("authorize は許可外 scope を 302 invalid_scope で拒否する", async () => {
      const app = createApp();
      const res = await app.request(
        authorizePath({
          response_type: "code",
    ...
    it("token は authorization_code 以外の grant_type を 400 で拒否する", async () => {
      const app = createApp();
      const location = await issueCode(app);
      const res = await tokenRequest(app, {
    ...
    it("userinfo は期限切れ access token を 401 で拒否する", async () => {
      const app = createApp();
    ...
    it("token は必須パラメータを欠いたリクエストを 400 で拒否する", async () => {
      const app = createApp();
    ...
    it("authorize は client_id が無いリクエストを 400 で拒否する", async () => {
      const app = createApp();
    ...
    it("userinfo は sub が文字列でない Token を空 sub に正規化する", async () => {
