...
|Variable Name|Description|Type|Example|Notes|
|---|---|---|---|---|
|SECURITY_ALLOWED_ORIGINS|CORS allow|csv|`*`|Per-environment value — `local` allows any origin, so a frontend dev server works on whichever port it defaults to: the bundled Grafana (`3000`) and DynamoDB Local (`8000`) already hold the ones it would otherwise take. `ci` carries the same value only so the two files do not differ gratuitously — nothing in CI serves a browser, so the value reaches no test. `AllowCredentials` is off (`internal/controller/httpstack/cors`), so `*` grants no credentialed access. `dast` and every deploy environment name their origin explicitly, which is the stricter path the DAST scan exercises|
...
- Wildcard origin guard: `local` and `ci` set `SECURITY_ALLOWED_ORIGINS=*`, and nothing about that value announces itself as local-only — `url.Parse("*")` yields a relative reference with no scheme, so the http-only-for-localhost check never sees it and a copy into a deploy file would boot happily. Config validation therefore rejects `*` outright when the effective `APP_MODE` is `production`, on the same mode axis as the embedded-env guard above. `development` mode passes, which is what keeps `local` / `ci` / `dev` working. The guard is on the value, not on the file, so it also catches a `*` injected at run time.
