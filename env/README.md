...
|REALTIME_TOPIC|Identifier (an ARN on AWS) of the topic carrying wakeups and revocation notices to every serve instance|string|`arn:aws:sns:us-east-1:000000000000:realtime-fanout-local`|Code default empty. A resource identifier, not a host, so it is not an `ENDPOINT_*`. Empty fails startup as soon as the fan-out is wired (same treatment as `ENDPOINT_OUTBOX`). Per-environment value — `local` / `ci` name the topic `make realtime-init` creates on GoAWS (the `ci` account id is the image default, `100010001000`, because a service container takes no config file); `dast` leaves it empty because the DAST job does not wire the fan-out (should a later phase do so, the `ci` value applies: DAST also runs against a GoAWS service container); from `dev` onward the value is **injected at deploy time** for the topic the deployment provisions|
