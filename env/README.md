...
The DynamoDB-compatible store behind Realtime Delivery ([`docs/design/realtime-delivery.md`](../docs/design/realtime-delivery.md)): the EventLog (bounded replay), the stream tickets and the instance leases. The usecase depends on the vendor-neutral `realtime` boundaries; the adapter is DynamoDB (AWS SDK v2), so `local` and `ci` connect to DynamoDB Local while deploy environments target AWS DynamoDB by leaving `ENDPOINT_REALTIME` empty. Table names are fixed (`realtime_event_log` / `realtime_stream_ticket` / `realtime_instance_lease`) with `_<REALTIME_TABLE_SUFFIX>` appended, so one DynamoDB (or one shared DynamoDB Local) can hold several environments side by side. Tables are created by the one-shot `make realtime-init`, never at application start.
