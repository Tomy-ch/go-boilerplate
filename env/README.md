...
|ENDPOINT_EXCHANGE_RATE|Base URL of the exchange-rate service|string||Sample API. Empty means the feature is not used, and the endpoint answers 503. Removed together with the sample API|
|ENDPOINT_ADDRESS|Base URL of the postal-code lookup service|string|`https://zipcloud.ibsnet.co.jp`|Sample API. Removed together with the sample API|
