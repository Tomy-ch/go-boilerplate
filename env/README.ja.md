...
|APP_SHUTDOWN_TIMEOUT|Graceful shutdown時間|duration|65s|Code default `65s`。SIGTERM時の待機時間。`SERVER_REQUEST_TIMEOUT` 以上でなければならない（未満だと起動失敗）ため、drain が予算内のリクエストを打ち切ることはない。この検証は全プロセスで走るため、HTTP サーバーを持たないプロセス（worker / job）でこの値を下げる場合は `SERVER_REQUEST_TIMEOUT` も併せて下げること|
