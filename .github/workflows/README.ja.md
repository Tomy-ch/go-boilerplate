...
- **`run:` 本文へ展開された式はコードであり、それを見ているのは zizmor だけ**。`${{ }}` の置換はシェルが構文を解釈するより前に起きるため、未クオートの `github.event.*` はコマンドを終わらせて攻撃者のコマンドを始められる。`actionlint` も `make actions-shellcheck` もこれを捕まえられない。どちらも shellcheck へ渡す前に式をプレースホルダへ潰しており、潰した後はクオートの有無に関わらずただの語だからである。zizmor の `template-injection` は代わりに展開位置そのものを判定し、式の出自が攻撃者制御可能かで重み付けして `github.event.*` の場合を `high` とする。`make actions-zizmor` が `make actions-lint` の中ではなくその隣に pre-commit フックへ載っているのはこのため。示される直し方は、式を `env:` へ束ねてシェル側では `"$VAR"` を読む形で、こうすれば値はデータとして届く
