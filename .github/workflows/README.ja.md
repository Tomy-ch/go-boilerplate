...
週次実行は月曜内で 1 時間ごとにずらしています（`0 0` Trivy FS、`0 1` govulncheck、`0 2` TruffleHog、`0 3` OSV-Scanner、`0 4` Scorecard、`0 5` CodeQL、`0 6` Image Scan、`0 7` gitleaks（全履歴）、`0 8` zizmor（オンライン監査）、`0 9` npm cooldown 監査）。同一時刻に全スキャナが並ぶのを避けるためです。
...
- zizmor の `cache-poisoning` audit は、キャッシュを持つ `setup-*` アクションと publish 的とみなされるトリガーの組み合わせで発火するため、ここのほぼ全ワークフローが該当する。よってファイル単位で潰さず `medium` に remap している。キャッシュは branch-scoped であり、run が復元できるのは自分の ref とデフォルトブランチのキャッシュだけなので、pull request の run が後続の `release/*` push が読むキャッシュを書くことはできない。したがって 1 ファイルだけ `cache: false` にしてもアラートが消えるだけでリスクは減らず、他が追従しない例外が残る。この audit が実害になる形は、`pull_request_target` / `workflow_run` で PR head を checkout **かつ** キャッシュを保存するワークフローである。これらは base ref の scope で動くため、信頼できないコードのキャッシュが特権 run の読む場所に入る。現在そのようなワークフローは無く、追加してはならない
