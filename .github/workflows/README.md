...
#### Codacy's floating tool images

`codacy/codacy-analysis-cli-action` is pinned to a SHA like every other action here, and that pin covers less than it does elsewhere. The action carries no analyser of its own: it starts Docker containers for each language's linter, pulled at run time from Codacy's registry on floating tags. Those images are outside [`.github/actions-pin.toml`](../actions-pin.toml), so `make pin-actions-check` cannot see them, and a rerun of an unchanged commit can therefore produce a different result.

Two settings that would make this worse are deliberately left off. `run-gosec` and `run-staticcheck` each make the action fetch an installer over `curl | sh` from a floating branch and download a `latest` artifact — floating twice over, where the default path floats once. `gosec` already runs as a gate through `go-lint.yaml`, from a version [`mise.toml`](../../mise.toml) pins, so the setting would trade a pinned gate for an unpinned report.

The exception is accepted rather than hidden: a scanner whose rule selection lives in a vendor's project settings is already outside version control, and pinning the images would not bring the rules back.
...
| the lockfile entries in [`.github/actions-pin.toml`](../actions-pin.toml) that nothing else references | `github/codeql-action@v4` — thirteen other workflows upload SARIF with it |
