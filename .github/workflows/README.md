...
- **An expression interpolated into a `run:` body is code, and only zizmor sees that.** `${{ }}` is substituted before the shell parses anything, so an unquoted `github.event.*` value ends the command and starts the attacker's. Neither `actionlint` nor `make actions-shellcheck` can catch it: both blank each expression out to a placeholder before handing the script to shellcheck, and a placeholder is a plain word whether or not it was quoted. zizmor's `template-injection` audit judges the interpolation site instead, grades it by whether the expression's origin is attacker-controllable, and rates the `github.event.*` case `high` — which is why `make actions-zizmor` sits on the pre-commit hook next to `make actions-lint` rather than inside it. The fix it points at is to bind the expression to an `env:` entry and read `"$VAR"` in the shell, where the value arrives as data.
